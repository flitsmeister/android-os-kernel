...
#ifdef CONFIG_SND_SOC_AW8737_SUPPORT
unsigned int GPIO_SPK_EN;
extern int headset_status;
void pa_request_gpio_control(void)
{
	unsigned int ret = 0;
	static struct device_node *node;
	node = of_find_compatible_node(NULL, NULL, "mediatek,mt6768-mt6358-sound");
	ret = of_get_named_gpio(node, "gpio_spk_en", 0);
    pr_debug("key=gpio_spk_en, ret=%d\n", ret);
    if(ret >= 0) {
		GPIO_SPK_EN = (unsigned int )ret;
		gpio_request(GPIO_SPK_EN, "GPIO_SPK_EN");
    }
}
#endif
...
#endif
#ifdef CONFIG_SND_SOC_AW8737_SUPPORT
	/*Turn on the enable pin of the amplifier when no headphone is detected*/
		// if (!headset_status)  // remove for ATA audio loopback test
...
static int mt6768_mt6358_vow_startup(struct snd_pcm_substream *substream)
{
	...
	for_each_rtdcom(rtd, rtdcom) {
       ...
       if (ret < 0) {
               printk("%s: add_codec_controls failed, err %d\n",__func__, ret);
               return ret;
       };
#endif
	return 0;
