...
//wuhao add for I2S Enable and Uplink path control Start
#if defined(CONFIG_SND_SOC_ES7210)
static void StartAudioI2sInAWBHardware_capture(struct snd_pcm_substream *substream)
{
	...
	SetCLkMclk(Soc_Aud_I2S0, substream->runtime->rate); // select I2S  /*Mclk = I2S_APll * [1/(I2s_ck_div+1)]*/
	EnableI2SCLKDiv(Soc_Aud_I2S0_MCKDIV, true);
    u32Audio2ndI2sIn = SampleRateTransform(substream->runtime->rate, Soc_Aud_Digital_Block_I2S_IN_2) << 8;
    ...
    pr_err("%s, AFE_I2S_CON = 0x%x, AFE_DAC_CON1 = 0x%x\n", __func__,
             Afe_Get_Reg(AFE_I2S_CON), Afe_Get_Reg(AFE_DAC_CON1));
...
#endif
//wuhao add for I2S Enable and Uplink path control End
...
#if defined(CONFIG_SND_SOC_ES7210)		
		StartAudioI2sInAWBHardware_capture(substream);
#endif		
		mPrepareDone = true;
	}
	return 0;
