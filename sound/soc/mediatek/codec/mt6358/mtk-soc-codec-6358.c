...
extern int aw87xxx_add_codec_controls(void *component);
#endif
...
static char *aw_profile[] = {"Music", "Off"};/*aw87xxx_acf.bin ???t?D????3??бу*/
...
#define SPK_WARM_UP_TIME        (25)	/* unit is ms */
	if (enable) {
		...
		printk("roco %s:%d AW_DEV_1 :%d\n",__func__,__LINE__,AW_DEV_1);
#endif		
