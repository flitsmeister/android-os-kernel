...
#define	SGM4154X_DEBUG//default enable

#ifdef SGM4154X_DEBUG
#define SGM_TAG                  "[sgm4154x] "
#define SGM_FUN(f)               printk(SGM_TAG"%s %d \n", __FUNCTION__ , __LINE__)
#define SGM_ERR(fmt, args...)    printk(SGM_TAG"%s %d : "fmt, __FUNCTION__, __LINE__, ##args)
#define SGM_LOG(fmt, args...)    printk(SGM_TAG fmt, ##args)
#define SGM_DBG(fmt, args...)    printk(SGM_TAG fmt, ##args)
#else
#define SGM_ERR(fmt, args...)
#define SGM_LOG(fmt, args...)
#define SGM_DBG(fmt, args...)
#endif
...
static int __sgm4154x_read_byte(struct sgm4154x_device *sgm, u8 reg, u8 *data)
{
    ...
    if (ret < 0) {
        SGM_ERR("i2c read fail: can't read from reg 0x%02X\n", reg);
        return ret;
    }
...
static int __sgm4154x_write_byte(struct sgm4154x_device *sgm, int reg, u8 val)
{
    ...
    if (ret < 0) {
        SGM_ERR("i2c write fail: can't write 0x%02X to reg 0x%02X: %d\n",
               val, reg, ret);
        return ret;
    }
    return 0;
...
static int sgm4154x_adc_read_charge_current(struct sgm4154x_device *sgm, u32 *ibus)
{
	int ret;
	u8 ichg;
    unsigned int curr;
...
static int sgm4154x_get_ibus_adc(struct charger_device *chgdev, u32 *ibus)
{
    struct sgm4154x_device *sgm = dev_get_drvdata(&chgdev->dev);
    
    return sgm4154x_adc_read_charge_current(sgm, ibus);
}
...
static int sgm4154x_set_ichrg_curr(struct charger_device *chg_dev, unsigned int uA)
{
	...
	SGM_ERR("xuchen sgm4154x_set_ichrg_curr set uA=%d\n",uA);
...
#endif
	SGM_ERR("xuchen sgm4154x_update_bits SGM4154x_CHRG_CTRL_2 set reg_val=0x%02x\n",reg_val);
...
static int sgm4154x_get_current(struct charger_device *chg_dev, unsigned int *ichg)
{
	u8 reg_val;
	unsigned int ret = 0;
	struct sgm4154x_device *sgm = charger_get_data(chg_dev);
...
static int sgm4154x_set_input_volt_lim(struct charger_device *chg_dev, unsigned int vindpm)
{
	...
	SGM_ERR("xuchen sgm4154x_set_input_volt_lim set reg_val=0x%02x\n",reg_val);
...
static int sgm4154x_set_input_curr_lim(struct charger_device *chg_dev, unsigned int iindpm)
{
	...
	SGM_ERR("xuchen sgm4154x_set_input_curr_lim iindpm=%d\n",iindpm);
		...
		struct sgm4154x_device *sgm, uint32_t vbus_target)
{
	...
	SGM_ERR("Set vbus target %dv\n", vbus_target);
	switch (vbus_target) {
	case 5:
		ret = sgm4154x_set_dpdm(sgm, SGM_DP_DM_VOL_0P6, SGM_DP_DM_VOL_0P0);
		sgm4154x_set_input_volt_lim(sgm->chg_dev, 4200000);
		break;
	case 9:		
...
{	
	...
	{
		sgm4154x_set_input_volt_lim(sgm->chg_dev,4200000); //4.5v
		return 0;
	}
	mdelay(1);
	while(i--){		
			     ...
			     struct sgm4154x_state *state)
{
	...
	if (ret){
		ret = sgm4154x_read_reg(sgm, SGM4154x_CHRG_STAT, &chrg_stat);
		if (ret){
			SGM_ERR("%s read SGM4154x_CHRG_STAT fail\n",__func__);
			return ret;
		}
	}
	...
	if (ret){
		SGM_ERR("%s read SGM4154x_CHRG_CTRL_0 fail\n",__func__);
		return ret;
	}
	...
	if (ret){
		SGM_ERR("%s read SGM4154x_CHRG_CTRL_5 fail\n",__func__);
		return ret;
	}
	...
	if (ret){
		SGM_ERR("%s read SGM4154x_CHRG_CTRL_a fail\n",__func__);
		return ret;
	}
...
static int sgm4154x_dump_register(struct charger_device *chg_dev)
{
	...
	for (i = 0; i < SGM4154x_REG_NUM+1; i++) {
		...
		if (ret != 0) {
			SGM_ERR("[sgm4154x] i2c transfor error\n");
			return 1;
		}
		SGM_ERR("%s,[0x%x]=0x%x ",__func__, i, sgm4154x_reg[i]);
	}
...
static int sgm4154x_hw_chipid_detect(struct sgm4154x_device *sgm)
{
	...
	if (ret < 0)
	{
		SGM_ERR("[%s] read SGM4154x_CHRG_CTRL_b fail\n", __func__);
		return ret;
	}		
	val = val & SGM4154x_PN_MASK;
	SGM_ERR("[%s] Reg[0x0B]=0x%x\n", __func__,val);
		...
		*chg_dev)
{
	...
	SGM_ERR("charging_reset_watch_dog_timer\n");
...
#if (defined(__SGM41542_CHIP_ID__)|| defined(__SGM41516D_CHIP_ID__)|| defined(__SGM41543D_CHIP_ID__)||defined(__SGM41513A_CHIP_ID__))
		...
		*chg_dev,bool is_up)
{
	...
	struct sgm4154x_device *sgm = charger_get_data(chg_dev);
	sgm4154x_update_bits(sgm, SGM4154x_CHRG_CTRL_1,SGM4154x_CHRG_EN, SGM4154x_CHRG_EN);
	ret = sgm4154x_update_bits(sgm, SGM4154x_CHRG_CTRL_d,
				SGM4154x_EN_PUMPX, SGM4154x_EN_PUMPX);
	if (ret < 0)
	{
		SGM_ERR("[%s] read SGM4154x_CHRG_CTRL_d fail\n", __func__);
		return ret;
	}
	if (is_up)
		ret = sgm4154x_update_bits(sgm, SGM4154x_CHRG_CTRL_d,
...
static int sgm4154x_set_ta20_reset(struct charger_device *chg_dev) {
    struct sgm4154x_device *sgm = charger_get_data(chg_dev);
    u8 reg_val;
    int ret;
    ...
    ret = sgm4154x_update_bits(sgm, SGM4154x_CHRG_CTRL_0,SGM4154x_IINDPM_I_MASK, 0x0);

    msleep(300);

    return sgm4154x_update_bits(sgm, SGM4154x_CHRG_CTRL_0,SGM4154x_IINDPM_I_MASK, reg_val);
}
...
static void charger_detect_work_func(struct work_struct *work)
{
	...
	switch(sgm->state.chrg_type) {
		case SGM4154x_USB_SDP:
			SGM_ERR("SGM4154x charger type: SDP\n");
			curr_in_limit = 500000;
			break;

		case SGM4154x_USB_CDP:
			SGM_ERR("SGM4154x charger type: CDP\n");
			curr_in_limit = 1500000;
			break;

		case SGM4154x_USB_DCP:
			SGM_ERR("SGM4154x charger type: DCP\n");
		...
		case SGM4154x_UNKNOWN:
			SGM_ERR("SGM4154x charger type: UNKNOWN\n");
			curr_in_limit = 500000;
			break;	

		default:
			SGM_ERR("SGM4154x charger type: default\n");
			//curr_in_limit = 500000;
			//break;
			return;
	}
...
static irqreturn_t sgm4154x_irq_handler_thread(int irq, void *private)
{
	...
	schedule_delayed_work(&sgm->charge_detect_delayed_work, 10000);
	//power_supply_changed(sgm->charger);
...
static int sgm4154x_hw_init(struct sgm4154x_device *sgm)
{
	...
	ret = sgm4154x_set_ichrg_curr(s_chg_dev_otg,
				bat_info.constant_charge_current_max_ua);
	if (ret) {
		SGM_ERR("xuchen sgm4154x_set_ichrg_curr error\n");
		goto err_out;
	}

	ret = sgm4154x_set_prechrg_curr(sgm, bat_info.precharge_current_ua);
	if (ret) {
		SGM_ERR("xuchen sgm4154x_set_prechrg_curr error\n");
		goto err_out;
	}
	...
	ret = sgm4154x_set_chrg_volt(s_chg_dev_otg,
				bat_info.constant_charge_voltage_max_uv);
	if (ret) {
		SGM_ERR("xuchen sgm4154x_set_chrg_volt error\n");
		goto err_out;
	}

	ret = sgm4154x_set_term_curr(sgm, bat_info.charge_term_current_ua);
	if (ret) {
		SGM_ERR("xuchen sgm4154x_set_term_curr error\n");
		goto err_out;
	}
	...
	if (ret) {
		SGM_ERR("xuchen sgm4154x_set_input_curr_lim error\n");
		goto err_out;
	}
	/* set ovp 6.5V */
	ret = sgm4154x_update_bits(sgm, SGM4154x_CHRG_CTRL_6,
			SGM4154x_VAC_OVP_MASK, 0xC0);
	if (ret)
		goto err_out;	

	ret = sgm4154x_set_recharge_volt(sgm, 200);//100~200mv
	if (ret) {
		SGM_ERR("xuchen sgm4154x_set_recharge_volt error\n");
		goto err_out;
	}
...
static int sgm4154x_enable_chg_type_det(struct charger_device *chg_dev)
{
	struct sgm4154x_device *sgm = charger_get_data(chg_dev);
	struct sgm4154x_state state;
	int ret,charge_type;
	...
	do{
		...
		sgm->state = state;	
		mutex_unlock(&sgm->lock);	
		dpdm_status = sgm4154x_dpdm_detect_is_done(sgm);	
		bq_detect_count++;
		SGM_ERR("%s: count_max=%d,iidet_bit=%d,dpdm_status =%d\n",
	...
	if(!sgm->state.vbus_gd) {
		dev_err(sgm->dev, "sgm4154x_enable_chg_type_det Vbus not present, disable charge\n");
	}
	if(!state.online)
	{
		dev_err(sgm->dev, "sgm4154x_enable_chg_type_det Vbus not online\n");		
	}
	if(!dpdm_status) {
		dev_err(sgm->dev, "sgm4154x_enable_chg_type_det DPDM detecte not done, disable charge\n");
	}
...
static int sgm4154x_update_chg_type(struct charger_device *chg_dev, bool en)
{
	struct sgm4154x_device *sgm = charger_get_data(chg_dev);
	struct sgm4154x_state state;
	int ret,charge_type;
	...
	ret = sgm4154x_set_dpdm(sgm, SGM_DP_DM_VOL_HIZ, SGM_DP_DM_VOL_HIZ);
	do{
		sgm4154x_update_bits(sgm, SGM4154x_CHRG_CTRL_7,
			SGM4154x_IINDET_EN, SGM4154x_IINDET_EN);
		msleep(250);
		ret = sgm4154x_get_state(sgm, &state);
		mutex_lock(&sgm->lock);
		sgm->state = state;
		...
		SGM_ERR("%s: count_max=%d,iidet_bit=%d,dpdm_status =%d\n",
				__func__,bq_detect_count,iidet_bit,
				dpdm_status);
		if((bq_detect_count>2)||(sgm->state.vbus_gd && dpdm_status))
			iidet_bit = 0;
	}while (iidet_bit);
	...
	propval.intval = charge_type;
	SGM_ERR("wang SGM4154x charger type: intval %d\n",propval.intval);
...
static int sgm4154x_is_chip_enabled(struct charger_device *chg_dev, bool *en)
{
        *en = sgm4154x_hw_exist;
        return 0;
}

static int sgm4154x_charger_is_enabled(struct charger_device *chg_dev, bool *en)
{
        int ret = 0;
		u8 chrg_stat = 0;
        ...
        ret = sgm4154x_read_reg(sgm, SGM4154x_CHRG_CTRL_1, &chrg_stat);
        if (ret < 0){
            return ret;
		}	
		chrg_stat = chrg_stat & SGM4154x_CHRG_EN;		
        *en = chrg_stat;
		SGM_ERR("%s : chrg_stat =%d \n",__func__,chrg_stat);
...
static struct charger_ops sgm4154x_chg_ops = {
	...
	.is_enabled = sgm4154x_charger_is_enabled,
	...
	.is_chip_enabled = sgm4154x_is_chip_enabled,
	...
	.reset_ta = sgm4154x_set_ta20_reset,
#else
...
//	.set_ta20_reset = NULL,
	...
	.enable_chg_type_det = sgm4154x_update_chg_type,
	.get_ext_chgtyp = sgm4154x_enable_chg_type_det,
	/* ADC */
    .get_adc = NULL,
	.get_vbus_adc = sgm4154x_get_chrg_volt,
    .get_ibus_adc = sgm4154x_get_ibus_adc,
    .get_ibat_adc = NULL,
    .get_tchg_adc = NULL,
    .get_zcv = NULL,
};
				...
				const struct i2c_device_id *id)
{
	...
	SGM_ERR("xuchen sgm4154x_driver_probe1 is ok\n");
	...
	if (IS_ERR_OR_NULL(sgm->chg_dev)) {
		SGM_ERR("%s: register charger device  failed\n", __func__);
		ret = PTR_ERR(sgm->chg_dev);
		return ret;
	}
	SGM_ERR("xuchen sgm4154x_driver_probe2 is ok\n");
	/* otg regulator */
	...
	SGM_ERR("xuchen sgm4154x_driver_probe3 is ok\n");
	ret = sgm4154x_power_supply_init(sgm, dev);
	if (ret) {
		SGM_ERR("Failed to register power supply\n");
		return ret;
	}
	SGM_ERR("xuchen sgm4154x_driver_probe4 is ok\n");
	...
	SGM_ERR("xuchen sgm4154x_driver_probe5 is ok\n");
	//OTG setting
	...
	otg_in_wakelock = wakeup_source_register(&client->dev,"sgm415xx_otg_wakelock");
	SGM_ERR("xuchen sgm4154x_driver_probe is ok\n");
	return ret;
...
static void sgm4154x_charger_shutdown(struct i2c_client *client)
{
    ...
    ret = sgm4154x_disable_charger(sgm);
    if (ret) {
        SGM_ERR("Failed to disable charger, ret = %d\n", ret);
    }
    SGM_ERR("sgm4154x_charger_shutdown\n");
}
...
static int __init sgm4154x_init(void)
{
	...
	if (i2c_add_driver(&sgm4154x_driver) != 0)
		SGM_ERR("Failed to register sgm4154x i2c driver.\n");
	else
		SGM_ERR("Success to register sgm4154x i2c driver.\n");
