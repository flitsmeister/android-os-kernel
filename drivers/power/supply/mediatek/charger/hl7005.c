...
//#include "linux/sched.h"
#include <uapi/linux/sched.h>//for kernel4.19
...
static int psc5425e_thread(void *unused);
struct task_struct *thread_psc5425e = NULL;
...
const unsigned int CSTH_6937[] = {
	...
	1050000, 1150000, 1250000,1350000
};
...
static int hl7005_charger_enable_otg(struct charger_device *chg_dev, bool en)
{
	...
	if(en){
		set_wake_up_otg(1);//for psc5425E otg watchdog
	}else{
		set_wake_up_otg(0);//for psc5425E otg watchdog
	}
	return 0;
}
...
static int psc5425e_thread(void *unused)
{
	...
	do{
		...
		hl7005_get_vender_code();
		printk("otg_kthread_print i2c reg to reset watchdog for 5425e 0x%x\n",hl7005_get_vender_code());
		ssleep(20);
			      ...
			      const struct i2c_device_id *id)
{
	...
	if (psc5425e_reg3) {
		printk("%s:%d kzh hl7005_driver_probe psc5425e_reg3:0x%x\n",__func__,__LINE__,psc5425e_reg3);
	}else{
...
//#endif
	...
	hl7005_dump_register(info->chg_dev);
			/*****add by mid for 5425e watchdog otg will close 5V problem  ,create a new thread psc5425e_thread********************************/
	if(psc5425e_reg3 == 0xf0){
		thread_psc5425e = kthread_run(psc5425e_thread,0, OTG_NAME);
		if (IS_ERR(thread_psc5425e)) {
			printk("mid register kthread_run failed (%s)\n", __func__);
