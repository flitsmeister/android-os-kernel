...
#include <linux/miscdevice.h>//add by kzh for misc_register
/****************kzh add start***********************************/
enum MID_CHARGER_MODE {
	MID_NORMAL,//100
	CHARGER_MODE_80
};
int mid_charger_mode = MID_NORMAL;//100
/*****************kzh add end**********************************/
struct tag_bootmode {
	u32 size;
	u32 tag;
...
static int charger_routine_thread(void *arg)
{
	...
	while (1) {
		...
		chr_err("Vbat=%d,Ibat=%d,I=%d,VChr=%d,T=%d,Soc=%d:%d,CT:%d:%d hv:%d pd:%d:%d\n",
			...
			info->pd_reset);
        chr_err("charger_thread kzhkzh ########### current mid_charger_mode:%d\n",mid_charger_mode);
...
//###############################################
static ssize_t show_mid_charger_mode(struct device *dev, struct device_attribute *attr,
					       char *buf)
{
	chr_err(" ########### %s:%d mid_charger_mode:%d\n",__func__,__LINE__,mid_charger_mode);
	return sprintf(buf, "%d\n", mid_charger_mode);
}

static ssize_t store_mid_charger_mode(struct device *dev, struct device_attribute *attr,
						const char *buf, size_t size)
{
	unsigned int temp;

	if (kstrtoint(buf, 10, &temp) == 0)//
		mid_charger_mode = temp;
	else
		chr_err("%s: format error!\n", __func__);
...
static DEVICE_ATTR(mid_charger_mode, 0664,
	show_mid_charger_mode, store_mid_charger_mode);
//###############################################
...
static int mtk_charger_probe(struct platform_device *pdev)
{
	...
	ret_device_file = device_create_file(&(pdev->dev),
		&dev_attr_sc_test);
//###########kzh add for ./sys/devices/platform/charger/mid_charger_mode####################################
	ret_device_file = device_create_file(&(pdev->dev), &dev_attr_mid_charger_mode);
//###############################################
	info->chg1_consumer =
