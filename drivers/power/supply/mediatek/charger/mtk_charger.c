...
extern int chrdet_inform_psy_changed(enum charger_type chg_type, bool chg_online);
#endif
//IPF460_UX30 add end
...
static void _get_diso_interrupt_state(void)
{
	...
	if (vol > VBUS_MIN_VOLTAGE/1000 && vol < VBUS_MAX_VOLTAGE/1000) {
		//if (!mt_usb_is_device()) {
		if (gpio_get_value(41 + 64) == 0) {//GPIO160 -- IDDIG
...
static void _get_polling_state(void)
{
...
#if DUAL_DEBUG
	printk("[DISO]	Current vdc_vol 6358 voltage mV = %d\n", vdc_vol);
#endif

	if(vdc_vol <= 2000 && dc_charge){
		dc_charge = KAL_FALSE;
		printk("dc_charge = KAL_FALSE\n");
    }else if(vdc_vol >= 4000 && !dc_charge){
		dc_charge = KAL_TRUE;
		do_charger_detect();
		mtk_charger_plug_in(chg_info, NONSTANDARD_CHARGER);
    }
...
#if DUAL_DEBUG
	printk("[DISO]	Current vusb_vol 6358 voltage mV = %d\n", vusb_vol);
#endif
	...
	if(vusb_vol <= 500 && usb_charge){
		...
		do_charger_detect();
		if(dc_charge){
			chrdet_inform_psy_changed(NONSTANDARD_CHARGER, 1);
			mtk_charger_plug_in(chg_info, NONSTANDARD_CHARGER);//set current
		}
    }else
    if(vusb_vol >= 4000 && !usb_charge){
		usb_charge = KAL_TRUE;
		do_charger_detect();
		if(dc_charge){
			mtk_charger_plug_in(chg_info, NONSTANDARD_CHARGER);//set current
...
#if defined(CONFIG_MTK_DC_USB_INPUT_CHARGER_SUPPORT)
				chg_info = info;
				charging_get_diso_state(&DISO_data);
#endif
//IPF460_UX30 add end
			} else
