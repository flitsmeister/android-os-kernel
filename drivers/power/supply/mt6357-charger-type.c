...
#if defined(CONFIG_MTK_DC_USB_INPUT_CHARGER_SUPPORT) && !defined(CONFIG_TCPC_CLASS)
static int __maybe_unused custom_get_charger_type(struct mtk_charger_type *info)
{
	enum power_supply_usb_type type;
	unsigned int dc_vol = 0, usb_vol = 0;
...
#ifndef CONFIG_MTK_DC_USB_INPUT_CHARGER_SUPPORT
//#ifndef CONFIG_TCPC_CLASS
#ifdef CONFIG_MID_DOCKING_SUPPORT
       if (1 == get_typec_status()){
#else
	if (!mt_usb_is_device()) {
#endif
			...
			const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_ONLINE:
#if defined(CONFIG_MTK_DC_USB_INPUT_CHARGER_SUPPORT) && !defined(CONFIG_TCPC_CLASS)
		info->type = custom_get_charger_type(info);
#else
