...
struct wakeup_source *otg_in_wakelock;

static int psc5425e_reg3 = 0; // add by mid for psc5425e_reg3 charger ic
...
//5425E  reg[0x03] = 0xf0
const unsigned int CSTH_5425E[] = {
	1000000, 1200000, 1600000, 1800000,
	2000000, 2400000, 2700000, 3000000
};
//7007  reg[0x03] = 0x41
const unsigned int CSTH_7007[] = {
	550000, 650000, 750000, 850000,
	950000, 1050000, 1150000, 1250000,
	1350000,1450000,1650000,1750000,
	1825000,2050000,2175000,2350000
};
//6937  reg[0x03] = 0x54
const unsigned int CSTH_6937[] = {
	550000, 650000, 750000, 850000,
	950000, 1050000, 1150000, 1250000,
...
#if (!defined (CONFIG_ROCO_OTG_EN)&&!defined (CONFIG_EXTCON_USB_POGO))||(defined CONFIG_OTG_SEPARATE_POGO)
...
void eta6937_set_iocharge(unsigned int val)
{
	hl7005_config_interface((unsigned char)(HL7005_CON4),
				(unsigned char)(val),
				(unsigned char)(ETA6937_CON4_I_CHR_MASK),
				(unsigned char)(ETA6937_CON4_I_CHR_SHIFT)
				);
}
...
void eta6937_set_ichg3(unsigned int val)
{
	hl7005_config_interface((unsigned char)(HL7005_CON5),
				(unsigned char)(val),
				(unsigned char)(CON5_ICHG3_MASK),
				(unsigned char)(CON5_ICHG3_SHIFT)
				);
}
void eta6937_set_ichg4(unsigned int val)
{
	hl7005_config_interface((unsigned char)(HL7005_CON5),
				(unsigned char)(val),
				(unsigned char)(CON5_ICHG4_MASK),
				(unsigned char)(CON5_ICHG4_SHIFT)
				);
}
...
static int hl7005_dump_register(struct charger_device *chg_dev)
{
	...
	for (i = 0; i < HL7005_REG_NUM; i++) {
		...
		psc5425e_reg3 = hl7005_reg[3];// add by mid for psc5425e_reg3 charger ic
		pr_err("kzh[0x%x]=0x%x ", i, hl7005_reg[i]);
	}
	if ((0x54 == psc5425e_reg3)||(0x52 == psc5425e_reg3)){
		hl7005_set_tmr_rst(1);
		#ifdef CONFIG_MID_MT6739_BAT_4V4
			hl7005_reg_config_interface(0x06,0x7d);	//2A, 4.4V
			hl7005_reg_config_interface(0x00,0xC0);
		#else
			hl7005_reg_config_interface(0x06,0x77);	//2A, 4.34V
			hl7005_reg_config_interface(0x00,0xC0);
		#endif
	}
	pr_debug("\n");
...
static int hl7005_set_current(struct charger_device *chg_dev, u32 current_value)
{
	...
	pr_info("%s current_value:%d\n",__func__,current_value);
	if (current_value <= 35000) {
		hl7005_set_io_level(1);
	} else {
		hl7005_set_io_level(0);
		if(0xf0 == psc5425e_reg3)//PR5425E
		{
			array_size = ARRAY_SIZE(CSTH_5425E);
			set_chr_current = bmt_find_closest_level(CSTH_5425E, array_size, current_value);
			register_value = charging_parameter_to_value(CSTH_5425E, array_size, set_chr_current);
			register_value = (register_value<<1);
			printk("%s:%d psc5425e_reg3:0x%x  register_value=%d current_value=%d\n",__func__,__LINE__,psc5425e_reg3,register_value,current_value);
			hl7005_set_iocharge(register_value);
		}
		else if ((0x54 == psc5425e_reg3)||(0x52 == psc5425e_reg3))//ETA6937
		{
         if ((current_value>1350000)&&(current_value<=2150000))
            {
            current_value = current_value-800000;
		    eta6937_set_ichg3(1);		 	
			eta6937_set_ichg4(0);		 	
            }
	     else if ((current_value>2150000)&&(current_value<=3150000))
	     	{
	     	current_value = current_value-1600000;
		    eta6937_set_ichg4(1);			
			eta6937_set_ichg3(0);			
	     	}
		 else{
			...
			array_size = ARRAY_SIZE(CSTH_6937);
			set_chr_current = bmt_find_closest_level(CSTH_6937, array_size, current_value);
			register_value = charging_parameter_to_value(CSTH_6937, array_size, set_chr_current);

			register_value = (register_value<<1);
			printk("%s:%d ETA6937:0x%x  register_value=%d current_value=%d\n",__func__,__LINE__,psc5425e_reg3,register_value,current_value);
			eta6937_set_iocharge(register_value);
		}
		else if (0x41 == psc5425e_reg3)//HL7007
		{
			array_size = ARRAY_SIZE(CSTH_7007);
			set_chr_current = bmt_find_closest_level(CSTH_7007, array_size, current_value);
			register_value = charging_parameter_to_value(CSTH_7007, array_size, set_chr_current);
			printk("%s:%d HL7007:0x%x  register_value=%d current_value=%d\n",__func__,__LINE__,psc5425e_reg3,register_value,current_value);
			hl7005_set_iocharge(register_value);			
		}		
		else{
		array_size = ARRAY_SIZE(CSTH);
...
EXPORT_SYMBOL(hl7005_enable_otg);
...
static void enable_boost_polling(bool poll_en)
{
	...
	if (g_info) {
		if (poll_en) {
			...
			alarm_start_relative(&g_info->otg_kthread_gtimer, ms_to_ktime(g_info->polling_interval*1000));
...
static void usbotg_boost_kick_work(struct work_struct *work)
{
	...
	if (0x54 == psc5425e_reg3||0x52 == psc5425e_reg3||0x41 == psc5425e_reg3)
	hl7005_set_tmr_rst(1);
	
	hl7005_dump_register(NULL);
	...
	if (boost_manager->polling_enabled == true) {
		...
		timespec_add(time_now, time);
		ktime = ktime_set(end_time.tv_sec, end_time.tv_nsec);
		alarm_start_relative(&boost_manager->otg_kthread_gtimer, ms_to_ktime(boost_manager->polling_interval*1000));
	}
}
			      ...
			      const struct i2c_device_id *id)
{
	...
	psc5425e_reg3 = hl7005_get_vender_code();

	if (psc5425e_reg3) {
		printk("%s:%d kzhkzh hl7005_driver_probe psc5425e_reg3:0x%x\n",__func__,__LINE__,psc5425e_reg3);
	}else{
...
//#endif
	...
	hl7005_reg_config_interface(0x01, 0xF8);
	hl7005_reg_config_interface(0x05, 0x03);
	/* 146mA */
	hl7005_reg_config_interface(0x04, 0x1A);

	if((0x54 == psc5425e_reg3)||(0x52 == psc5425e_reg3))//if(psc5425e_reg3 == 0x54)	
	{       
	    hl7005_reg_config_interface(0x07, 0x0d);
	    //fan5405_config_interface(0x07,0x0d,0xFF, 0);
	    printk("eta6937 fan5405_driver_probe !!!   psc5425e_reg3=0x%x \n",psc5425e_reg3);    
	}	
	...
	g_info = info;
	otg_in_wakelock = wakeup_source_register(&client->dev,"hl7005_otg_wakelock"); 
	return 0;
}
