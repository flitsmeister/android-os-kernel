...
/*sgm41513 REG04 VREG[7:3] BQ25601*/
const unsigned int VBAT_CV_VTH[] = {
	...
	4335300, 4400000, 4432000, 4464000,
	4496000, 4528000, 4560000, 4592000,
	4624000

};

/*sgm41513 REG02 ICHG[6:0]*/
...
/*	0, 6000, 12000, 18000, 24000,
	...
	0,500,1000,1500,2000,2500,3000,3500,4000,5000,6000,7000,8000,9000,10000,11000,
	13000,15000,17000,19000,21000,23000,25000,27000,30000,33000,36000,39000,42000,45000,48000,51000,
	54000,60000,66000,72000,78000,84000,90000,102000,108000,114000,120000,126000,132000,138000,144000,
	150000,162000,174000,186000,198000,210000,222000,234000,246000,258000,270000,282000,294000,300000,300000,300000

};

/*sgm41513 REG00 IINLIM[4:0] BQ25601*/
const unsigned int INPUT_CS_VTH[] = {
	...
	300000, 310000, 315000, 320000
	/*24000,25000,26000,27000,28000,29000,30000,31000,32000,32000,32000,32000*/
};
...
/* sgm41513 REG0A BOOST_LIM[2:0], mA */
 ...
 *
 *********************************************************/
unsigned char sgm41513_reg[sgm41513_REG_NUM] = { 0 };

static DEFINE_MUTEX(sgm41513_i2c_access);
static DEFINE_MUTEX(sgm41513_access_lock);

int g_sgm41513_hw_exist;

/**********************************************************
 *
 *   [I2C Function For Read/Write sgm41513]
 *
 *********************************************************/
#ifdef CONFIG_MTK_I2C_EXTENSION
unsigned int sgm41513_read_byte(unsigned char cmd,
...
unsigned int sgm41513_write_byte(unsigned char cmd,
 ...
 *
 *********************************************************/
unsigned int sgm41513_read_interface(unsigned char RegNum,
...
unsigned int sgm41513_config_interface(unsigned char RegNum,
				      unsigned char val, unsigned char MASK,
				      unsigned char SHIFT)
{
	unsigned char sgm41513_reg = 0;
	...
	pr_debug_ratelimited("[%s] write Reg[%x]=0x%x from 0x%x\n", __func__,
			     RegNum,
			     sgm41513_reg, sgm41513_reg_ori);

	/* Check */
	/* sgm41513_read_byte(RegNum, &sgm41513_reg); */
...
unsigned int sgm41513_reg_config_interface(unsigned char RegNum,
		unsigned char val)
{
	unsigned int ret = 0;

	ret = sgm41513_write_byte(RegNum, val);
 ...
 *********************************************************/
/* CON0---------------------------------------------------- */
void sgm41513_set_en_hiz(unsigned int val)
{
	unsigned int ret = 0;

	ret = sgm41513_config_interface((unsigned char) (sgm41513_CON0),
				       (unsigned char) (val),
...
void sgm41513_set_iinlim(unsigned int val)
{
	unsigned int ret = 0;
...
void sgm41513_set_stat_ctrl(unsigned int val)
{
	unsigned int ret = 0;
...
void sgm41513_set_reg_rst(unsigned int val)
{
	unsigned int ret = 0;

	ret = sgm41513_config_interface((unsigned char) (sgm41513_CON11),
				       (unsigned char) (val),
...
void sgm41513_set_pfm(unsigned int val)
{
	unsigned int ret = 0;

	ret = sgm41513_config_interface((unsigned char) (sgm41513_CON1),
				       (unsigned char) (val),
...
void sgm41513_set_wdt_rst(unsigned int val)
{
	unsigned int ret = 0;
...
void sgm41513_set_otg_config(unsigned int val)
{
	unsigned int ret = 0;
...
unsigned int sgm41513_get_otg_config(void)
{
	unsigned int ret = 0;
	unsigned char val = 0;

	ret = sgm41513_read_interface((unsigned char) (sgm41513_CON1),
				     (&val),
				     (unsigned char) (CON1_OTG_CONFIG_MASK),
				     (unsigned char) (CON1_OTG_CONFIG_SHIFT)
				    );
...
void sgm41513_set_chg_config(unsigned int val)
{
	unsigned int ret = 0;
...
void sgm41513_set_sys_min(unsigned int val)
{
	unsigned int ret = 0;
...
void sgm41513_set_batlowv(unsigned int val)
{
	unsigned int ret = 0;
...
/* CON2---------------------------------------------------- */
void sgm41513_set_rdson(unsigned int val)
{
	unsigned int ret = 0;

	ret = sgm41513_config_interface((unsigned char) (sgm41513_CON2),
				       (unsigned char) (val),
...
void sgm41513_set_boost_lim(unsigned int val)
{
	unsigned int ret = 0;
...
void sgm41513_set_ichg(unsigned int val)
{
	unsigned int ret = 0;
...
#ifdef FIXME //this function does not exist on sgm41513
void sgm41513_set_force_20pct(unsigned int val)
{
	unsigned int ret = 0;
...
void sgm41513_set_iprechg(unsigned int val)
{
	unsigned int ret = 0;

	ret = sgm41513_config_interface((unsigned char) (sgm41513_CON3),
				       (unsigned char) (val),
...
void sgm41513_set_iterm(unsigned int val)
{
	unsigned int ret = 0;
...
void sgm41513_set_vreg(unsigned int val)
{
	unsigned int ret = 0;

	ret = sgm41513_config_interface((unsigned char) (sgm41513_CON4),
				       (unsigned char) (val),
...
void sgm41513_set_topoff_timer(unsigned int val)
{
	unsigned int ret = 0;
...
void sgm41513_set_vrechg(unsigned int val)
{
	unsigned int ret = 0;
...
void sgm41513_set_en_term(unsigned int val)
{
	unsigned int ret = 0;

	ret = sgm41513_config_interface((unsigned char) (sgm41513_CON5),
				       (unsigned char) (val),
...
void sgm41513_set_watchdog(unsigned int val)
{
	unsigned int ret = 0;
...
void sgm41513_set_en_timer(unsigned int val)
{
	unsigned int ret = 0;
...
void sgm41513_set_chg_timer(unsigned int val)
{
	unsigned int ret = 0;
...
void sgm41513_set_treg(unsigned int val)
{
#ifdef FIXME
	unsigned int ret = 0;

	ret = sgm41513_config_interface((unsigned char) (sgm41513_CON6),
				       (unsigned char) (val),
...
void sgm41513_set_vindpm(unsigned int val)
{
	unsigned int ret = 0;
...
void sgm41513_set_ovp(unsigned int val)
{
	unsigned int ret = 0;
...
void sgm41513_set_boostv(unsigned int val)
{
...
void sgm41513_set_tmr2x_en(unsigned int val)
{
	unsigned int ret = 0;

	ret = sgm41513_config_interface((unsigned char) (sgm41513_CON7),
					(unsigned char) (val),
...
void sgm41513_set_batfet_disable(unsigned int val)
{
	unsigned int ret = 0;
...
void sgm41513_set_batfet_delay(unsigned int val)
{
	unsigned int ret = 0;
...
void sgm41513_set_batfet_reset_enable(unsigned int val)
{
	unsigned int ret = 0;
...
unsigned int sgm41513_get_system_status(void)
{
	unsigned int ret = 0;
	unsigned char val = 0;

	ret = sgm41513_read_interface((unsigned char) (sgm41513_CON8),
...
unsigned int sgm41513_get_vbus_stat(void)
{
	unsigned int ret = 0;
	unsigned char val = 0;
...
unsigned int sgm41513_get_chrg_stat(void)
{
	unsigned int ret = 0;
	unsigned char val = 0;
...
unsigned int sgm41513_get_vsys_stat(void)
{
	unsigned int ret = 0;
	unsigned char val = 0;
...
unsigned int sgm41513_get_pg_stat(void)
{
	unsigned int ret = 0;
	unsigned char val = 0;
...
void sgm41513_set_int_mask(unsigned int val)
{
	unsigned int ret = 0;

	ret = sgm41513_config_interface((unsigned char) (sgm41513_CON10),
				       (unsigned char) (val),
...
static int sgm41513_dump_register(struct charger_device *chg_dev)
{
	...
	for (i = 0; i <= sgm41513_REG_NUM; i++) {
		...
		if (ret == 0) {
			pr_info("[sgm41513] i2c transfor error\n");
			return 1;
		}
		pr_info("[0x%x]=0x%x ", i, sgm41513_reg[i]);
	}
	pr_debug("\n");
	return 0;
}
...
static void sgm41513_hw_component_detect(void)
{
	...
	//ret = sgm41513_read_interface(sgm41513_CON11, &val, 0xFF, 0x0);
    ret = sgm41513_read_byte(sgm41513_CON11, &val);
	printk("[%s] exist=%d, %d Reg[0x0B]=0x%x\n", __func__,__LINE__,
		g_sgm41513_hw_exist, val);
	...
	pr_info("[%s] exist=%d, WANG Reg[0x0B]=0x%x\n", __func__,
		g_sgm41513_hw_exist, val);
}
...
static int sgm41513_get_current(struct charger_device *chg_dev,
			       u32 *ichg)
{
	...
	/* Get Force 20% option */
	sgm41513_read_interface(sgm41513_CON2, &ret_force_20pct,
...
static unsigned int charging_hw_init(void)
{
	...
	sgm41513_set_en_hiz(0x0);
	sgm41513_set_vindpm(0x6);	/* VIN DPM check 4.6V */
	sgm41513_set_wdt_rst(0x1);	/* Kick watchdog */
	sgm41513_set_sys_min(0x5);	/* Minimum system voltage 3.5V */
	sgm41513_set_iprechg(0xF);	/* Precharge current 540mA */
	sgm41513_set_iterm(0xC);	/* Termination current 180mA */
			    ...
			    struct device *dev)
{
	...
	/*
	 * sgm41513_en_pin = of_get_named_gpio(np,"gpio_sgm41513_en",0);
	 * if(sgm41513_en_pin < 0){
	 * pr_info("%s: no sgm41513_en_pin\n", __func__);
	 * return -ENODATA;
	 * }
	 * gpio_request(sgm41513_en_pin,"sgm41513_en_pin");
	 * gpio_direction_output(sgm41513_en_pin,0);
	 * gpio_set_value(sgm41513_en_pin,0);
	 *
	 *
			    ...
			    u32 args)
{
	...
	info = (struct sgm41513_info *)dev_get_drvdata(&chg_dev->dev);
	if (info == NULL)
		return -EINVAL;
...
static int sgm41513_is_enabled_vbus(struct regulator_dev *rdev)
{
	return sgm41513_get_otg_config();
}

static const struct regulator_ops sgm41513_vbus_ops = {
	.enable = sgm41513_enable_vbus,
	.disable = sgm41513_disable_vbus,
	.is_enabled = sgm41513_is_enabled_vbus,
};
...
static enum power_supply_property sgm41513_psy_properties[] = {
	POWER_SUPPLY_PROP_STATUS,
	POWER_SUPPLY_PROP_ONLINE,
	POWER_SUPPLY_PROP_USB_TYPE,
};

static enum power_supply_usb_type sgm41513_usb_types[] = {
...
static int psy_sgm41513_get_property(struct power_supply *psy,
	enum power_supply_property psp, union power_supply_propval *val)
{
	...
	info = (struct sgm41513_info *)power_supply_get_drvdata(psy);
	if (info == NULL) {
...
static char *sgm41513_supplied_to[] = {
	"battery"
};
				...
				const struct i2c_device_id *id)
{
	...
	sgm41513_hw_component_detect();//读取硬件是否存在
	charging_hw_init();  //寄存器初始化
	...
	info->psy_desc.name = "charger_sgm41513";
	info->psy_desc.type = POWER_SUPPLY_TYPE_UNKNOWN;
	info->psy_desc.properties = sgm41513_psy_properties;
	info->psy_desc.num_properties = ARRAY_SIZE(sgm41513_psy_properties);
	info->psy_desc.get_property = psy_sgm41513_get_property;
	info->psy_desc.usb_types = sgm41513_usb_types,
	info->psy_desc.num_usb_types = ARRAY_SIZE(sgm41513_usb_types),

	info->psy_cfg.drv_data = info;
	info->psy_cfg.of_node = client->dev.of_node;
	info->psy_cfg.supplied_to = sgm41513_supplied_to;
	info->psy_cfg.num_supplicants = ARRAY_SIZE(sgm41513_supplied_to);
 ...
 *
 *********************************************************/
unsigned char g_reg_value_sgm41513;
static ssize_t sgm41513_access_show(struct device *dev,
				   struct device_attribute *attr, char *buf)
{
	pr_info("[%s] 0x%x\n", __func__, g_reg_value_sgm41513);
				    ...
				    const char *buf, size_t size)
{
	...
	if (buf != NULL && size != 0) {
		...
		if (size > 3) {
			...
			pr_info(
			"[%s] write sgm41513 reg 0x%x with value 0x%x !\n",
			__func__,
			...
			ret = sgm41513_read_interface(reg_address,
					     &g_reg_value_sgm41513, 0xFF, 0x0);
			pr_info(
			"[%s] read sgm41513 reg 0x%x with value 0x%x !\n",
			__func__,
			(unsigned int) reg_address, g_reg_value_sgm41513);
			pr_info(
			"[%s] use \"cat sgm41513_access\" to get value\n",
			__func__);
		}
	}
	return size;
...
struct platform_device sgm41513_user_space_device = {
	.name = "sgm41513-user",
	.id = -1,
};
...
#else
static struct i2c_board_info i2c_sgm41513 __initdata = {
	I2C_BOARD_INFO("sgm41513", (sgm41513_SLAVE_ADDR_WRITE >> 1))
};
#endif
...
static int __init sgm41513_init(void)
{
	...
	if (i2c_add_driver(&sgm41513_driver) != 0) {
		pr_info(
			"[%s] failed to register sgm41513 i2c driver.\n",
			__func__);
	} else {
		pr_info(
			"[%s] Success to register sgm41513 i2c driver.\n",
			__func__);
	}
