...
//mid add begin
#if defined(CONFIG_MTK_DC_USB_INPUT_CHARGER_SUPPORT) && !defined(CONFIG_TCPC_CLASS)
void __maybe_unused dc_usb_charger_plug_in(void){
	mtk_charger_plug_in(chg_info, POWER_SUPPLY_TYPE_USB_FLOAT);
}
#endif
//mid add end
...
#ifdef CONFIG_MID_KPOC_UNPLUG_DETECT
    int vbus = 0;
    unsigned int boot_mode;
    boot_mode = info->bootmode;
#endif
...
#ifdef CONFIG_MID_KPOC_UNPLUG_DETECT		
		vbus = get_vbus(info);
		if ((boot_mode == 8 || boot_mode == 9)&& (vbus < 2500)) {		
			chr_err("Unplug Charger/USB in KPOC mode, shutdown! shutdown! shutdown!\n");
			kernel_power_off();
			orderly_poweroff(true);
		}
#endif		
