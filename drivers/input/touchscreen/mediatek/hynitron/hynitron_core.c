...
#if 1//getsure resume
static int tpd_suspend_getsure_enable = 0xff;
static int tpd_suspend_getsure_report_done = 0xff;
static int tpd_suspend_down_state = 0xff;
static int tpd_suspend_geture_count = 0xff;
#endif
...
extern void hyn_sub_suspend(void);
extern void hyn_sub_resume(void);
/*****************************************************************************
...
#if 0
{
	...
	printk("cst3xx_i2c_read_register: %d, %d, %d, %d, %d, %d, %d\n", buf[0], buf[1], buf[2], buf[3], buf[4], buf[5], buf[6]);
	if(ret < 0) {
		HYN_ERROR("  iic read touch point data failed.\n");
		goto OUT_PROCESS;
	}
...
#if 1//getsure resume
	if(tpd_suspend_getsure_enable==1)  tpd_suspend_down_state  = buf[0] & 0x7F;
#endif
	cnt = buf[5] & 0x7F;
...
#endif
#if 1//getsure resume
	if(tpd_suspend_getsure_enable == 1)
	{
		printk(" cst3xx Point tpd_suspend_getsure_enable:%d, tpd_suspend_down_state:%d, tpd_suspend_geture_count:%d, tpd_suspend_getsure_report_done:%d. \n", tpd_suspend_getsure_enable, tpd_suspend_down_state, tpd_suspend_geture_count, tpd_suspend_getsure_report_done);
		if(cnt==1){
			if((tpd_suspend_down_state&0x0F) == 0x06) {
				if(tpd_suspend_geture_count < 200)  tpd_suspend_geture_count++;
			}else{
				if((tpd_suspend_getsure_report_done == 0)&&(tpd_suspend_geture_count > 3)&&(tpd_suspend_geture_count < 100)){
					tpd_suspend_getsure_report_done = 1;
...
static int hyn_parse_dt(struct device *dev, struct hynitron_ts_platform_data *pdata)
{
	...
	pdata->reset_gpio = of_get_named_gpio(np, "hynitron,reset-gpio", 0);
...
#if 0//delete for getsure resume
    hyn_release_all_finger();
