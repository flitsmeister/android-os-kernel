...
int match_gsl_tp_id(char *read_buf)
{
	...
	if(GSL_TP_ID_USED) {
		...
		printk("%s : name_tp_mode=%s, real top_name=%s, GSL_TP_ID_USED = %d\n", __func__, name_tp_mode, NAME_TP, GSL_TP_ID_USED);
	}
	return 0;
}
...
int tp_list_config(void)
{
	...
	printk("\n[%s] name_tp_mode =%s, gsl_tp_info[%d].name = %s\n",__func__, name_tp_mode, i, gsl_tp_info[i].name);
	if (i == sizeoflist)
	{
	...
	printk("\n[%s] TP_NUM =%d, KEY_NUM = %d\n",__func__, TP_NUM, KEY_NUM);
	...
	if( 1 != TP_NUM && 0 != TP_NUM) {  //项目配置有多个TP
		...
		if( KEY_NUM )   { 
			PTR_TPD_KEYS_DIM_LOCAL = PTR_TP_PROJECT_DATA[0].tpd_keys; 
		} 
...
int tp_input_orientation_config(void)
{
	...
	if(item_exist("ro.vendor.sf.hwrotation"))
		sf_hwrotation = item_integer("ro.vendor.sf.hwrotation",0);
	else
		sf_hwrotation = -1;
