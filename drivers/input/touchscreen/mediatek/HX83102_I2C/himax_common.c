...
static int i_get_FW(void)
{
	...
	int result = NO_ERR;
	int retry = 5;
	//ret = request_firmware(&hxfw, g_fw_boot_upgrade_name, private_ts->dev); //origin code
	...
	while((retry > 0)){
		...
		msleep(50);//每个循环sleep50
		printk("kzhkzh %s:%d ret:%d retry:%d\n",__func__,__LINE__,ret,retry);
		retry--;
		if(ret == 0)
		break;
	}
...
static void himax_point_report(struct himax_ts_data *ts)
{
	...
	for (i = 0; i < ts->nFinger_support; i++) {
		...
		if(1 == xy_swap)
		{
			input_tmp = g_target_report_data->p[i].x;
			g_target_report_data->p[i].x = g_target_report_data->p[i].y;
			g_target_report_data->p[i].y = input_tmp;
		}

		if(1 == x_reverse)
			g_target_report_data->p[i].x = ts->pdata->abs_x_max - g_target_report_data->p[i].x;

		if(1 == y_reverse)
			g_target_report_data->p[i].y = ts->pdata->abs_y_max - g_target_report_data->p[i].y;

		if(1 == xy_deal)
		{
			g_target_report_data->p[i].x = g_target_report_data->p[i].x * ts->pdata->abs_y_max / ts->pdata->abs_x_max;
			g_target_report_data->p[i].y = g_target_report_data->p[i].y * ts->pdata->abs_x_max / ts->pdata->abs_y_max;
		}
#endif	
...
/*			x_tmp = g_target_report_data->p[i].x;
			y_tmp = g_target_report_data->p[i].y;
			g_target_report_data->p[i].x = ((ts->pdata->abs_y_max - y_tmp)*ts->pdata->abs_x_max/ts->pdata->abs_y_max);
			g_target_report_data->p[i].y = (x_tmp * ts->pdata->abs_y_max/ts->pdata->abs_x_max);

 			input_report_abs(ts->input_dev, ABS_MT_POSITION_X,
					ts->pdata->abs_x_max - g_target_report_data->p[i].x);
			input_report_abs(ts->input_dev, ABS_MT_POSITION_Y,
					ts->pdata->abs_y_max - g_target_report_data->p[i].y); */
			input_report_abs(ts->input_dev, ABS_MT_POSITION_X,
						g_target_report_data->p[i].x);
			input_report_abs(ts->input_dev, ABS_MT_POSITION_Y,
						g_target_report_data->p[i].y);					
