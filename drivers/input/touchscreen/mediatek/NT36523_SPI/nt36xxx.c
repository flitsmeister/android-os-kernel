...
//struct pinctrl_state *gpio22_output0, *gpio22_as_spics;
...
info_retry:
	...
	ts->x_num = buf[3];
	ts->y_num = buf[4];
	//ts->abs_x_max = (uint16_t)((buf[7] << 8) | buf[8]);
	//ts->abs_y_max = (uint16_t)((buf[5] << 8) | buf[6]);
	ts->abs_x_max = (uint16_t)((buf[5] << 8) | buf[6]);
	ts->abs_y_max = (uint16_t)((buf[7] << 8) | buf[8]);
	NVT_LOG("ts->abs_x_max = %d, ts->abs_y_max = %d\n", ts->abs_x_max, ts->abs_y_max);
	...
	printk("fw_ver = 0x%02X, fw_type = 0x%02X\n", ts->fw_ver, buf[14]);
...
static int32_t nvt_parse_dt(struct device *dev)
{
	...
	//mid add for get width and Height from dts
	ret = of_property_read_u32_array(np, "novatek,panel-coords",
			mid_resolution, ARRAY_SIZE(mid_resolution));
	if (ret) {
		NVT_LOG("Joyar add not support novatek,display-coords from dts,use h file definition\n");
		ts->abs_x_max = TOUCH_DEFAULT_MAX_WIDTH;
		ts->abs_y_max = TOUCH_DEFAULT_MAX_HEIGHT;
		ret = 0;
	} else {
		ts->abs_x_max = mid_resolution[0];
		ts->abs_y_max = mid_resolution[1];
		NVT_LOG(" Joyar DT-%s:panel-coords = x:%d, Y:%d\n", __func__,
				mid_resolution[0],
				mid_resolution[1]);
	}
   ...
   if (POINT_DATA_LEN >= POINT_DATA_CHECKSUM_LEN) {
	...
	for (i = 0; i < ts->max_touch_num; i++) {
		...
		if (((point_data[position] & 0x07) == 0x01) || ((point_data[position] & 0x07) == 0x02)) {	//finger down (enter & moving)
			...
			//printk("input_x = %d, input_y = %d, x_max = %d, y_max = %d \n", input_x, input_y, ts->abs_x_max, ts->abs_y_max);
#ifdef CONFIG_MID_ITEMS_SUPPORT
			if(1 == xy_swap)
			{
				input_tmp = input_x;
			...
			if(1 == xy_deal)
			{
				input_x = input_x * ts->abs_y_max / ts->abs_x_max;
				input_y = input_y * ts->abs_x_max / ts->abs_y_max;
			}
#endif
			//printk("333 input_x = %d, input_y = %d \n", input_x, input_y);
...
/*
	pinctrl2 = devm_pinctrl_get(&client->dev);

	gpio22_output0 = pinctrl_lookup_state(pinctrl2, "state_gpio22_output0");
	if (IS_ERR(gpio22_output0)) {
		ret = PTR_ERR(gpio22_output0);
		printk("[spi_touch]Cannot find pinctrl state_gpio22_output0!\n");
	}
	gpio22_as_spics = pinctrl_lookup_state(pinctrl2, "state_gpio22_as_spics");
	if (IS_ERR(gpio22_as_spics)) {
		ret = PTR_ERR(gpio22_as_spics);
		printk("[spi_touch]Cannot find pinctrl state_gpio22_as_spics!\n");
	}
	pinctrl_select_state(pinctrl2, gpio22_as_spics);
*/
	mutex_init(&ts->lock);
...
static int32_t nvt_ts_suspend(struct device *dev)
{
	...
	msleep(50);
//	pinctrl_select_state(pinctrl2, gpio22_output0);
	NVT_LOG("end\n");
