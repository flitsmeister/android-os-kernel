...
{
    FTS_DEBUG("tp reset");
    gpio_set_value(fts_data->pdata->reset_gpio, 1);
    msleep(10);
    gpio_set_value(fts_data->pdata->reset_gpio, 0);
    msleep(1);
    gpio_set_value(fts_data->pdata->reset_gpio, 1);
    if (hdelayms) {
        msleep(hdelayms);
    ...
    do {
        ...
        if ((ret < 0) || (0x0 == chip_id[0]) || (0x0 == chip_id[1])) {
            FTS_DEBUG("i2c read invalid, read:0x%02x%02x",
                      chip_id[0], chip_id[1]);
			fts_reset_proc(20);   //yanjb20200601
        } else {
...
static int fts_irq_registration(struct fts_ts_data *ts_data)
{
    ...
    ts_data->pdata->irq_gpio = of_get_named_gpio(node, "fts,irq-gpio", 0);
    FTS_INFO("fts,irq-gpio=%d\n", ts_data->pdata->irq_gpio);
...
static int fts_input_init(struct fts_ts_data *ts_data)
{
	...
	FTS_INFO("TPD_RES_X: %d, TPD_RES_Y = %d \n", pdata->x_max, pdata->y_max);
#if FTS_REPORT_PRESSURE_EN
...
static int fts_gpio_configure(struct fts_ts_data *ts_data)
{
	...
	np = of_find_matching_node(np, fts_dt_match);
    if (NULL == np) {
        FTS_ERROR("Can not find touch eint device node!");
        return -ENODATA;
    }

    ts_data->pdata->reset_gpio = of_get_named_gpio(np, "fts,reset-gpio", 0);
    FTS_INFO("fts,reset-gpio=%d\n", ts_data->pdata->reset_gpio);

    /* request RST-pin (Output/High) */
    if (gpio_is_valid(ts_data->pdata->reset_gpio)) {
        ret = gpio_request(ts_data->pdata->reset_gpio, "FST-tp-rst");
        if (ret) {
            FTS_DEBUG("Failed to request FST-tp-rst GPIO\n");
        }
    }
...
static void fts_platform_data_init(struct fts_ts_data *ts_data)
{
    ...
    pdata->irq_gpio = 0;
    pdata->reset_gpio = 164;
    pdata->x_min = 0;
    pdata->x_max = DISP_GetScreenWidth();
    pdata->y_min = 0;
    pdata->y_max = DISP_GetScreenHeight();
...
static int __init tpd_driver_init(void)
{
    ...
    if (i2c_add_driver(&fts_ts_driver)) {
