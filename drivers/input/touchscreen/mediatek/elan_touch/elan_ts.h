...
struct elan_finger_struct {
    ...
    bool recv_2nd_packet;   /*for hid protocol, if fingernum > 5 ? true:false*/
};
...
//    int tip_status;            /**/
    ...
    int battery;             /*0: low power, 1: normal*/
};
...
enum elan_fwid_fwver_addr{
    FW_ID_ADDR = 0x1FE66,    //for elan_IC 6Axx
    FW_VER_ADDR = 0x9E,
};
...
#define print_log(level, level1, fmt, args...) \
do { \
        if (level >= level1) \
            printk("[elan]:"fmt"\n",##args);\
} while(0)
