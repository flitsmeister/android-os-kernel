...
static int elan_ts_recv_data(struct elan_ts_data *ts, uint8_t *buf)
{
    ...
    if (ts->chip_type == HID_TYPE_PROTOCOL) {
        if (rbuf[2] == HID_FID) {
            if (report->finger.recv_2nd_packet == true) {
                //rc = ts->ops->recv(rbuf, sizeof(rbuf));
                //if (rc != sizeof(rbuf)) {
                //    dev_err(&ts->client->dev, "[elan] recv second report data error!!\n");
                //    report->finger.recv_2nd_packet=false;
                //    return -1;
                //}
                ...
                if(rbuf[62] > 5) {
                    report->finger.recv_2nd_packet = true;
                    return rc;
                }
            }
...
//            report->stylus.inver                = rbuf[3] >> 1;
    ...
    if ( report->tool_type == ELAN_PEN) {
        for(i = 0; i < report->stylus.pbuf_valid_size/8 + 1; i++) {
            print_log(ts->level, TP_ERR,"%02x %02x %02x %02x %02x %02x %02x %02x\n",\
...
static void  elants_a_report(struct elan_ts_data *ts, uint8_t *buf)
{
    ...
    if (report->tool_type == ELAN_FINGER) {
        ...
        print_log(ts->level, TP_ERR,"[elan] stylus.key %d, pkey %d\n",stylus.key,pkey);
        ...
        print_log(ts->level, TP_ERR,"[elan] stylus.tip_status  %d, stylus.inrange_status %d\n",\
    ...
    if (report->tool_type == ELAN_FINGER) {
        ...
        //dev_err(&ts->client->dev, "[elan] elants_slot_report4\n");
            for (reportid = 0; reportid < finger.fvalid_num; reportid++ ) {
                ...
                elan_ts_fparse_xy(&buf[finger.freport_idx], &y, &x, finger.fid); //lcm x :y = 720 : 1280 tp x: y = 1296:720
                x = ts->hw_info.screen_x * x / ts->fw_info.finger_yres;
                y = ts->hw_info.screen_y * y / ts->fw_info.finger_xres;
        ...
        //print_log(ts->level, TP_ERR,"[elan] stylus.key %d, pkey %d\n",stylus.key,pkey);
        dev_info(&ts->client->dev, "[elan] stylus.key %d, pkey %d\n",stylus.key,pkey);
        if (stylus.key > 0 || pkey) {
            switch(stylus.key) {
                case 1:
                    pkey = BTN_STYLUS2;
        ...
        print_log(ts->level, TP_ERR ,"[elan] stylus.inrange_status = %d, stylus.barrel_tip = %d\n", \
                stylus.inrange_status,stylus.barrel_tip);

        if(stylus.inrange_status) {
             elan_ts_pparse_xy(&buf[0],&y,&x,&p);
             x = ts->hw_info.screen_x * x / ts->fw_info.pen_yres;
             y = ts->hw_info.screen_y * y / ts->fw_info.pen_xres;
            ...
            // x_tilt = (int8_t)(x_tilt_raw / 100);
             ...
             dev_info(&ts->client->dev, "[elan] pen X:Y:P:TX:TY ====%d:%d:%d:%d\n",x,y,p,stylus.battery);
        } else {
            input_mt_slot(ts->pen_idev, 0);
...
static void  elan_ts_work_func(struct work_struct *work)
{
    ...
    static uint8_t buf[HID_REPORT_MAX_LEN] = {0x00};
    int rc = 0;

    if(gpio_get_value(ts->hw_info.intr_gpio)) {
        print_log(ts->level, TP_ERR,"[elan]interrupt jitter\n.");
        goto out;
    }
    ...
    if(ts->report.finger.recv_2nd_packet) {
        print_log(ts->level, TP_INFO,"[elan]recv 2nd packet\n.");
        goto out;
    }

    elan_ts_report_data(ts,buf);
    memset(&ts->report, 0, sizeof(ts->report));
    ts->report.finger.recv_2nd_packet=false;
...
static irqreturn_t elan_ts_irq_handler(int irq, void *dev_id)
{
    ...
    if (ts->user_handle_irq) {
        ...
        print_log(ts->level, TP_INFO,"[elan]enter %s\n.",__func__);

        mutex_lock(&ts->irq_mutex);
        if(ts->irq_lock_flag == 0) {
            disable_irq_nosync(ts->hw_info.irq_num);
            ts->irq_lock_flag = 1;
        }
        mutex_unlock(&ts->irq_mutex);
...
static int elan_ts_register_interrupt(struct elan_ts_data *ts)
{
    ...
    err = request_threaded_irq(ts->hw_info.irq_num,
            NULL, elan_ts_irq_handler, IRQF_TRIGGER_LOW | IRQF_ONESHOT,
            ELAN_TS_NAME, ts);
...
*/
    ...
    ts->hw_info.screen_x = 1728;
    ts->hw_info.screen_y = 2368;
#if 0
    /*get lcm coordinate*/
...
static int elan_ts_set_power_state(struct i2c_client *client, int state)
{
    ...
    print_log(ts->level, TP_INFO, "[elan] set power stats success\n");
    return 0;
