...
#define	FILEOPS_OR_PROCOPS	    1    //1-file_operations, 0-proc_ops
...
#define DMA_BUFFER_LENGTH        (MAX_TRX_LEN+4)
...
static int msg_dma_alloct(void)
{
    ...
    if(NULL == tpd_i2c_dma_va){
        ...
        tpd_i2c_dma_va = (u8 *)dma_alloc_coherent(&tpd->dev->dev, DMA_BUFFER_LENGTH, &tpd_i2c_dma_pa, GFP_KERNEL);
		printk("Tlsc6x:%s ++!\n", __func__);
    }
    if(NULL == tpd_i2c_dma_va){
        ret = 0;
...
{
    if(tpd_i2c_dma_va){
        dma_free_coherent(NULL, DMA_BUFFER_LENGTH, tpd_i2c_dma_va, tpd_i2c_dma_pa);
        tpd_i2c_dma_va = NULL;
        tpd_i2c_dma_pa = 0;
        printk("Tlsc6x:Allocated DMA I2C Buffer release!!\n");
    }
}
#endif
...
void tlsc6x_tpd_reset_active(void)
{
	TLSC_FUNC_ENTER();
	tpd_gpio_output(tpd_rst_gpio_number, 0);
	msleep(20);
...
int tlsc6x_i2c_read_sub(struct i2c_client *client, char *writebuf, int writelen, char *readbuf, int readlen)
{
    ...
    memcpy(tpd_i2c_dma_va, (unsigned char*)writebuf, writelen);
    ...
    for(retry = 0; retry < HAL_MAX_TRY; retry++)
    {
        ...
        if(readlen == ret) break;
    }
    memcpy(readbuf, tpd_i2c_dma_va, readlen);
    client->addr = client->addr & I2C_MASK_FLAG & (~ I2C_DMA_FLAG);
...
int tlsc6x_proc_cfg_update(u8 *dir, int behave)
{
	...
	if (IS_ERR(file)) {
		...
		if (fileSize == vfs_read(file, (char *)pbt_buf, fileSize, &file->f_pos)) {//kernel_read
...
#else
static struct proc_ops tlsc6x_proc_ops = {
...
printk("hechuan ---- > tlsc6x_update_data: x = %lu, y = %lu\n",x, y);		
printk("hechuan ---- > tlsc6x_update_data: ABS_MT_POSITION_X = %lu, ABS_MT_POSITION_Y = %lu\n",ABS_MT_POSITION_X, ABS_MT_POSITION_Y);
...
TPD_DEBUG("TPD removed\n");
   ...
   // wake_lock_destroy(&tlsc6x_wakelock);
    gpio_free(tpd_rst_gpio_number);
    gpio_free(tpd_int_gpio_number);
...
static void tpd_resume(struct device *h)
{
    ...
    if(1 == gesture_enable){
       ...
       // wake_unlock(&gesture_timeout_wakelock);
    }
#endif
...
static void tpd_suspend(struct device *h)
{
    ...
    if(1 == gesture_enable){
        ...
        tlsc6x_irq_enable();
        //wake_lock(&gesture_timeout_wakelock);
        real_suspend_flag = 1;
        return;
    }
#endif
