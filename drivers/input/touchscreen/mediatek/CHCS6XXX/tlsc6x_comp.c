...
unsigned char g_cfg_update_flag = 0;
unsigned char g_boot_update_flag = 0;
unsigned short g_tlsc6x_chip_code = 0;	//g_tlsc6x_chip_code与我司IC型号对应关系:
...
int tlsx6x_update_burn_cfg(u16* ptcfg)
{
	...
	memcpy(tl_target_cfg, ptcfg, 204);
	g_tlsc6x_cfg_ver = (ptcfg[1] << 16) | ptcfg[0];
	g_tlsc6x_chip_code = (unsigned short)((ptcfg[53] >>8)&0x0f);
...
#else
	fupd = fw_file_tlsc6x;	  
	fw_size = ARRAY_SIZE(fw_file_tlsc6x);
	ret = tlsc6x_update_compat_ctl((u8 *) fupd, fw_size);
#endif
...
int tlsc6x_tp_dect(struct i2c_client *client)
{
	...
	if (tlsc6x_get_running_cfg(tl_buf_tmpcfg) == 0) {
		g_tlsc6x_cfg_ver = (unsigned int)tl_buf_tmpcfg[1];
		g_tlsc6x_cfg_ver = (g_tlsc6x_cfg_ver<<16) + (unsigned int)tl_buf_tmpcfg[0];
		g_tlsc6x_chip_code = (unsigned short)((tl_buf_tmpcfg[53] >>8)&0x0f);
		tlsc_cfg_version = g_tlsc6x_cfg_ver>>26;
		tlsc_vendor_id = (((g_tlsc6x_cfg_ver>>9)&0x7F) | ((g_tlsc6x_cfg_ver>>22&0x03)<<7));
		tlsc_project_id = ((g_tlsc6x_cfg_ver&0x01FF) | ((g_tlsc6x_cfg_ver>>20&0x03)<<9));
		tlsc_info("vendor_id= %d,project_id=%d,boot_version=0x%x,cfg_version=%d \n",\
		tlsc_vendor_id,tlsc_project_id,tlsc_boot_version,tlsc_cfg_version);
	} else {
