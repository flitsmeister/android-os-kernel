...
#if 0 //CONFIG_TINNO_DEVINFO
static unsigned char g_jd9366t_buf[128] = {0};
#endif
...
void jadard_report_points(struct jadard_ts_data *ts)
{
	...
	for (i = 0; i < pjadard_ic_data->JD_MAX_PT; i++) {
		...
		if (pjadard_ic_data->JD_X_RES <= pjadard_ic_data->JD_Y_RES) {
			...
			//input_report_abs(ts->input_dev, ABS_MT_POSITION_X, ts->pdata->abs_x_max - pjadard_host_data->x[i] - 1);
			//input_report_abs(ts->input_dev, ABS_MT_POSITION_Y, ts->pdata->abs_y_max - pjadard_host_data->y[i] - 1);
...
//===================debug message 
	if ((pjadard_host_data->x[0] == 0x0000 && pjadard_host_data->y[0] == 0x0000)\
	 && (pjadard_host_data->x[1] == 0x0000 && pjadard_host_data->y[1] == 0x0000)\
	 && (pjadard_host_data->x[2] == 0x0000 && pjadard_host_data->y[2] == 0x0000)\
	 && (pjadard_host_data->x[3] == 0x0000 && pjadard_host_data->y[3] == 0x0000)\
	 && (pjadard_host_data->x[4] == 0x0000 && pjadard_host_data->y[4] == 0x0000)\
	 && (pjadard_host_data->x[5] == 0x0000 && pjadard_host_data->y[5] == 0x0000)\
	 && (pjadard_host_data->x[6] == 0x0000 && pjadard_host_data->y[6] == 0x0000)\
	 && (pjadard_host_data->x[7] == 0x0000 && pjadard_host_data->y[7] == 0x0000)\
	 && (pjadard_host_data->x[8] == 0x0000 && pjadard_host_data->y[8] == 0x0000)\
	 && (pjadard_host_data->x[9] == 0x0000 && pjadard_host_data->y[9] == 0x0000)
	)
	{
...
{
	JD_I("Touch state1 = 0x%02x\n", pjadard_report_data->touch_state_info[0]);
	JD_I("Touch state2 = 0x%02x\n", pjadard_report_data->touch_state_info[1]);
	JD_I("Touch state3 = 0x%02x\n", pjadard_report_data->touch_state_info[2]);
	JD_I("Touch state4 = 0x%02x\n", pjadard_report_data->touch_state_info[3]);
}
...
static int jadard_err_ctrl(uint8_t *buf, int buf_len, int ts_status)
{
	...
	if (((checksum & 0x00FF) != 0) || (checksum == 0)) {
...
static void jadard_usb_status(int status, bool renew)
{
	struct jadard_ts_data *ts = pjadard_ts_data;
...
static void jadard_charger_init(struct work_struct *work)
{
	...
	*/
	psy_usb = power_supply_get_by_name("usb");
	if (!psy_usb) {
		JD_E("Get usb psy fail\n");
	}

	psy_ac = power_supply_get_by_name("ac");
	if (!psy_ac) {
		JD_E("Get ac psy fail\n");
	}
                ...
                if ((rdata[0] == 0xD8) || (rdata[0] == 0xDA) || (rdata[0] == 0xDC) || (rdata[0] == 0xDE) ||
                    ...
                    if (pjadard_ts_data->fw_ready == true) {
                        ...
                        if (same_data >= 5) {
                            ...
                            if (bypass == 0) {
                                JD_I("Reset the Touch chip by the same pc\n");
                                jadard_int_enable(false);
                                jd9366t_ResetSOC();
                            /* #ifdef JD_RST_PIN_FUNC
                            ...
                            JD_I("Reset the Touch chip by fault address\n");
                            jadard_int_enable(false);
                            jd9366t_ResetSOC();
                        /* #ifdef JD_RST_PIN_FUNC
                            g_module_fp.fp_ic_reset(false, false);
...
int jadard_chip_common_init(void)
{
	...
	int err;
	printk("jadard_chip_common_init 1\n");	
	JD_I("%s: enter\n", __func__);
...
#endif
	printk("jadard_chip_common_init 2\n");	
	pdata = kzalloc(sizeof(*pdata), GFP_KERNEL);
	if (pdata == NULL) {
		err = -ENOMEM;
		goto err_platform_data_fail;
	}
	printk("jadard_chip_common_init 3\n");	
	...
	printk("jadard_chip_common_init 4\n");	
#ifdef JD_RST_PIN_FUNC
	...
	printk("jadard_chip_common_init 5\n");	
	if (ptr == NULL) {
...
#if 0//CONFIG_TINNO_DEVINFO
	get_device_info(ID_LCD,g_jd9366t_buf);
	if (strcmp(g_jd9366t_buf, "jd9366tc_hd_dsi_vdo_dj") == 0) {
		memcpy(jd_i_CTPM_firmware_name,"Jadard_firmware.bin",22);
	} else {
		memcpy(jd_i_CTPM_firmware_name,"Jadard_firmware_jt.bin",22);
	}
#endif
...
#if 0//def CONFIG_TINNO_DEVINFO
	get_device_info(ID_LCD,g_jd9366t_buf);
	if (strcmp(g_jd9366t_buf, "jd9366tc_hd_dsi_vdo_dj") == 0) {
		memset(g_jd9366t_buf,0,sizeof(g_jd9366t_buf));
		sprintf(g_jd9366t_buf, "DJ_T402AA_jd9366tc_V0x%02x", pjadard_ic_data->fw_cid_ver);
	} else {
		memset(g_jd9366t_buf,0,sizeof(g_jd9366t_buf));
		sprintf(g_jd9366t_buf, "JT_T402AA_jd9366tc_V0x%02x", pjadard_ic_data->fw_cid_ver);
	}
	FULL_PRODUCT_DEVICE_INFO(ID_TP, g_jd9366t_buf);
#endif
	...
	printk("jadard_chip_common_init 6\n");	
...
#endif
	printk("jadard_chip_common_init 7\n");	
	return JD_NO_ERR;
...
int jadard_chip_common_suspend(struct jadard_ts_data *ts)
{
	...
	defined(JD_ROTATE_BORDER) || defined(JD_EARPHONE_DETECT)
