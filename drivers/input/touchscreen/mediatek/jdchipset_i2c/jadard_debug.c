...
/*
 * DataType	 => 1:RawData 2:Baseline 3:Difference 4:Listen 5:Label
 * KeepType	 => 1:NormalValue 2:MaxValue 3:MinValue 4:PeakValue
	...
	if ((DataType > 0) && (KeepType > 0)) {
		...
		if (KeepFrame > 0) {
			...
			case JD_DATA_TYPE_RawData:
				...
				vts_name = jd_getname_kernel(JD_RAWDATA_DUMP_FILE);
				jd_diag_mutual_fn = jd_file_open_name(vts_name, O_CREAT | O_TRUNC | O_WRONLY, 0);
			#else
			...
			#if defined(__JADARD_KMODULE__)
				vts_name = jd_getname_kernel(JD_BASE_DUMP_FILE);
				jd_diag_mutual_fn = jd_file_open_name(vts_name, O_CREAT | O_TRUNC | O_WRONLY, 0);
			#else
			...
			#if defined(__JADARD_KMODULE__)
				vts_name = jd_getname_kernel(JD_DIFF_DUMP_FILE);
				jd_diag_mutual_fn = jd_file_open_name(vts_name, O_CREAT | O_TRUNC | O_WRONLY, 0);
			#else
			...
			#if defined(__JADARD_KMODULE__)
				vts_name = jd_getname_kernel(JD_LISTEN_DUMP_FILE);
				jd_diag_mutual_fn = jd_file_open_name(vts_name, O_CREAT | O_TRUNC | O_WRONLY, 0);
			#else
			...
			#if defined(__JADARD_KMODULE__)
				vts_name = jd_getname_kernel(JD_LABEL_DUMP_FILE);
				jd_diag_mutual_fn = jd_file_open_name(vts_name, O_CREAT | O_TRUNC | O_WRONLY, 0);
			#else
		...
		if (g_module_fp.fp_EnterBackDoor != NULL) {
...
#if defined(__JADARD_KMODULE__)
	vts_name = jd_getname_kernel(JD_FW_DUMP_FILE);
	fn = jd_file_open_name(vts_name, O_CREAT | O_TRUNC | O_WRONLY, 0);
#else
	...
	if (!pjadard_debug->proc_send_flag) {
		...
		if (buf_tmp != NULL) {
			if (g_common_variable.dbi_dd_reg_mode == JD_DDREG_MODE_0) {
				...
				if (g_module_fp.fp_dd_reg_read(dd_reg_page, dd_reg_cmd, rdata, dd_reg_read_len) < 0) {
					ret += snprintf(buf_tmp + ret, len - ret, "Read display register fail!\n");
				} else {
					/* Set output data */
					ret += snprintf(buf_tmp + ret, len - ret, "Display register page: 0x%02X\n", dd_reg_page);
					ret += snprintf(buf_tmp + ret, len - ret, "Display register: 0x%02X\n", dd_reg_cmd);
	...
	if ((buf_tmp[0] == 'r' || buf_tmp[0] == 'w') && buf_tmp[1] == ':' && buf_tmp[2] == 'x') {
		...
		if (buf_tmp[0] == 'w') {
			if (g_common_variable.dbi_dd_reg_mode == JD_DDREG_MODE_0) {
				...
				if (g_module_fp.fp_dd_reg_write(dd_reg_page, dd_reg_cmd, w_data, w_data_len) < 0) {
					JD_E("%s: Write display register(%x) fail!\n", __func__, dd_reg_cmd);
				}
			}
...
static ssize_t jadard_proc_display_std_read(struct file *file, char *buf,
											size_t len, loff_t *pos)
{
	int ret = 0;
	uint8_t *rdata = NULL;
	...
	if (!pjadard_debug->proc_send_flag) {
		...
		if (buf_tmp != NULL) {
			if (g_common_variable.dbi_dd_reg_mode == JD_DDREG_MODE_1) {
				if (g_module_fp.fp_dd_reg_std_cmd_read(dd_reg_cmd, rdata, dd_reg_read_len) < 0) {
					ret += snprintf(buf_tmp + ret, len - ret, "Read display standard register fail!\n");
				} else {
					/* Set output data */
					ret += snprintf(buf_tmp + ret, len - ret, "Display standard register 0x%02X\n", dd_reg_cmd);
											...
											size_t len, loff_t *pos)
{
	...
	if ((buf_tmp[0] == 'r' || buf_tmp[0] == 'w') && buf_tmp[1] == ':' && buf_tmp[2] == 'x') {
		...
		if (buf_tmp[0] == 'w') {
			if (g_common_variable.dbi_dd_reg_mode == JD_DDREG_MODE_1) {
				if (g_module_fp.fp_dd_reg_std_cmd_write(dd_reg_cmd, w_data, w_data_len, 0x00) < 0) {
					JD_E("%s: Write display standard register(%x) fail!\n", __func__, dd_reg_cmd);
				}
			}
...
static struct file_operations jadard_proc_display_std_ops  = {
	.owner = THIS_MODULE,
	.read = jadard_proc_display_std_read ,
								 ...
								 size_t len, loff_t *pos)
{
	...
	if (buf_tmp[0] == 'v') {
		...
		if (g_module_fp.fp_EnterBackDoor != NULL) {
			...
			JD_I("Function pointer of enter backdoor was NULL\n");
		}
	} else if ((buf_tmp[0] == 'o') && (buf_tmp[1] == 'b')) {
		if (g_module_fp.fp_ExitBackDoor != NULL) {
			g_module_fp.fp_ExitBackDoor();
			JD_I("Exit backdoor\n");
		} else {
			JD_I("Function pointer of exit backdoor was NULL\n");
		}
	} else {
		debug_cmd = 0;
	}
...
int jadard_touch_proc_init(void)
{
	...
	if ( g_common_variable.dbi_dd_reg_mode == JD_DDREG_MODE_1 ) {
		jadard_proc_display_std_file = proc_create(JADARD_PROC_DISPLAY_STD_FILE, (S_IWUSR | S_IRUGO),
