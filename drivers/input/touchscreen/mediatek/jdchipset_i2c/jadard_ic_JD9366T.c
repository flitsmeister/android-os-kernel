...
void jd9366t_set_reset_pin_value_for_lcd(uint8_t value)
{
	if(tpd_type_cap == 0)
		return ;
	if(value) {
...
static int jd9366t_ReadSectionInfo(bool reinit_config)
{
	...
	if (jd9366t_GetSectionFirstValueStatus(50) == true) {
		...
		jd9366t_GetSectionInfo(g_jd9366t_section_info.dsram_section_info_start_addr,
								JD9366T_REPAIR_MODE_SWITCH + 1, &dsram_info_content[0]);
		...
		for (i = 0; i <= JD9366T_REPAIR_MODE_SWITCH; i++) {
...
static struct class  *TP_I2C_Test_Class = NULL;
static dev_t TP_I2C_Test_Devt;
static struct device *TP_I2C_Test_Device;
static char opr_flag = -1;
static uint8_t reg;
static uint8_t read_data[10];
static uint8_t write_data[10];

static ssize_t jdtp_rw_reg_show(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	int32_t ret;

	if(opr_flag == 0) {
		return sprintf(buf, "Write reg=%x: write_data=%s success\n", reg, write_data);
	} else if(opr_flag == 1) {
		read_data[0] = reg;
		//ilits->wrapper(NULL, 0, read_data, 1, OFF, OFF);
		module_jd9366t_register_read(reg,read_data,1);
		if (ret < 0) {
			pr_info("%s: spi read fail!\n", __func__);
		}
		return sprintf(buf, "Read reg=%x, read_data=%x\n", reg, read_data);
	}
	return sprintf(buf, "jdtp_rw_reg_show operation error\n");
}

static ssize_t jdtp_rw_reg_store(struct device *dev,
			struct device_attribute *attr,
			const char *buf, size_t count)
{
	uint64_t data = 0;

	pr_info("jdtp_rw_reg_store strlen(buf) = %d  buf = %s \n",strlen(buf),buf);
	if(strlen(buf) == 9) {
		sscanf(buf, "%x", &data);
		reg = data & 0xff;
		opr_flag = 1;
		pr_info("jdtp_rw_reg_store: read reg:%x\n" , reg);
	} else if(strlen(buf) == 17) {
		sscanf(buf, "%x", &data);
		reg = (data >> 32) & 0xff;
		write_data[0] = reg;
		write_data[1] = data&0xff;
		//ilits->wrapper(write_data, 2, NULL, 0, OFF, OFF);
		module_jd9366t_flash_write(reg,write_data,2);
		opr_flag = 0;
		pr_info("jdtp_rw_reg_store: write reg:%x, data:%x\n", reg, write_data);
	} else {
		opr_flag = -1;
		pr_info("jdtp_rw_reg_store buf error.\n");
	}
	return count;
}

static DEVICE_ATTR(jdtp_rw_reg, 0664, jdtp_rw_reg_show, jdtp_rw_reg_store);
static struct attribute * tp_iic_test_attributes[] = {
	&dev_attr_jdtp_rw_reg.attr,
	NULL,
};

static struct attribute_group tp_iic_test_attribute_group = {
	.attrs = tp_iic_test_attributes
};

static const struct attribute_group * tp_iic_test_groups[] = {
	&tp_iic_test_attribute_group,
	NULL,
};
...
static void module_jd9366t_chip_init(void)
{
	...
	if (jd9366t_ReadSectionInfo(false) < 0) {
		JD_E("Chip init fail 1 \n");
	}else{
		g_module_fp.fp_read_fw_ver();

		/* for i2c auto test, and current node is fix */
		TP_I2C_Test_Class = class_create(THIS_MODULE, "tp_iic_test");
		if (IS_ERR(TP_I2C_Test_Class)) {
			ret = PTR_ERR(TP_I2C_Test_Class);
			pr_err("create TP_I2C_Test_Class error\n");
		}
		TP_I2C_Test_Class->dev_groups = tp_iic_test_groups;
		ret = alloc_chrdev_region(&TP_I2C_Test_Devt, 0, 1, "TP_I2C_Test_devt");
		if(ret < 0) {
			pr_err("alloc_chrdev_region failed!");
		}
		TP_I2C_Test_Device = device_create(TP_I2C_Test_Class, NULL, TP_I2C_Test_Devt, pjadard_ts_data->client,"tp_rw_reg");
		if (IS_ERR(TP_I2C_Test_Device)) {
			ret = PTR_ERR(TP_I2C_Test_Device);
			pr_err("TP_I2C_Test_Device create error: %d" , ret);
		}
	}
#endif
}
...
static uint8_t module_jd9366t_DbicReg_write(uint8_t cmd, uint8_t *par, uint8_t par_len)
{
	uint8_t i;
...
static uint8_t module_jd9366t_DbicReg_read(uint8_t cmd, uint8_t *rpar, uint8_t rpar_len)
{
	uint8_t i;
...
static void module_jd9366t_read_fw_ver(void)
{
	...
	if ((ReCode_cid < 0) || (ReCode_ver < 0) || (ReCode_maker < 0) || (ReCode_panel < 0)) {
		...
		snprintf(pjadard_ic_data->panel_maker, sizeof(pjadard_ic_data->panel_maker), "%s", jd_panel_maker_list[JD_PANEL_MAKER_NUM]);
		...
		snprintf(pjadard_ic_data->panel_maker, sizeof(pjadard_ic_data->panel_maker), "%s", maker_name);
	}
...
void module_jd9366t_charg_mode_set(unsigned int enable)
{
	uint8_t wdata[JD_TWO_SIZE] = {0x00, 0x00};

	wdata[0] = (uint8_t)enable;
...
static bool module_jd9366t_APP_ReadSortingBusyStatus(uint8_t mpap_handshake_finish)
{
...
static void jd9366t_func_reinit(void)
{
	...
	g_module_fp.fp_DbicReg_write		= module_jd9366t_DbicReg_write;
	g_module_fp.fp_DbicReg_read			= module_jd9366t_DbicReg_read;
...
static bool module_jd9366t_chip_detect(void)
{
	...
	if ( DDREG_MODE != JD_DDREG_MODE_0 ) {
		printk("%s:klm Don't support this mode ,dbi_dd_reg_mode %02X not equal %02X\n", __func__, DDREG_MODE, JD_DDREG_MODE_0);
		return false;
	}
	...
	do {
		...
		if (romid == CHIP_ID) {
			printk("%s:klm detect IC %s successfully\n", __func__, JD9366T_OUTER_ID);
			snprintf(pjadard_ic_data->chip_id, sizeof(pjadard_ic_data->chip_id), JD9366T_OUTER_ID);
...
static int jadard_jd9366t_probe(void)
{
	...
	printk("%s:klm jd9366_probe Enter 1\n", __func__);

	pChip = kzalloc(sizeof(struct jadard_support_chip), GFP_KERNEL);
	if (pChip == NULL) {
		printk("%s: allocate jadard_ts_data failed\n", __func__);
		return -ENOMEM;
	}
