...
struct ilitek_ic_func_ctrl func_ctrl[FUNC_CTRL_NUM] = {
	...
	[8] = {"lock_point", {0x1, 0x13, 0x0}, 3, 0x1, 0, 0xFF},
	[9] = {"game_mode", {0x1, 0x13, 0x0}, 3, 0x1, 0, 0xFF},	
	[10] = {"active", {0x1, 0x14, 0x0}, 3, 0x0, 2, 0xFF},
	[11] = {"idle", {0x1, 0x19, 0x0}, 3, 0x1, 0, 0xFF},
	[12] = {"gesture_demo_en", {0x1, 0x16, 0x0}, 3, 0x0, 0, 0xFF},
	[13] = {"tp_recore", {0x1, 0x18, 0x0}, 3, 0x0, 2, 0xFF},
	[14] = {"knock_en", {0x1, 0xA, 0x8, 0x03, 0x0, 0x0}, 6, 0xFF, 0, 0xFF},
	[15] = {"int_trigger", {0x1, 0x1B, 0x0}, 3, 0x0, 2, 0xFF},
	[16] = {"ear_phone", {0x1, 0x17, 0x0}, 3, 0x0, 0, 0xFF},
	[17] = {"knuckle", {0x1, 0xF, 0x0}, 3, 0x0, 0, 0xFF},
};
...
int ili_ic_get_core_ver(void)
{
	...
	do {
		...
		if (buf[0] == P5_X_GET_CORE_VERSION ||
			...
			ILI_INFO("Get core ver successfully, retry %d times\n", retry);
			break;
		}
...
int ili_ic_get_fw_ver(void)
{
	...
	do {
		...
		if (buf[0] != P5_X_GET_FW_VERSION) {
			...
			ILI_INFO("Get firmware ver successfully, retry %d times\n", retry);
			ret = 0;
			break;
		}
	} while(++retry < 3);
...
int ili_ic_get_panel_info(void)
{
	...
	do {
		...
		if (buf[0] != cmd) {
			...
			ILI_INFO("Get panel info successfully, retry %d times\n", retry);
	...
	if (ilits->chip->core_ver >= CORE_VER_1470) {
		do{
			...
			if (buf[0] != cmd) {
				...
				ILI_INFO("Get report format info successfully, retry %d times\n", retry);
	...
	do {
		...
		if (ret < 0)
			ILI_ERR("Read pen info error\n");

		if (buf[0] != cmd) {
			ILI_INFO("Invalid pen info, use default pen info resolution\n");
			...
			ILI_INFO("Get pen info successfully, retry %d times\n", retry);
	...
	do {
		...
		if (ret < 0)
			ILI_ERR("Read cascade info error\n");

		if (buf[0] != cmd) {
			ILI_INFO("Invalid cascade info, use default value\n");
			ilits->cascade_info_block.nDisable = ENABLE;
			ilits->cascade_info_block.nNum = 2;
		} else {
			ILI_INFO("Get cascade info successfully, retry %d times\n", retry);
...
int ili_ic_get_tp_info(void)
{
	...
	do {
		...
		if (buf[0] != P5_X_GET_TP_INFORMATION) {
			...
			ILI_INFO("Get tp info successfully, retry %d times\n", retry);
			ret = 0;
			break;
		}
	} while (++retry < 3);
...
void ili_ic_get_compress_info(void)
{
	...
	do {
		...
		if (buf[0] != cmd) {
			...
			buf[1] = 0xFF; /*set compress disable*/
		} else {
			ILI_INFO("Get compress info successfully, retry %d times\n", retry);
			break;
		}
	} while(++retry < 3);
...
int ili_ic_get_protocl_ver(void)
{
	...
	do {
		...
		if (buf[0] != P5_X_GET_PROTOCOL_VERSION) {
			...
			ILI_INFO("Get protocol ver successfully, retry %d times\n", retry);
			ret = 0;
			break;
		}
	} while(++retry < 3);
