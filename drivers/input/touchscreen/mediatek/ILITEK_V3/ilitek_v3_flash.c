...
void ili_flash_dma_write_cascade(u32 start,  u32 end, u32 len)
{
	...
	if (ili_ice_mode_write_by_mode(FLASH2_reg_tx_data, 0x3b, 1, BOTH) < 0)
		ILI_ERR("Cascade write 0x0b at %x failed\n", FLASH2_reg_tx_data);
	...
	if (ili_ice_mode_write_by_mode(FLASH2_reg_tx_data, (start & 0xFF0000) >> 16, 1, BOTH) < 0)
	...
	if (ili_ice_mode_write_by_mode(FLASH2_reg_tx_data, (start & 0x00FF00) >> 8, 1, BOTH) < 0)
		ILI_ERR("Cascade write %x at %x failed\n", (start & 0x00FF00) >> 8, FLASH2_reg_tx_data);
	...
	if (ili_ice_mode_write_by_mode(FLASH2_reg_tx_data, (start & 0x0000FF), 1, BOTH) < 0)
		ILI_ERR("Cascade write %x at %x failed\n", (start & 0x0000FF), FLASH2_reg_tx_data);
	...
	if (ili_ice_mode_bit_mask_write_cascade(FLASH0_dual_mode, FLASH0_reg_rx_qual, BIT(7), SLAVE))
		ILI_ERR("Slave mask write: mask=%lu, value=%lu, addr=0x%x failed\n", FLASH0_reg_rx_qual, BIT(7), FLASH0_dual_mode);

	if (ili_ice_mode_write_by_mode(FLASH2_reg_tx_data, 0x00, 1, BOTH) < 0)
		ILI_ERR("Cascade write dummy failed\n");
...
static void ilitek_tddi_fw_update_block_info(u8 *pfw)
{
	...
	ILI_INFO("pen_info_block : nPxRaw = %d, nPyRaw = %d, nPxVa = %d,nPyVa = %d, nPenX_MP = %d, nReserved01 = %d, nReserved2 = %d, nReserved03 = %d\n",
		ilits->pen_info_block.nPxRaw, ilits->pen_info_block.nPyRaw, ilits->pen_info_block.nPxVa, ilits->pen_info_block.nPyVa, ilits->pen_info_block.nPenX_MP, ilits->pen_info_block.nReserved01, ilits->pen_info_block.nReserved02, ilits->pen_info_block.nReserved03);
#endif
...
{
	int ret = 0, fsize = 0 , test_count = 0;
	...
	case REQUEST_FIRMWARE:
		if (request_firmware(&fw, ilits->md_fw_rq_path, ilits->dev) < 0) {
			...
			for (test_count = 50; test_count > 0;test_count --)
			{
				mdelay(50);
				if (request_firmware(&fw, ilits->md_fw_rq_path, ilits->dev) >= 0) {
					ILI_ERR("Request firmware succuse, braeak\n");
					break;
				} else {
					ILI_ERR("Request firmware failed after retry = %d\n",test_count);
				}
			}
			}
			if (request_firmware(&fw, ilits->md_fw_rq_path, ilits->dev) < 0) {
