...
int tp_int ,tp_rst ;
	...
	tp_int = of_get_named_gpio(dt, "himax,irq-gpio", 0);
	if (!gpio_is_valid(tp_int))
		printk(" DT:gpio_irq value is not valid\n");

	tp_rst = of_get_named_gpio(dt, "himax,rst-gpio", 0);
	if (!gpio_is_valid(tp_rst))
		printk(" DT:gpio_rst value is not valid\n");

	himax_tpd_rst_gpio_number = tp_rst;
	himax_tpd_int_gpio_number = tp_int;
...
int himax_bus_write(uint8_t cmd, uint32_t addr, uint8_t *data, uint32_t len)
{
	...
	if (addr != NUM_NULL) {
		...
		memcpy(gBuffer+offset, tmp_addr, 4);
		offset += 4;
		tmp_len -= 4;
	}
...
{
	if (value)
		gpio_direction_output(himax_tpd_rst_gpio_number, 1);
	else
		gpio_direction_output(himax_tpd_rst_gpio_number, 0);
}
#endif
...
RET_REMOVE himax_common_remove_spi(struct spi_device *spi)
{
	int ret = 0;
	struct himax_ts_data *ts = hx_s_ts;
