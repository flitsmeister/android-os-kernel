...
#include "himax_ic_HX83102j.h"
#include "himax_modular.h"
#include "himax_inspection.h"

#if defined(HX_ZERO_FLASH)
static void hx83102j_en_hw_crc(int en);
#endif
...
struct ic_setup_collect g_hx83102j_setup = {
	...
	._addr_rawout_sel = HX83102J_ADDR_RAWOUT_SEL,
...
void hx83102j_burst_enable(uint8_t auto_add_4_byte)
{
...
int hx83102j_register_write(uint32_t addr, uint8_t *val, uint32_t len)
{
	...
	if (addr == ADDR_FLASH_SPI200_DATA)
		hx83102j_burst_enable(0);
	else if (len > DATA_LEN_4)
		hx83102j_burst_enable(1);
	else
		hx83102j_burst_enable(0);
...
static int hx83102j_register_read(uint32_t addr, uint8_t *buf, uint32_t len)
{
	int ret = -1;
	uint8_t tmp_data[DATA_LEN_4];
...
void hx83102j_system_reset(void)
{
	...
	hx83102j_register_write(ADDR_CTRL_FW,
		tmp_data, DATA_LEN_4);
	do {
		/* reset code*/
		/**
		...
		hx83102j_register_read(ADDR_FLAG_RESET_EVENT,
			tmp_data, DATA_LEN_4);
...
static bool hx83102j_sense_off(bool check_en)
{
	...
	do {
		...
		/* check fw status */
		hx83102j_register_read(ADDR_CS_CENTRAL_STATE,
			tmp_data, DATA_LEN_4);
		...
		hx83102j_register_read(ADDR_CTRL_FW, tmp_data, 4);
	...
	do {
		...
		if (tmp_data[0] == 0x0C) {
			...
			hx_parse_assign_cmd(DATA_CLEAR, tmp_data, DATA_LEN_4);
			hx83102j_register_write(HX83102J_ADDR_TCON_ON_RST,
				tmp_data, DATA_LEN_4);
			usleep_range(10000, 10001);
			ret = true;
			goto END;
...
static void hx83102j_en_hw_crc(int en)
{
	...
	if (en) {
		hx_parse_assign_cmd(HX83102J_DATA_EN_HW_CRC,
			wrt_data, DATA_LEN_4);
	} else {
		hx_parse_assign_cmd(DATA_CLEAR,
			wrt_data, DATA_LEN_4);
	}

	do {
		hx83102j_register_write(HX83102J_ADDR_EN_HW_CRC,
			wrt_data, DATA_LEN_4);
		usleep_range(1000, 1100);
		hx83102j_register_read(HX83102J_ADDR_EN_HW_CRC,
			tmp_data, DATA_LEN_4);
		I("%s: En data[1]=%d, data[0]=%d, cnt=%d\n", __func__,
				tmp_data[1], tmp_data[0], cnt);
		cnt++;
	} while ((tmp_data[1] != wrt_data[1]
		|| tmp_data[0] != wrt_data[0])
		&& cnt < HIMAX_REG_RETRY_TIMES);
}
#endif
...
bool hx83102j_read_event_stack(uint8_t *buf, uint32_t length)
{
...
void hx83102j_init(void)
{
	...
	hx_s_core_fp._read_event_stack = hx83102j_read_event_stack;
...
#endif
	hx_s_core_fp._system_reset = hx83102j_system_reset;
	...
	hx_s_core_fp._burst_enable = hx83102j_burst_enable;
	hx_s_core_fp._register_read = hx83102j_register_read;
	hx_s_core_fp._register_write = hx83102j_register_write;

	hx_s_core_fp._sense_off = hx83102j_sense_off;
	hx_s_core_fp._sense_on = hx83102j_sense_on;
	hx_s_core_fp._chip_init = hx83102j_chip_init;

#if defined(HX_ZERO_FLASH)
	hx_s_core_fp._resume_ic_action = hx83102j_resume_ic_action;
	hx_s_core_fp._0f_reload_to_active = hx83102j_reload_to_active;
	hx_s_core_fp._en_hw_crc = hx83102j_en_hw_crc;
...
static bool hx83102j_chip_detect(void)
{
	uint8_t tmp_data[DATA_LEN_4];
	bool ret = false;
	int i = 0;
	...
	if (hx83102j_sense_off(false) == false) {
		ret = false;
		E("%s:_sense_off Fail:\n", __func__);
		return ret;
	}
	for (i = 0; i < 5; i++) {
		if (hx83102j_register_read(HX83102J_REG_ICID,
		...
		&& ((tmp_data[1] == 0x29))) {
			...
			hx_s_ic_data->dsram_sz =
				HX83102J_DSRAM_SZ;
			hx_s_ic_data->flash_size = HX83102J_FLASH_SIZE;
			...
			g_zf_opt_crc.en_opt_hw_crc = OPT_CRC_SWITCH;
#endif
			hx_s_ic_setup = g_hx83102j_setup;
			hx83102j_init();
...
bool _hx83102j_init(void)
{
	bool ret = false;

	I("%s\n", __func__);
	ret = hx83102j_chip_detect();
	return ret;
}
