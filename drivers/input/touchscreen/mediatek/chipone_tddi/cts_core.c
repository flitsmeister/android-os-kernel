...
#ifdef CFG_CTS_ACTIVEPEN_SUPPORTED            
...
int cts_resume_device(struct cts_device *cts_dev)
{
    ...
    do {
        ...
        if(cts_plat_is_normal_mode(cts_dev->pdata)) {
            cts_info("Resume from sleep, normal mode");
            goto normal_exit;
        }
        else {
            cts_info("Resume from sleep, abnormal mode, retries=%d", retries);
        }
    } while (++retries < 3);
#else 
    cts_reset_device(cts_dev);
...
#endif
    ret = cts_request_firmware(cts_dev, cts_dev->hwdata->hwid, CTS_DEV_FWID_ANY, 0);
    if (ret) {
        cts_info("resume request firmware fail");
