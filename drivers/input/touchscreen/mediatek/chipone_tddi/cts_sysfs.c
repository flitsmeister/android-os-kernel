...
static ssize_t glove_mode_en_show(struct device *dev,
        struct device_attribute *attr, char *buf)
{
    struct chipone_ts_data *cts_data = dev_get_drvdata(dev);
    struct cts_device *cts_dev = &cts_data->cts_dev;

    return sprintf(buf, "Glove mode is %s\n",
            cts_is_glove_enabled(cts_dev) ? "true" : "false");
}

static ssize_t glove_mode_en_store(struct device *dev,
        struct device_attribute *attr, const char *buf, size_t count)
{
    struct chipone_ts_data *cts_data = dev_get_drvdata(dev);
    struct cts_device *cts_dev = &cts_data->cts_dev;
    ...
    if (enable) {
//	cts_err("cts_enter_glove_mode");
        cts_enter_glove_mode(cts_dev);
    } else {
//	cts_err("cts_exit_glove_mode");
...
static DEVICE_ATTR(glove_mode_en, S_IWUSR | S_IRUGO, glove_mode_en_show,
        glove_mode_en_store);
#endif
