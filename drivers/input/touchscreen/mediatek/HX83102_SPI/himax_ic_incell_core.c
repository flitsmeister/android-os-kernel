...
static uint32_t himax_mcu_check_CRC(uint8_t *start_addr, int reload_length)
{
	...
	ret = g_core_fp.fp_register_read(pfw_op->addr_reload_addr_cmd_beat,
							tmp_data,
							DATA_LEN_4);

	if ((tmp_data[1] != (length >> 8)) || (tmp_data[0] != (length&0xff))) {
		E("%s: Flash content is Wrong\n", __func__);
		return HW_CRC_FAIL;
	}
...
static bool himax_mcu_flash_programming(uint8_t *FW_content, int FW_Size)
{
		...
		int len, bool change_iref)
{
	...
	for (counter = 0; counter < 3; counter++) {
		...
		if (g_core_fp.fp_flash_programming(fw, FW_SIZE_64k) == false) {
		#if defined(HX_RST_PIN_FUNC)
			g_core_fp.fp_ic_reset(false, false);
		#else
			g_core_fp.fp_system_reset();
		...
		int len, bool change_iref)
{
	...
	for (counter = 0; counter < 3; counter++) {
		...
		if (g_core_fp.fp_flash_programming(fw, FW_SIZE_128k) == false) {
		#if defined(HX_RST_PIN_FUNC)
			g_core_fp.fp_ic_reset(false, false);
		#else
			g_core_fp.fp_system_reset();
		...
		int len, bool change_iref)
{
	...
	for (counter = 0; counter < 3; counter++) {
		...
		if (g_core_fp.fp_flash_programming(fw, FW_SIZE_255k) == false) {
		#if defined(HX_RST_PIN_FUNC)
			g_core_fp.fp_ic_reset(false, false);
		#else
			g_core_fp.fp_system_reset();
...
int himax_zf_part_info(const struct firmware *fw, int type)
{
	...
	if (cfg_sz > private_ts->chip_max_dsram_size) {
		E("%s: config size error[%d, %d]!!\n", __func__,
			cfg_sz, private_ts->chip_max_dsram_size);
		ret = LENGTH_FAIL;
		goto ALOC_CFG_BUF_FAIL;
	}
