...
#endif
#if 0//CONFIG_TINNO_DEVINFO
static unsigned char g_jd9366t_buf[80] = {0};
#endif
#if defined(CONFIG_TP_TEST_READWRITE_REG)
static struct class  *TP_SPI_Test_Class = NULL;
static dev_t TP_SPI_Test_Devt;
static struct device *TP_SPI_Test_Device;
...
static void module_jd9366t_read_fw_ver(void)
{
	...
	sprintf(g_jd9366t_buf, "YKL_T302AA_jd9366tc_V0x%02x", pjadard_ic_data->fw_cid_ver);
			...
			const char *buf, size_t count)
{
	...
	pr_info("fts_rw_reg_store strlen(buf) = %d  buf = %s \n",strlen(buf),buf);
	if(strlen(buf) == 9) {
		sscanf(buf, "%x", &data);
		reg = data & 0xff;
		opr_flag = 1;
...
static struct attribute * tp_spi_test_attributes[] = {
	&dev_attr_fts_rw_reg.attr,
	NULL,
};

static struct attribute_group tp_spi_test_attribute_group = {
	.attrs = tp_spi_test_attributes
};

static const struct attribute_group * tp_spi_test_groups[] = {
	&tp_spi_test_attribute_group,
	NULL,
};
#endif /*CONFIG_TP_TEST_READWRITE_REG*/
...
static bool module_jd9366t_chip_detect(void)
{
	...
	do {
		...
		if (romid == CHIP_ID) {
			...
			jd9366t_func_reinit();
		#if defined(CONFIG_TP_TEST_READWRITE_REG)
			/* for spi auto test, and current node is fix */
			TP_SPI_Test_Class = class_create(THIS_MODULE, "TP_SPI_Test");
			if (IS_ERR(TP_SPI_Test_Class)) {
				result = PTR_ERR(TP_SPI_Test_Class);
				pr_err("create TP_SPI_Test_Class error\n");
			}
			TP_SPI_Test_Class->dev_groups = tp_spi_test_groups;
			result = alloc_chrdev_region(&TP_SPI_Test_Devt, 0, 1, "TP_SPI_Test_devt");
			if(result < 0) {
				pr_err("alloc_chrdev_region failed!");
			}
			TP_SPI_Test_Device = device_create(TP_SPI_Test_Class, NULL, TP_SPI_Test_Devt, pjadard_ts_data->client,"TP_SPI_Test_Node");
			if (IS_ERR(TP_SPI_Test_Device)) {
				result = PTR_ERR(TP_SPI_Test_Device);
				pr_err("TP_SPI_Test_Device create error: %d" , result);
			}
		#endif
			ret = true;
		} else {
