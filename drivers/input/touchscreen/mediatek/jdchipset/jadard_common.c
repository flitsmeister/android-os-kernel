...
#ifdef JD_PLAM_EN
	/* Report plam event key */
	ts->plam_detect = pjadard_report_data->touch_state_info[0] & 0x01;
	if (ts->plam_detect == 1) {
		/* Plam detect from 0 to 1 */
		if (ts->pre_plam_detect == 0) {
			JD_I("%s: Plam On\n", __func__);
			/*
			* Add report plam on event to system
			*/
		}
	} else {
		/* Plam detect from 1 to 0 */
		if (ts->pre_plam_detect) {
			JD_I("%s: Plam Off\n", __func__);
			/*
			* Add report plam off event to system
			*/
		}
	}
	ts->pre_plam_detect = ts->plam_detect;
#endif
