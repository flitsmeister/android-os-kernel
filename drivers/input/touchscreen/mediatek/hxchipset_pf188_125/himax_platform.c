...
static int hx_panel_coords[4] = {0, 1920, 0, 1200};
static int hx_display_coords[4] = {0, 1920, 0, 1200};
...
//printk("dsy log :   %s %d \n",__FUNCTION__,__LINE__);
	if (dt) {
		...
		if (!match) {
		//	printk("dsy log :   %s %d [Himax]Error: No device match found \n",__FUNCTION__,__LINE__);
			printk("kzhkzh [Himax]Error: No device match found\n");
			return -ENODEV;
		}
	}
...
//	printk("dsy log :   %s %d himax_tpd_rst_gpio_number =%d himax_tpd_int_gpio_number=%d\n",__FUNCTION__,__LINE__,himax_tpd_rst_gpio_number,himax_tpd_int_gpio_number);
//	printk(" dsy kzhkzh %s: int : %2.2x\n", __func__, pdata->gpio_irq);
	printk(" dsy kzhkzh %s: rst : %2.2x\n", __func__, pdata->gpio_reset);
	...
	printk(" dsy kzhkzh %s:panel-coords = %d, %d, %d, %d\n", __func__,
			pdata->abs_x_min,
			pdata->abs_x_max,
			pdata->abs_y_min,
			pdata->abs_y_max);
	...
	printk("dsy kzhkzh  %s:display-coords = (%d, %d)\n", __func__,
			pdata->screenWidth,
			pdata->screenHeight);
	/* report type */
	pdata->protocol_type = report_type;
	printk("dsy log :   %s %d end\n",__FUNCTION__,__LINE__);
	return 0;
}
...
//	printk("dsy log :   %s %d  \n",__FUNCTION__,__LINE__);
	...
	//printk("dsy log :   %s %d retry =%d toRetry =%d\n",__FUNCTION__,__LINE__,retry,toRetry);
	mutex_unlock(&private_ts->rw_lock);
	return 0;
}
EXPORT_SYMBOL(himax_bus_read);
		...
		const struct i2c_device_id *id)
{
	...
	printk("-------dsy kzhkzh %s %d  client->addr =0x%02x ------- \n", __func__,__LINE__,client->addr);
#if defined(MTK_I2C_DMA)
	...
	printk("dsy kzhkzh %s:%d client->addr:0x%x\n",__func__,__LINE__,client->addr);
	client->addr = 0x48;
...
{
	printk("Himax_common touch panel driver init\n");
