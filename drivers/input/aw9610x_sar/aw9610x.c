...
#if 1
static struct input_dev *mid_change_input_dev;
#endif
	...
	for (i = 0; i < AW_CHANNEL_MAX; i++) {
		...
		if (aw9610x->aw_pad[j * AW_CHANNEL_MAX + i].curr_state !=
			...
			case FAR:
				if(i == 1)
				{
					input_report_switch(mid_change_input_dev, SW_MID_SAR_CHANNEL1,0);//The principle of the upper antenna WIFI
					input_sync(mid_change_input_dev);
				}else if(i == 0){
					input_report_switch(mid_change_input_dev, SW_MID_SAR_CHANNEL0,0);//Lower Antenna LTE Principle
					input_sync(mid_change_input_dev);
				}
				input_report_abs(
			...
			case TRIGGER_TH0:
				if(i == 1)
				{
					input_report_switch(mid_change_input_dev, SW_MID_SAR_CHANNEL1,1);//On the antenna WIFI close
					input_sync(mid_change_input_dev);
				}else if(i == 0){
					input_report_switch(mid_change_input_dev, SW_MID_SAR_CHANNEL0,1);//Lower Antenna LTE Close
					input_sync(mid_change_input_dev);
				}
...
//printk("-------dsy  %s %d aw9610x->irq_gpio = %d------- \n", __func__,__LINE__,aw9610x->irq_gpio);
		...
		if (ret) {
			...
			printk("-------dsy  %s %d aw9610x->to_irq = %d------- \n", __func__,__LINE__,aw9610x->to_irq);
			   ...
			   struct device_node *np)
{
	...
	printk("-------dsy  %s %d aw9610x->sar_num = %d------- \n", __func__,__LINE__,aw9610x->sar_num);
	if (val != 0) {
	...
	aw9610x->firmware_flag =
			...
			printk("-------dsy  %s %d aw9610x->firmware_flag = %d------- \n", __func__,__LINE__,aw9610x->firmware_flag);
	AWLOGI(aw9610x->dev, "firmware_flag = <%d>", aw9610x->firmware_flag);

	aw9610x->pwprox_dete =
		of_property_read_bool(np, "aw9610x,using-pwon-prox-dete");
		printk("-------dsy  %s %d aw9610x->pwprox_dete = %d------- \n", __func__,__LINE__,aw9610x->pwprox_dete);
	AWLOGI(aw9610x->dev, "pwprox_dete = <%d>", aw9610x->pwprox_dete);

	aw9610x->satu_release =
		of_property_read_bool(np, "aw9610x,using-satu");
		printk("-------dsy  %s %d aw9610x->satu_release = %d------- \n", __func__,__LINE__,aw9610x->satu_release);
...
//printk("-------dsy  %s %d i2c->addr =0x%02x start ------- \n", __func__,__LINE__,i2c->addr);
...
#if 1
	/* kzhkzh Create input device*/
	mid_change_input_dev = input_allocate_device();
	if (!mid_change_input_dev) {
		printk("%s input_allocate_device fail.\n", __func__);
		goto exit_mid_input_alloc_failed;
	}

	__set_bit(EV_SW, mid_change_input_dev->evbit);
	__set_bit(EV_SYN, mid_change_input_dev->evbit);
	__set_bit(SW_MID_SAR_CHANNEL0, mid_change_input_dev->swbit);//kzhkzh add for MID SAR switch 
	__set_bit(SW_MID_SAR_CHANNEL1, mid_change_input_dev->swbit);//kzhkzh add for MID SAR switch 
	
	mid_change_input_dev->id.bustype = BUS_HOST;
	mid_change_input_dev->id.vendor = 0x0001;
	mid_change_input_dev->id.product = 0x0001;
	mid_change_input_dev->id.version = 0x0100;
	mid_change_input_dev->name = "MID_SAR_NUM";
	
	ret = input_register_device(mid_change_input_dev);
	if (ret) {
		pr_notice("%s input_register_device fail.ret:%d\n", __func__,
			ret);
		goto exit_mid_input_register_device_failed;
	}
	...
	for (i = 0; i < AW_CHANNEL_MAX; i++) {
		...
		__set_bit(SW_MID_SAR_CHANNEL0,aw9610x->aw_pad[j * AW_CHANNEL_MAX + i].input->swbit);//kzhkzh add for MID SAR switch 
...
//printk("-------dsy  %s %d i2c->addr =0x%02x end ------- \n", __func__,__LINE__,i2c->addr);
	return AW_SAR_SUCCESS;
...
exit_mid_input_register_device_failed:
	input_unregister_device(mid_change_input_dev);//kzhkzh add 
exit_input_alloc_failed:
	for (i = 0; i < err_num; i++)
		input_free_device(aw9610x->aw_pad[j * AW_CHANNEL_MAX + i].input);
exit_mid_input_alloc_failed:
	input_free_device(mid_change_input_dev);
err_requst_irq:
	if (gpio_is_valid(aw9610x->irq_gpio))
