...
// added by elink_phil@20171016 for hdmi start <<<
#include <elink_gpio.h>

ELINK_GPIO_POWER power = {
	.gpio = {.name = "power_enable_gpio", .gpio = -1},
	.ref = ATOMIC_INIT(0),
};
// added end >>>
...
#else
	// added by elink_phil for hdmi start <<<
	elink_set_gpio_power_value(&power, 1);
	// added end >>>>
...
int it66121_i2c_read_byte_16addr(u16 addr, u8 *data)
{
	...
	if (true) {
		...
		if (ret < 0) {
			printk("send command error!!,ret = %d\n",ret);
			return -EFAULT;
		}
...
static int hdmi_timer_kthread(void *data)
{
	...
	for (;;) {
		...
		if (hdmi_powerenable == 1)
		{
			...
			HDMITX_DEBUG_PRINTF("ms7210  htotal = %d, vtotal = %d, hactive = %d, pixclk = %d\n",inputtiming.u16_hactive,inputtiming.u16_vtotal,inputtiming.u16_hactive,inputtiming.u16_pixclk);
			mculib_delay_ms(100);
...
static int MS7210_init(void)
{
	...
	/* HDMI_reset(); */
/* FOR MS7210 Init */
	IT66121_LOG("ms7210 chip connect = ", ms7210_chip_connect_detect(0x56));
			  ...
			  const struct i2c_device_id *id)
{
	...
	struct device_node *dn;
	int hdmi_reset_pin;
	printk("move in ms7200 hdmi_ite_probe111\n");
	/* static struct mxc_lcd_platform_data *plat_data; */
	if (!i2c_check_functionality(client->adapter,
				     I2C_FUNC_SMBUS_BYTE | I2C_FUNC_I2C))
		return -ENODEV;
	printk("move in ms7200 hdmi_ite_probe222\n");
#if 0
	...
	if (true) {
		...
		printk(">>Match id Done\n");

		it66121_i2c_client = client;
		// added by elink_phil for hdmi start <<<
		elink_register_common_gpio(client->dev.of_node, &power.gpio);
		// added end >>>
		...
		printk("ms7200->address = %02X\n",it66121_i2c_client->addr);
...
int ite66121_pmic_power_off(void)
{
	...
	// added by elink_phil for hdmi start <<<
	elink_set_gpio_power_value(&power, 0);
	// added end >>>>
...
#if 1
    hdmi_use_pmic_supply = 0;
#else
	if (!!(of_property_read_bool(pdev->dev.of_node, "not_use_pmic_supply")))
		hdmi_use_pmic_supply = 0;
	else
		hdmi_use_pmic_supply = 1;
#endif
...
static int mtk_hdmitx_init(void)
{
	int ret;
