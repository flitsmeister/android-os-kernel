...
//kzhkzh add for bias interface
struct mt6370_pmu_chip *pmu_chip;
#endif
...
#if defined(CONFIG_MFD_MT6370_PMU)
//kzhkzh add for bias interface
int pmu_reg_write(u8 addr, u8 data)
{
#ifdef CONFIG_RT_REGMAP
       struct rt_reg_data rrd = {0};
       int ret = 0;

       mt_dbg(pmu_chip->dev, "%s: reg %02x data %02x\n", __func__,
               addr, data);
       mutex_lock(&pmu_chip->io_lock);
       ret = rt_regmap_reg_write(pmu_chip->rd, &rrd, addr, data);
       mutex_unlock(&pmu_chip->io_lock);
       return ret;
#else
       int ret = 0;
       ...
       mt_dbg(pmu_chip->dev, "%s: reg %02x data %02x\n", __func__,
               addr, data);
       mutex_lock(&pmu_chip->io_lock);
       ret = mt6370_pmu_write_device(pmu_chip->i2c, addr, 1, &data);
       mutex_unlock(&pmu_chip->io_lock);
       return (ret < 0 ? ret : data);
#endif /* #ifdef CONFIG_RT_REGMAP */
}
EXPORT_SYMBOL(pmu_reg_write);
#endif
