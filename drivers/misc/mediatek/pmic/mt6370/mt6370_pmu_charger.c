...
#ifdef  CONFIG_MID_DOCKING_SUPPORT
	if ((otg_mode)||(1 == get_docking_status())) {
#else
	if (otg_mode) {
#endif
...
static int mt6370_enable_otg(struct charger_device *chg_dev, bool en)
{
	...
	mt6370_enable_hidden_mode(chg_data, true);
       ...
       /* Set OTG_OC to 1500mA *///huangxk add for USB DISK
       ret = mt6370_set_otg_current_limit(chg_dev, 1500000);
       if (ret < 0) {
...
#ifndef CONFIG_MT6370_DIS_VBUS_OUT
	ret = (en ? mt6370_pmu_reg_set_bit : mt6370_pmu_reg_clr_bit)
...
#ifdef CONFIG_TCPC_CLASS
#if defined(CONFIG_MACH_MT6771)
//mtk add for cc start
int ret = 0;

int boot_mode = chg_data->bootmode;
...
if (boot_mode == KERNEL_POWER_OFF_CHARGING_BOOT ||
        ...
        dev_info(chg_data->dev, "[KPOC]%s: online = %d\n", __func__, online);
