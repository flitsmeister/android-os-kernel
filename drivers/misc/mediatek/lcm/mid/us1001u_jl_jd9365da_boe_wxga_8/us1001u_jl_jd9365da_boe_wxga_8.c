...
/*************mid add for bias ic-i2c-control interface start****************************/
...
static void ocp2138_dump_register(void)
{
	...
	for (reg_index = 0; reg_index < 4; reg_index++) {
		...
		printk("joyar [0x%x]=0x%x ", reg_index, ocp2138_reg[reg_index]);
	}
	return;
}
static int ocp2138_i2c_probe(struct i2c_client *i2c, const struct i2c_device_id *id)
{
	...
	printk("joyar %s:%d i2c->addr:0x%x\n",__func__,__LINE__,ocp2138_client->addr);
	if(0x3e == ocp2138_client->addr){
		printk("joyar %s:%d enter ocp2138_dump_register\n",__func__,__LINE__);
...
#endif
/*************mid add for bias ic-i2c-control interface end****************************/
...
#if OCP2138_BIAS_I2C_ENABLE	
static void bias_enable(unsigned char enabled)
{
    if (enabled)
    {
...
struct LCM_DRIVER us1001u_jl_jd9365da_boe_wxga_8_lcm_drv = 
{
    .name			= "us1001u_jl_jd9365da_boe_wxga_8",
