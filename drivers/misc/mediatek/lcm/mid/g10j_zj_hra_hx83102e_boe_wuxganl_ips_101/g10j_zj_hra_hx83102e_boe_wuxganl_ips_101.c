...
#define GPIO_LCD_BIAS_EN         GPIO_LCM_BIAS_EN
#define GPIO_LCD_DSI_TE_EN         GPIO_LCM_DSI_TE_EN
...
static void avdd_en(unsigned char enabled)
{
 printk(" [Kernel/LCM]   %s %d  enabled = %d\n",__FUNCTION__,__LINE__,enabled);
	lcm_set_gpio_output(GPIO_LCD_BIAS_EN, enabled);
}

static void dsi_te_en(unsigned char enabled)
{
  printk(" [Kernel/LCM]   %s %d  enabled = %d\n",__FUNCTION__,__LINE__,enabled);
    lcm_set_gpio_output(GPIO_LCD_DSI_TE_EN, enabled);
}
static void tp_pwr_en(unsigned char enabled)
{
    printk(" [Kernel/LCM]   %s %d  enabled = %d\n",__FUNCTION__,__LINE__,enabled); 
    lcm_set_gpio_output(GPIO_CTP_LDO, enabled);
}
static void tp_rst_en(unsigned char enabled)
{
      printk(" [Kernel/LCM]   %s %d  enabled = %d\n",__FUNCTION__,__LINE__,enabled);
    lcm_set_gpio_output(GPIO_CTP_TP_RST_EN, enabled);
}
...
static void lcm_suspend(void)
{
    ...
    dsi_te_en(0);
     MDELAY(30);
printk("-------dsy [Kernel/LCM] %s %d ------ \n", __func__,__LINE__);
	printk("[Kernel/LCM]:  enter lcm_suspend  end !\n");
}
...
struct LCM_DRIVER g10j_zj_hra_hx83102e_boe_wuxganl_ips_101_lcm_drv = {
	.name = "g10j_zj_hra_hx83102e_boe_wuxganl_ips_101",
