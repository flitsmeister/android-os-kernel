...
unsigned int GPIO_LCM_BIAS_ENN;
unsigned int GPIO_LCM_BIAS_ENP;
...
unsigned int GPIO_LCM_LT8911B_RST_EN;
...
static void get_dts_info(void)
{
	...
	ret = of_get_named_gpio(node1, "gpio_lcm_bias_enn", 0);
    LOG("key=gpio_lcm_bias_enn, err=%d\n", ret);
    if(ret >= 0) {
        GPIO_LCM_BIAS_ENN = (unsigned int )ret;
        gpio_request(GPIO_LCM_BIAS_ENN, "GPIO_LCM_BIAS_ENN");
    }

	ret = of_get_named_gpio(node1, "gpio_lcm_bias_enp", 0);
    LOG("key=gpio_lcm_bias_enp, err=%d\n", ret);
    if(ret >= 0) {
        GPIO_LCM_BIAS_ENP = (unsigned int )ret;
        gpio_request(GPIO_LCM_BIAS_ENP, "GPIO_LCM_BIAS_ENP");
    }
    ...
    ret = of_get_named_gpio(node1, "gpio_lcm_lt8911b_rst_en", 0);
    LOG("LCM:key=gpio_lcm_lt8911b_rst_en, ret=%d\n", ret);
    if(ret >= 0) {
        GPIO_LCM_LT8911B_RST_EN = (unsigned int )ret;
        gpio_request(GPIO_LCM_LT8911B_RST_EN, "GPIO_LCM_LT8911B_RST_EN");
    } 
