...
static uint8_t     typec_cc_orientation;
...
/************************************************************************
 *
 *       husb320_typec_cc_orientation_show
 *
 *  Description :
 *  -------------
 *  show cc_status
 *
 ************************************************************************/
static ssize_t husb320_typec_cc_orientation_show(struct device *dev,
										 struct device_attribute *attr,
										 char *buf)
{
	int ret;
...
DEVICE_ATTR(husb320_typec_cc_orientation, S_IRUGO, husb320_typec_cc_orientation_show, NULL);
...
static int husb320_create_devices(struct device *cdev)
{
    ...
    ret = device_create_file(cdev, &dev_attr_husb320_typec_cc_orientation);
	if (ret < 0) {
		dev_err(cdev, "failed to create dev_attr_typec_cc_orientation\n");
		ret = -ENODEV;
		goto err14;
	}
	return ret;
err14:
	device_remove_file(cdev, &dev_attr_husb320_typec_cc_orientation);
err13:
...
static void husb320_work_handler(struct work_struct *work)
{
	...
	if (int_sts & HUSB320_I_DETACH) {
		husb320_detach(chip);
		dev_info(cdev, "%s: interrupt=0x%02x, husb320_detach\n", __func__, int_sts);
	} else {
		if (int_sts & HUSB320_I_ATTACH) {
			husb320_attach(chip);
			dev_info(cdev, "%s: interrupt=0x%02x, husb320_attach!\n", __func__, int_sts);
		}
		if (int_sts & HUSB320_I_BC_LVL) {
			husb320_bclvl_changed(chip);
			dev_info(cdev, "%s: interrupt=0x%02x, husb320_bclvl_changed!!\n", __func__, int_sts);
		}
		if (int_sts & HUSB320_I_AUTOSNK) {
			husb320_autosnk_changed(chip);
			dev_info(cdev, "%s: interrupt=0x%02x, husb320_autosnk_changed!!!\n", __func__, int_sts);
		}
		if (int_sts & HUSB320_I_VBUS_CHG) {
			husb320_vbus_changed(chip);
			dev_info(cdev, "%s: interrupt=0x%02x, husb320_vbus_changed!!!!\n", __func__, int_sts);
		}
		if (int_sts & HUSB320_I_FAULT) {
			husb320_fault_changed(chip);
			dev_info(cdev, "%s: interrupt=0x%02x, husb320_fault_changed!!!!!\n", __func__, int_sts);
		}
		if (int_sts & HUSB320_I_ORIENT) {
			husb320_orient_changed(chip);
			dev_info(cdev, "%s: interrupt=0x%02x, husb320_orient_changed!!!!!!\n", __func__, int_sts);
		}
	}
...
static int husb320_parse_dt(struct husb320_chip *chip)
{
	...
	printk(">>>>>>>>>>>>>>>husb320 data->init_mode = %d \n\r",data->init_mode);
