...
#ifdef CONFIG_CHARGER_HL7005
extern int hl7005_enable_otg(bool en);
#endif

#ifdef  CONFIG_MID_DOCKING_SUPPORT
#include <linux/of_irq.h>
#define COMPATILE_OTG  "mediatek,mid_typec"
static struct workqueue_struct *typec_workqueue;
static struct delayed_work typec_check_work;

extern int get_typec_status(void);
...
static void mtk_usb_extcon_update_role(struct work_struct *work)
{
	...
	printk("[joyar] cur_dr(%d) new_dr(%d)\n", cur_dr, new_dr);
	/* none -> device */
				...
				unsigned long event, void *data)
{
	...
	dev_info(extcon->dev, "online=%d, ignore_usb=%d, type=%d\n",
	 ...
	 printk("[joyar] mtk_usb_extcon_psy_notifier\n");		
	...
	if (pval.intval && (tval.intval == POWER_SUPPLY_TYPE_USB ||
			tval.intval == POWER_SUPPLY_TYPE_USB_CDP)){
	#ifdef CONFIG_MID_DOCKING_SUPPORT
		printk(" DOCKING extcon %s:%d\n",__func__,__LINE__);
		if((get_docking_status() == 0) && (get_typec_status() == 0)){
			printk(" DOCKING extcon current is not OTG plug,set DUAL_PROP_DR_DEVICE %s:%d\n",__func__,__LINE__);
			mtk_usb_extcon_set_role(extcon, DUAL_PROP_DR_DEVICE);
		}
	#else
		mtk_usb_extcon_set_role(extcon, DUAL_PROP_DR_DEVICE);
		...
		printk(" DOCKING extcon %s:%d\n",__func__,__LINE__);
		if((get_docking_status() == 0) && (get_typec_status() == 0)){
			printk(" DOCKING extcon current is not OTG plug,set DUAL_PROP_DR_NONE %s:%d\n",__func__,__LINE__);
			mtk_usb_extcon_set_role(extcon, DUAL_PROP_DR_NONE);
		}
	#else
		mtk_usb_extcon_set_role(extcon, DUAL_PROP_DR_NONE);
...
static int mtk_usb_extcon_psy_init(struct mtk_extcon_info *extcon)
{
	...
	if (IS_ERR_OR_NULL(extcon->usb_psy)) {
		...
		printk("[joyar] fail to get usb_psy\n");
		extcon->usb_psy = NULL;
		return -EINVAL;
	}
							...
							bool is_on)
{
...
#ifdef CONFIG_MID_CHARGER_OTG_DRVBUS
	dev_info(dev, "vbus turn %s\n", is_on ? "on" : "off");
	ret = hl7005_enable_otg(is_on);	
#else
	...
	if (is_on) {
	//liuwuchuan otg start
	    //dev_info(dev, "liuwuchuan--->OTG--vbus regulator enable\n");
		gpiod_set_value(extcon->typeotg_en_gpiod, 1);
		devm_gpiod_put(extcon->dev, extcon->typeotg_en_gpiod);
    //liuwuchuan otg end		
	#ifdef CONFIG_MID_DOCKING_SUPPORT
		gpiod_set_value(extcon->drvbus_gpiod, 1);
		devm_gpiod_put(extcon->dev, extcon->drvbus_gpiod);
	#elif defined(CONFIG_MID_US828ZA_TYPEC_OTG_SUPPORT)
		dev_info(dev, "CONFIG_MID_US828ZA_TYPEC_OTG_SUPPORT 111\n");
		gpiod_set_value(extcon->drvbus_gpiod, 1);
		devm_gpiod_put(extcon->dev, extcon->drvbus_gpiod);

		gpiod_set_value(extcon->otg_5v_en, 1);
		devm_gpiod_put(extcon->dev, extcon->otg_5v_en);
	...
	//liuwuchuan otg start
	    dev_info(dev, "liuwuchuan--->OTG--vbus regulator off\n");
		gpiod_set_value(extcon->typeotg_en_gpiod, 0);
		devm_gpiod_put(extcon->dev, extcon->typeotg_en_gpiod);
    //liuwuchuan otg end		
	#ifdef CONFIG_MID_DOCKING_SUPPORT
		...
		devm_gpiod_put(extcon->dev, extcon->drvbus_gpiod);
	#elif defined(CONFIG_MID_US828ZA_TYPEC_OTG_SUPPORT)
		dev_info(dev, "CONFIG_MID_US828ZA_TYPEC_OTG_SUPPORT 000\n");
		gpiod_set_value(extcon->drvbus_gpiod, 0);
		devm_gpiod_put(extcon->dev, extcon->drvbus_gpiod);

		gpiod_set_value(extcon->otg_5v_en, 0);
		devm_gpiod_put(extcon->dev, extcon->otg_5v_en);
		...
		unsigned long event, void *data)
{
	...
	case TCP_NOTIFY_TYPEC_STATE:
		...
		if (noti->typec_state.old_state == TYPEC_UNATTACHED &&
			...
			dev_info(dev, "liuwuhcuan---->Type-C plug out\n");
			mtk_usb_extcon_set_role(extcon, DUAL_PROP_DR_NONE);
		}
		break;
...
static void mtk_usb_extcon_detect_cable(struct work_struct *work)
{
	...
	if (id) {
		printk("%s: switch to usb, turn off vbus!",__func__);
		mtk_usb_extcon_set_vbus(extcon, false);
		mtk_usb_extcon_set_role(extcon, DUAL_PROP_DR_NONE);
	} else {
		printk("%s: switch to host, turn on vbus!",__func__);
...
static int mtk_usb_extcon_id_pin_init(struct mtk_extcon_info *extcon)
{
	...
	extcon->otg_5v_en = devm_gpiod_get(extcon->dev, "otg_5v_en", GPIOD_OUT_LOW);
	if (!extcon->otg_5v_en || IS_ERR(extcon->otg_5v_en)) {
		dev_info(extcon->dev, "failed to get otg_5v_en gpio\n");
		return -ENODEV;
	}

	extcon->typec_otg_en = devm_gpiod_get(extcon->dev, "typec_otg_en", GPIOD_OUT_LOW);
	if (!extcon->typec_otg_en || IS_ERR(extcon->typec_otg_en)) {
...
#ifdef CONFIG_MID_DOCKING_SUPPORT
static void mt6370_typec_mode_switch(struct work_struct *work){

	struct regulator *vbus = g_extcon->vbus;

	if (IS_ERR_OR_NULL(vbus))
		return;
	if(1 == get_typec_status()){//plug OTG mouse
		printk("#######TYPE-C##############plug in, %s, %d\n",__func__, __LINE__);

		if (regulator_set_voltage(vbus, 5000000, 5000000))
			printk("vbus regulator set voltage failed\n");

		if (regulator_set_current_limit(vbus, 1500000, 1800000))
			printk("vbus regulator set current limit failed\n");

		if (regulator_enable(vbus))
			printk("vbus regulator enable failed\n");
	}else{
		printk("#######TYPE-C##############plug out, %s, %d\n",__func__, __LINE__);
		regulator_disable(vbus);
	}
}

static irqreturn_t typec_eint_interrupt_handler(void)
{
	printk("#######TYPE-C############## %s, %d\n",__func__, __LINE__);
	queue_delayed_work(typec_workqueue, &typec_check_work, msecs_to_jiffies(500));
	return IRQ_HANDLED;
}

static const struct of_device_id typec_of_match[] = {
	{.compatible = COMPATILE_OTG},
	{},
};
#endif
...
//liuwuchuan start
static int mtk_usb_extcon_otg_power_pin_init(struct mtk_extcon_info *extcon)
{

       //liuwuchuan

       dev_info(extcon->dev, "liuwuchuan--->mtk_usb_extcon_otg_power_pin_init\n");
        extcon->typeotg_en_gpiod = devm_gpiod_get(extcon->dev, "typeotg_en", GPIOD_OUT_LOW);
       if (!extcon->typeotg_en_gpiod || IS_ERR(extcon->typeotg_en_gpiod)) {
               dev_info(extcon->dev, "failed to get typeotg_en_gpiod gpio\n");
               return -ENODEV;
       }
...
static int mtk_usb_extcon_probe(struct platform_device *pdev)
{
	...
	static unsigned int typec_id_det;
#endif
...
#ifndef CONFIG_MID_CHARGER_OTG_DRVBUS
	/* vbus */
	...
	mtk_usb_extcon_set_vbus(extcon, false);
       ...
       /*liuwuchuan---statr*/
       ret = mtk_usb_extcon_otg_power_pin_init(extcon);
       if (ret < 0)
               dev_info(dev, "failed to init otg power pin\n");
...
#ifdef CONFIG_MID_DOCKING_SUPPORT
	typec_workqueue = create_workqueue("typec-workqueue");
	INIT_DELAYED_WORK(&typec_check_work, mt6370_typec_mode_switch);
	node = of_find_matching_node(node, typec_of_match);
	if (node) {
		typec_id_det = irq_of_parse_and_map(node, 0);
		ret = request_irq(typec_id_det, (irq_handler_t)typec_eint_interrupt_handler, IRQF_TRIGGER_RISING | IRQF_TRIGGER_FALLING, "typec-eint", NULL);
		if (ret != 0) {
			printk("[MID DOCKING]EINT IRQ LINE NOT AVAILABLE\n");
		} else {
			printk("[MID DOCKING]set EINT finished %s, %d, typec_id_det=%d\n",__func__, __LINE__, typec_id_det);
			queue_delayed_work(typec_workqueue, &typec_check_work, 100);
		}
	} else {
		printk("[MID DOCKING]%s can't find compatible node\n", __func__);
	}
#endif
	return 0;
}
