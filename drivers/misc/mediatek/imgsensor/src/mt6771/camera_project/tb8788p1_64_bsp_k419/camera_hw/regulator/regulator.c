...
extern int mt6357_cust_set_vcamd(struct regulator *regulator, int min_uV, int max_uV);
#endif
	...
	enum IMGSENSOR_HW_PIN_STATE pin_state)
{
...
#ifdef CONFIG_MID_MT8788_CAM_REGULATOR_USE_SAME
	...
	enable_cnt = preg->enable_cnt + (reg_type_offset + pin -
     ...
     //kzhkzh add for MT8788 main camera AF control
     if((pin == 5) && (sensor_idx == 0) && (pin_state >= 0)){//after DOVDD to supply af;
         mt6358_upmu_set_rg_ldo_vldo28_en(1);//main AF power on
     }else if((pin == 5) && (sensor_idx == 0) && (pin_state == 0)){
         mt6358_upmu_set_rg_ldo_vldo28_en(0);//main AF power off 2.8
     }

     //kzhkzh add for MT8788 sub camera AF control
     if((pin == 5) && (sensor_idx == 1) && (pin_state >= 0)){
         mt6358_upmu_set_rg_ldo_vldo28_en(1);//main AF power on
     }else if((pin == 5) && (sensor_idx == 1) && (pin_state == 0)){
         mt6358_upmu_set_rg_ldo_vldo28_en(0);//main AF power off 2.8
     }///*add end*/
	...
	if (pregulator) {
		if (pin_state != IMGSENSOR_HW_PIN_STATE_LEVEL_0) {
			...
			if(pin == 4){	//DVDD
				...
				PK_DBG(
				    "[regulator]mt6357_cust_set_vcamd, powertype:%d powerId:%d\n",
				    pin,
				    regulator_voltage[
				   pin_state - IMGSENSOR_HW_PIN_STATE_LEVEL_0]);
				}
