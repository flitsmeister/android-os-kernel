...
#define PFX "s5k5e8yx_camera_sensor"
#define LOG_1 pr_info_printk("s5k5e8yx, MIPI 2LANE\n")
/****************************   Modify end    *******************************************/

#define pr_info_printk(format, args...)    printk(PFX "[%s] " format, __func__, ##args)
...
{ 2592, 1944,	  0,    0, 2592, 1944, 2592, 1944, 0000, 0000, 2592, 1944,	  0,	0, 2592, 1944},
//hight speed video
{ 2592, 1944,	  24,  20, 2560, 1920,  640,  480, 0000, 0000,  640,  480,	  0,	0,  640,  480},
// slim video
{ 2592, 1944,	  24, 260, 2560, 1440, 1280,  720, 0000, 0000, 1280,  720,	  0,	0, 1280,  720} };
...
{
	pr_info_printk("dummyline = %d, dummypixels = %d\n",
			...
			kal_bool min_framelength_en)
{
	...
	pr_info_printk("framerate = %d, min framelength should enable? %d\n",
...
static void write_shutter(kal_uint16 shutter)
{
	...
	pr_info_printk("shutter =%d, framelength =%d\n",
			...
			kal_uint16 frame_length)
{
	...
	write_cmos_sensor(0x0203, shutter  & 0xFF);
	//write_cmos_sensor(0x0104, 0x00);
	pr_info_printk("Add for N3D! shutter =%d, framelength =%d\n",
		shutter, imgsensor.frame_length);
...
static kal_uint16 set_gain(kal_uint16 gain)
{
	...
	spin_unlock(&imgsensor_drv_lock);
	pr_info_printk("gain = %d , reg_gain = 0x%x\n ",
		gain, reg_gain);
...
{
	pr_info_printk("le:0x%x, se:0x%x, gain:0x%x\n", le, se, gain);
...
static void set_mirror_flip(kal_uint8 image_mirror)
{
	...
	case IMAGE_HV_MIRROR:
		...
		break;
	default:
		pr_info_printk("Error image_mirror setting\n");
	}
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
	...
	pr_info_printk("S5K5E8YXFRONT_I2CBUS = %d\n", I2C_bus);
	if (I2C_bus != S5K5E8YXFRONT_I2CBUS) {
		*sensor_id = 0xFFFFFFFF;
		pr_info_printk("S5K5E8YXFRONT_I2CBUS:%d Check Error!\n", I2C_bus);
		...
		do {
			*sensor_id = return_sensor_id();
			if (*sensor_id == imgsensor_info.sensor_id) {
				pr_info_printk("i2c write id: 0x%x, sensor id: 0x%x\n",
					imgsensor.i2c_write_id, *sensor_id);
				return ERROR_NONE;
			}
			pr_info_printk("Read id fail,i2c_write_id 0x%x id: 0x%x\n",
				imgsensor.i2c_write_id, *sensor_id);
			retry--;
		} while (retry > 0);
		i++;
...
static kal_uint32 open(void)
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			...
			pr_info_printk("R sensorid fail,write_id 0x%x id: 0x%x\n",
				imgsensor.i2c_write_id, sensor_id);
			retry--;
		} while (retry > 0);
		i++;
			...
			MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	if (imgsensor.current_fps == imgsensor_info.cap.max_framerate) {
		...
		pr_info_printk("current_fps %d not support, so cap1's: %d fps!\n",
		...
		pr_info_printk("current_fps %d not support, so use cap2's: %d fps!\n",
			imgsensor.current_fps,
			...
			MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	case MSDK_SCENARIO_ID_SLIM_VIDEO:
		...
		break;
	default:
		pr_info_printk("Error ScenarioId setting");
		preview(image_window, sensor_config_data);
...
{
	pr_info_printk("enable = %d, framerate = %d\n", enable, framerate);
	spin_lock(&imgsensor_drv_lock);
			...
			MUINT32 framerate)
{
	...
	pr_info_printk("scenario_id = %d, framerate = %d\n",
	...
	case MSDK_SCENARIO_ID_CAMERA_CAPTURE_JPEG:
		if (imgsensor.current_fps ==
			...
			if (imgsensor.current_fps !=
				...
				pr_info_printk("%d fps not support,cap's: %d fps\n",
					framerate,
	...
	default:  //coding with  preview scenario by default
		...
		spin_unlock(&imgsensor_drv_lock);
		set_dummy();
		pr_info_printk("error scenario_id = %d, preview scenario\n",
			scenario_id);
		break;
	}
	return ERROR_NONE;
			...
			UINT8 *feature_para, UINT32 *feature_para_len)
{
	...
	pr_info_printk("feature_id = %d", feature_id);
	switch (feature_id) {
	...
	case SENSOR_FEATURE_GET_PIXEL_CLOCK_FREQ:
		pr_info_printk("imgsensor.pclk = %d,imgsensor.current_fps = %d\n",
	...
	case SENSOR_FEATURE_SET_FRAMERATE:
		pr_info_printk("current fps :%d\n", (UINT32)*feature_data);
		spin_lock(&imgsensor_drv_lock);
		imgsensor.current_fps = *feature_data;
		spin_unlock(&imgsensor_drv_lock);
		break;
	case SENSOR_FEATURE_SET_HDR:
		//pr_info_printk("ihdr enable :%d\n", (BOOL)*feature_data_16);
		pr_info_printk("Warning! Not Support IHDR Feature");
		spin_lock(&imgsensor_drv_lock);
	...
	case SENSOR_FEATURE_GET_CROP_INFO:
		pr_info_printk("SENSOR_FEATURE_GET_CROP_INFO scenarioId:%llu\n",
			*feature_data);
	...
	case SENSOR_FEATURE_SET_IHDR_SHUTTER_GAIN:
		pr_info_printk("SET_SENSOR_IHDR LE=%d, SE=%d, Gain=%d\n",
