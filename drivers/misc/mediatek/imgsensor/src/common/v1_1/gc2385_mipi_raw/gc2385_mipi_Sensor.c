...
//#define LOG_INF(fmt,arg...)    printk("<<GC2385_camera_sensor>>[%s:%d] "fmt"\n", __func__, __LINE__, ##arg)
...
static struct imgsensor_info_struct imgsensor_info = {
    ...
    .max_frame_length = 0x24cf,//max framelength by sensor register's limitation
        ...
        do {
            *sensor_id = return_sensor_id()&0xffef;
            if (*sensor_id == imgsensor_info.sensor_id) {
                LOG_INF("2385kzh i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
                return ERROR_NONE;
            }
            LOG_INF("2385kzh Read sensor id fail, write id: 0x%x, id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
            retry--;
        } while(retry > 0);
        i++;
        retry = 2;
...
static kal_uint32 open(void)
{
    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
        ...
        do {
            sensor_id = return_sensor_id()&0xffef;
            if (sensor_id == imgsensor_info.sensor_id) {
                LOG_INF("kzh open i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,sensor_id);
                break;
            }
            LOG_INF(" kzh open Read sensor id fail, write id: 0x%x, id: 0x%x\n", imgsensor.i2c_write_id,sensor_id);
            retry--;
        } while(retry > 0);
        i++;
        if (sensor_id == imgsensor_info.sensor_id)
...
UINT32 GC2385MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
