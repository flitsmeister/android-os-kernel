...
{{ 4208, 3120,	  0,	0, 4208, 3120, 2104,  1560, 0000, 0000, 2104,  1560,	  0,	0, 2104, 1560}, // Preview
 ...
 { 4208, 3120,	  0,	0, 4208, 3120, 2104,  1560, 0000, 0000, 2104,  1560,	  0,	0, 2104, 1560}}; // custom2
...
static void sensor_init(void)
{
	...
	write_cmos_sensor(0x9707,	0x03); 
	write_cmos_sensor(0x9708,	0x03); 
	write_cmos_sensor(0x97BE,	0x01); 
	write_cmos_sensor(0x97BF,	0x01); 
	write_cmos_sensor(0x97C0,	0x01); 
...
static void preview_setting(kal_uint16 currefps)
{
	...
	// 24fps
        ...
        write_cmos_sensor(0x0221,0x11);
        write_cmos_sensor(0x0222,0x01);
        write_cmos_sensor(0x0340,0x0A);// modify framlength 2112 to 2662
        write_cmos_sensor(0x0341,0x66);
        write_cmos_sensor(0x0342,0x13);
        write_cmos_sensor(0x0343,0x90);
        write_cmos_sensor(0x0344,0x00);
        write_cmos_sensor(0x0345,0x00);
		...
		do {
			*sensor_id = ((read_cmos_sensor(0x0016) << 8) | read_cmos_sensor(0x0017));
			if (*sensor_id == imgsensor_info.sensor_id) {
				printk("imx214 i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
				/*iReadData(0x00,452,OTPData);*//*LukeHu--150810*/
				return ERROR_NONE;
			}
			printk("imx214 Read sensor id fail, write id:0x%x id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
			retry--;
		} while(retry > 0);
		i++;
		retry = 2;
...
static kal_uint32 open(void)
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			sensor_id = ((read_cmos_sensor(0x0016) << 8) | read_cmos_sensor(0x0017));
			if (sensor_id == imgsensor_info.sensor_id) {
				printk("xuchen imx214 i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,sensor_id);
				break;
			}
			printk("xuchen imx214 Read sensor id fail, write id:0x%x id: 0x%x\n", imgsensor.i2c_write_id,sensor_id);
			retry--;
		} while(retry > 0);
		i++;
		if (sensor_id == imgsensor_info.sensor_id)
						  ...
						  MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
    ...
    imgsensor.autoflicker_en = KAL_FALSE;
	spin_unlock(&imgsensor_drv_lock);
	capture_setting(imgsensor_info.custom1.max_framerate);
						  ...
						  MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
    ...
    imgsensor.autoflicker_en = KAL_FALSE;
	spin_unlock(&imgsensor_drv_lock);
	preview_setting(imgsensor_info.custom2.max_framerate);
	return ERROR_NONE;
}   /*	custom2	 */
