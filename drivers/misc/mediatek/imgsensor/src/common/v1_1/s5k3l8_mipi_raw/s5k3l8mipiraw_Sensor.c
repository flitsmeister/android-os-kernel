 ...
 *  15072011511229: add ?¨¦¦Ì?log??¨¨Y¡ê?D?¦Ì?log?¨²?a??¡ã?¡À?2??¨¹¡ä¨°¨®?log¡ê?¡ê?
 *  15072209190629: non - continue mode bandwidth limited , has <tiaowen> , modify to continue mode
 ...
 *  15080409230000: pass!
 *  15082414330000: move to mt6580.USE NEW LOG FORMAT
 *------------------------------------------------------------------------------
...
static struct imgsensor_info_struct imgsensor_info = {
	...
	.grabwindow_height = 3104,		/*record different mode's height of grabwindow*/
		/*       following for MIPIDataLowPwr2HighSpeedSettleDelayCount by different scenario   */
		.mipi_data_lp2hs_settle_dc = 85,
		/*       following for GetDefaultFramerateByScenario()  */
		.max_framerate = 300,
	...
	.slim_video = {
		.pclk = 560000000,	/* record different mode's pclk */
		.linelength = 5808,	/* record different mode's linelength */
		.framelength = 3250,			/*record different mode's framelength*/
		.startx = 0,	/* record different mode's startx of grabwindow */
		.starty = 0,	/* record different mode's starty of grabwindow */
		.grabwindow_width = 1280,	/* record different mode's width of grabwindow */
		.grabwindow_height = 720,	/* record different mode's height of grabwindow */
...
#if defined CONFIG_CAM_FLIP_HV_3L8
...
static struct SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info[10] = {
	{ 4208, 3120, 0, 0, 4208, 3120, 2104, 1560, 0,	0, 2104, 1560, 4, 4, 2096, 1552}, /* Preview */
	{ 4208, 3120, 0, 0, 4208, 3120, 4208, 3120, 0,	0, 4208, 3120, 8, 8, 4192, 3104}, /* capture */
	{ 4208, 3120, 0, 0, 4208, 3120, 4208, 3120, 0,	0, 4208, 3120, 8, 8, 4192, 3104}, /* video */
...
static void preview_setting(void)
{
	...
	write_cmos_sensor(0x316A, 0x0030);
	write_cmos_sensor(0x0100, 0x0100);
	check_output_stream_on();
}				/*      preview_setting  */
...
static void capture_setting(kal_uint16 currefps)
{
	...
	check_output_stream_on();
		...
		/* PIP 15fps settings,?¨¤¡À¨¨Full 30fps */
		/* -VT : 560-> 400M */
...
{
#if defined(CONFIG_KTE_DRV_BIND_CAMERA_IDX)
	extern int pinSetIdx;
	printk("[qyCam] sensor_idx:%d\n", pinSetIdx);
	return ((read_cmos_sensor_byte(0x0000) << 8) | read_cmos_sensor_byte(0x0001)) + pinSetIdx;
#else
...
{
	LOG_INF("enable: %d\n", enable);
		return ERROR_NONE; /*xiaohua add 20220919*/
