...
#include "sc201csmipi_Sensor.h"

/************************** Modify Following Strings for Debug **************************/
#define PFX "sc201cs_camera_sensor_JF"
#define LOG_1 LOG_INF("SC201CSMIPI, 1LANE\n")
/****************************   Modify end    *******************************************/
#define SC201CS_DEBUG 1
#if SC201CS_DEBUG
...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = SC201CSMIPI_SENSOR_ID,
...
static kal_uint16 set_gain(kal_uint16 gain)
{
	...
	kal_uint16 SC201CS_AGC_Param[SC201CS_SENSOR_GAIN_MAP_SIZE][2] = {
		{  1024,  0x00 },
		{  2048,  0x01 },
		{  4096,  0x03 },
		{  8192,  0x07 },
	...
	LOG_INF("Exit! SC201CS_AGC_Param[gain_index][1] = 0x%x, temp_gain = 0x%x, gain = 0x%x, reg_gain = %d\n",
		SC201CS_AGC_Param[gain_index][1], temp_gain, gain, reg_gain);
		...
		do {
			*sensor_id = return_sensor_id();
			if (*sensor_id == imgsensor_info.sensor_id) {
				pr_debug("[sc201cs_camera_sensor]get_imgsensor_id:i2c write id: 0x%x, sensor id: 0x%x\n",
					imgsensor.i2c_write_id, *sensor_id);
				return ERROR_NONE;
			}
			pr_debug("[sc201cs_camera_sensor]get_imgsensor_id:Read sensor id fail, write id: 0x%x, id: 0x%x\n",
				imgsensor.i2c_write_id, *sensor_id);
			retry--;
		} while (retry > 0);
		i++;
...
static kal_uint32 open(void)
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			sensor_id = return_sensor_id();
			if (sensor_id == imgsensor_info.sensor_id) {
				pr_debug("[sc201cs_camera_sensor]open:i2c write id: 0x%x, sensor id: 0x%x\n",
					imgsensor.i2c_write_id, sensor_id);
				break;
			}
			pr_debug("[sc201cs_camera_sensor]open:Read sensor id fail, write id: 0x%x, id: 0x%x\n",
				imgsensor.i2c_write_id, sensor_id);
			retry--;
		} while (retry > 0);
		i++;
