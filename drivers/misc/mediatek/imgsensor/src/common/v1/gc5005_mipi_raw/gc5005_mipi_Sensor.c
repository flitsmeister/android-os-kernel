...
static imgsensor_info_struct imgsensor_info = {
    .sensor_id = GC5005_SENSOR_ID,  //   GC5005MIPI_SENSOR_ID    //record sensor id defined in Kd_imgsensor.h
...
static void gc5005_gcore_update_dd(void)
{
	...
	if(0x01 ==gc5005_otp_info.dd_flag)
	{
		...
		for(i=0; i<gc5005_otp_info.dd_cnt; i++)
		{
			...
			while((i < gc5005_otp_info.dd_cnt - 1) && (gc5005_otp_info.dd_param_x[i]==gc5005_otp_info.dd_param_x[i+1]) && (gc5005_otp_info.dd_param_y[i]==gc5005_otp_info.dd_param_y[i+1]))
				{
					flag = 1;
					i++;
				}
        ...
        do {
            *sensor_id = return_sensor_id();
            if (*sensor_id == imgsensor_info.sensor_id) {
				printk("hezhensheng---i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
                //LOG_INF("i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
                return ERROR_NONE;
            }
           // LOG_INF("Read sensor id fail, write id: 0x%x, id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
			printk("hezhensheng---Read sensor id fail, write id: 0x%x, id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
            retry--;
        } while(retry > 0);
        i++;
        retry = 2;
