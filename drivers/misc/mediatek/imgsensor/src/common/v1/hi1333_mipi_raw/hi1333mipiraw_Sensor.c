...
static struct imgsensor_info_struct imgsensor_info = { 
	...
	.checksum_value = 0x17affdd,       //0x6d01485c // Auto Test Mode ÃßÈÄ..
	.pre = {
...
static struct SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info[5] =
{
	...
	{ 4240, 3124,   0,   0, 4240, 3124,	 2120, 1562,  16, 2, 2104, 1560, 0, 0, 2104, 1560},		// VIDEO (4208 x 3120)
...
write_cmos_sensor(0x000e, 0x0200); //image orient//mirror
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id) 
{
			...
			do {
				*sensor_id = ((read_cmos_sensor(0x0F17) << 8) | read_cmos_sensor(0x0F16));
				...
				if (*sensor_id == imgsensor_info.sensor_id) {				
					pr_info("[hi1333mipiraw_Sensor] i2c write id  : 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);	  

                    return ERROR_NONE;
			}	
				pr_info("[hi1333mipiraw_Sensor]get_imgsensor_id Read sensor id fail, id: 0x%x 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
				retry--;
			} while(retry > 0);
			i++;
			retry = 2;
	...
	if (*sensor_id != imgsensor_info.sensor_id) {
		...
		*sensor_id = 0xFFFFFFFF;
        pr_info( "hi1333mipiraw_Sensor] get_imgsensor_id(): FAIL\n" );
		return ERROR_SENSOR_CONNECT_FAIL;
	}
...
static kal_uint32 open(void)
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			...
			pr_info("[hi1333mipiraw_Sensor]Read sensor id fail, write id:0x%x id: 0x%x\n", imgsensor.i2c_write_id,sensor_id);
			retry--;
		} while(retry > 0);
		i++;
		if (sensor_id == imgsensor_info.sensor_id)
	...
	pr_info("hi1333_find sensor pass!! Start initial setting\n");
