...
{{ 2592, 1944, 0, 0, 2592, 1944, 2592, 1944, 0000, 0000, 2592, 1944, 0000, 0000, 2592, 1944}, // Preview 2112*1558
 { 2592, 1944, 0, 0, 2592, 1944, 2592, 1944, 0000, 0000, 2592, 1944, 0000, 0000, 2592, 1944}, // capture 4206*3128
 { 2592, 1944, 0, 0, 2592, 1944, 2592, 1944, 0000, 0000, 2592, 1944, 0000, 0000, 2592, 1944}, // video 
 { 2592, 1944, 0, 0, 2592, 1944, 2592, 1944, 0000, 0000, 2592, 1944, 0000, 0000, 2592, 1944}, //hight speed video 
 { 2592, 1944, 0, 0, 2592, 1944, 2592, 1944, 0000, 0000, 2592, 1944, 0000, 0000, 2592, 1944},// slim video
};
   	    	...
   	    	reg_gain = 0x10 * gain/BASEGAIN ;        //change mtk gain base to aptina gain base
   	    	 ...
   	    	 if(reg_gain<=0x10)
   	    	 {
			 	...
			 	write_cmos_sensor(0x55, 0x24);
   	    	 	write_cmos_sensor(0x01, 0x01);
   	    	 	LOG_INF("SP5508MIPI reg_gain =%d, SP5508MIPI Gain = %d,P1:0x55 = 0x%x\n", reg_gain, gain,read_cmos_sensor(0x55));
   	    	 }
			else if(reg_gain <= 0x18)
			{
				write_cmos_sensor(0xfd, 0x01);
    ...
    /*    MIPI_sensor_Init  */
{
	...
	if(fromsd == 1)//是否从SD读取//gepeiwei   120903
		...
		sp5508MIPI_Initialize_from_T_Flash();//从SD卡读取的主要函数
        ...
        do {
            *sensor_id = return_sensor_id();
            if (*sensor_id == imgsensor_info.sensor_id) {
                printk("sp5508 superpix i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
                return ERROR_NONE;
            }
			printk("sp5508 superpix Read sensor id fail, write id: 0x%x, read id: 0x%x\n", imgsensor.i2c_write_id, *sensor_id);
            retry--;
        } while(retry > 0);
        i++;
        retry = 2;
...
UINT32 SP5508_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
