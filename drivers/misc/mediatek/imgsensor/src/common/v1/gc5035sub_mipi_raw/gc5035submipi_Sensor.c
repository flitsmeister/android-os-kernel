...
#include "gc5035submipi_Sensor.h"

/**************** Modify Following Strings for Debug ******************/
#define PFX "gc5035sub_camera_sensor"
#define LOG_1 cam_pr_debug("GC5035SUBMIPI, 2LANE\n")
/********************   Modify end    *********************************/
...
static kal_uint32 Dgain_ratio = GC5035SUB_SENSOR_DGAIN_BASE;
static struct gc5035sub_otp_t gc5035sub_otp_data;

static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = GC5035SUBMIPI_SENSOR_ID,
...
#if GC5035SUB_MIRROR_FLIP_ENABLE
...
static kal_uint8 gc5035sub_otp_read_byte(kal_uint16 addr)
{
...
static void gc5035sub_otp_read_group(kal_uint16 addr,
...
static void gc5035sub_gcore_read_dpc(void)
{
	kal_uint8 dpcFlag = 0;
	...
	default:
		pDPC->flag = GC5035SUB_OTP_FLAG_INVALID;
		break;
	}
}

static void gc5035sub_gcore_read_reg(void)
{
	kal_uint8 i = 0;
	kal_uint8 j = 0;
	kal_uint16 base_group = 0;
	kal_uint8 reg[GC5035SUB_OTP_REG_DATA_SIZE];
	struct gc5035sub_reg_update_t *pRegs = &gc5035sub_otp_data.regs;
	...
	if (pRegs->flag == GC5035SUB_OTP_FLAG_VALID) {
		...
		for (i = 0; i < GC5035SUB_OTP_REG_MAX_GROUP; i++) {
			base_group = i * GC5035SUB_OTP_REG_BYTE_PER_GROUP;
			for (j = 0; j < GC5035SUB_OTP_REG_REG_PER_GROUP; j++)
				if (GC5035SUB_OTP_CHECK_1BIT_FLAG(reg[base_group],
						(4 * j + 3))) {
					...
					pRegs->reg[pRegs->cnt].addr =
						reg[base_group + j *
						GC5035SUB_OTP_REG_BYTE_PER_REG
						+ 1];
...
#if GC5035SUB_OTP_FOR_CUSTOMER
static kal_uint8 gc5035sub_otp_read_module_info(void)
{
	kal_uint8 i = 0;
	kal_uint8 idx = 0;
	kal_uint8 flag = 0;
	kal_uint16 check = 0;
	kal_uint16 module_start_offset = GC5035SUB_OTP_MODULE_DATA_OFFSET;
	kal_uint8 info[GC5035SUB_OTP_MODULE_DATA_SIZE];
	struct gc5035sub_module_info_t module_info = { 0 };

	memset(&info, 0, GC5035SUB_OTP_MODULE_DATA_SIZE);
	...
	for (idx = 0; idx < GC5035SUB_OTP_GROUP_CNT; idx++) {
		...
		case GC5035SUB_OTP_FLAG_VALID: {
			...
			gc5035sub_otp_read_group(module_start_offset,
				&info[0], GC5035SUB_OTP_MODULE_DATA_SIZE);
			for (i = 0; i < GC5035SUB_OTP_MODULE_DATA_SIZE - 1; i++)
				check += info[i];
...
static void gc5035sub_otp_read_wb_info(void)
{
	kal_uint8 i = 0;
	kal_uint8 idx = 0;
	kal_uint8 flag = 0;
	...
	kal_uint16 wb_start_offset = GC5035SUB_OTP_WB_DATA_OFFSET;
	kal_uint16 golden_start_offset = GC5035SUB_OTP_GOLDEN_DATA_OFFSET;
	kal_uint8 wb[GC5035SUB_OTP_WB_DATA_SIZE];
	kal_uint8 golden[GC5035SUB_OTP_GOLDEN_DATA_SIZE];
	struct gc5035sub_wb_t *pWB = &gc5035sub_otp_data.wb;
	struct gc5035sub_wb_t *pGolden = &gc5035sub_otp_data.golden;
	...
	for (idx = 0; idx < GC5035SUB_OTP_GROUP_CNT; idx++) {
		...
		case GC5035SUB_OTP_FLAG_VALID: {
			...
			gc5035sub_otp_read_group(wb_start_offset, &wb[0],
				GC5035SUB_OTP_WB_DATA_SIZE);

			for (i = 0; i < GC5035SUB_OTP_WB_DATA_SIZE - 1; i++)
				wb_check += wb[i];
			...
			if ((wb_check % 255 + 1) ==
				...
				pWB->rg = pWB->rg == 0 ?
					GC5035SUB_OTP_WB_RG_TYPICAL : pWB->rg;
				pWB->bg = pWB->bg == 0 ?
		...
		case GC5035SUB_OTP_FLAG_VALID: {
			...
			gc5035sub_otp_read_group(golden_start_offset,
				&golden[0], GC5035SUB_OTP_GOLDEN_DATA_SIZE);
			for (i = 0; i < GC5035SUB_OTP_GOLDEN_DATA_SIZE - 1; i++)
				golden_check += golden[i];

			if ((golden_check % 255 + 1) ==
				golden[GC5035SUB_OTP_GOLDEN_DATA_SIZE - 1]) {
				pGolden->rg = (golden[0] | ((golden[1] &
						0xf0) << 4));
				pGolden->bg = (((golden[1] & 0x0f) << 8) |
						golden[2]);
				pGolden->rg = pGolden->rg == 0 ?
					GC5035SUB_OTP_WB_RG_TYPICAL : pGolden->rg;
...
static kal_uint8 gc5035sub_otp_read_sensor_info(void)
{
	kal_uint8 moduleID = 0;
#if GC5035SUB_OTP_DEBUG
	kal_uint16 i = 0;
	kal_uint8 debug[GC5035SUB_OTP_DATA_LENGTH];
#endif

	gc5035sub_gcore_read_dpc();
	gc5035sub_gcore_read_reg();
#if GC5035SUB_OTP_FOR_CUSTOMER
	moduleID = gc5035sub_otp_read_module_info();
	gc5035sub_otp_read_wb_info();

#endif

#if GC5035SUB_OTP_DEBUG
	memset(&debug[0], 0, GC5035SUB_OTP_DATA_LENGTH);
	gc5035sub_otp_read_group(GC5035SUB_OTP_START_ADDR, &debug[0],
			GC5035SUB_OTP_DATA_LENGTH);
	for (i = 0; i < GC5035SUB_OTP_DATA_LENGTH; i++)
		cam_pr_debug("addr = 0x%x, data = 0x%x\n",
				GC5035SUB_OTP_START_ADDR + i * 8, debug[i]);
#endif

	return moduleID;
}

static void gc5035sub_otp_update_dd(void)
{
	kal_uint8 state = 0;
	kal_uint8 n = 0;
	struct gc5035sub_dpc_t *pDPC = &gc5035sub_otp_data.dpc;

	if (pDPC->flag == GC5035SUB_OTP_FLAG_VALID) {
...
#if GC5035SUB_OTP_FOR_CUSTOMER
static void gc5035sub_otp_update_wb(void)
{
	kal_uint16 r_gain = GC5035SUB_OTP_WB_GAIN_BASE;
	kal_uint16 g_gain = GC5035SUB_OTP_WB_GAIN_BASE;
	kal_uint16 b_gain = GC5035SUB_OTP_WB_GAIN_BASE;
	kal_uint16 base_gain = GC5035SUB_OTP_WB_CAL_BASE;
	kal_uint16 r_gain_curr = GC5035SUB_OTP_WB_CAL_BASE;
	kal_uint16 g_gain_curr = GC5035SUB_OTP_WB_CAL_BASE;
	kal_uint16 b_gain_curr = GC5035SUB_OTP_WB_CAL_BASE;
	kal_uint16 rg_typical = GC5035SUB_OTP_WB_RG_TYPICAL;
	kal_uint16 bg_typical = GC5035SUB_OTP_WB_BG_TYPICAL;
	struct gc5035sub_wb_t *pWB = &gc5035sub_otp_data.wb;
	struct gc5035sub_wb_t *pGolden = &gc5035sub_otp_data.golden;

	if (GC5035SUB_OTP_CHECK_1BIT_FLAG(pGolden->flag, 0)) {
		rg_typical = pGolden->rg;
		bg_typical = pGolden->bg;
	} else {
		rg_typical = GC5035SUB_OTP_WB_RG_TYPICAL;
		bg_typical = GC5035SUB_OTP_WB_BG_TYPICAL;
	}
	cam_pr_debug("typical rg = 0x%x, bg = 0x%x\n", rg_typical, bg_typical);

	if (GC5035SUB_OTP_CHECK_1BIT_FLAG(pWB->flag, 0)) {
		r_gain_curr = GC5035SUB_OTP_WB_CAL_BASE * rg_typical / pWB->rg;
		b_gain_curr = GC5035SUB_OTP_WB_CAL_BASE * bg_typical / pWB->bg;
		g_gain_curr = GC5035SUB_OTP_WB_CAL_BASE;

		base_gain = (r_gain_curr < b_gain_curr) ?
			r_gain_curr : b_gain_curr;
		base_gain = (base_gain < g_gain_curr) ? base_gain : g_gain_curr;

		r_gain = GC5035SUB_OTP_WB_GAIN_BASE * r_gain_curr / base_gain;
		g_gain = GC5035SUB_OTP_WB_GAIN_BASE * g_gain_curr / base_gain;
		b_gain = GC5035SUB_OTP_WB_GAIN_BASE * b_gain_curr / base_gain;
...
static void gc5035sub_otp_update_reg(void)
{
	kal_uint8 i = 0;

	cam_pr_debug("reg count = %d\n", gc5035sub_otp_data.regs.cnt);

	if (GC5035SUB_OTP_CHECK_1BIT_FLAG(gc5035sub_otp_data.regs.flag, 0))
		for (i = 0; i < gc5035sub_otp_data.regs.cnt; i++) {
			write_cmos_sensor(0xfe,
					gc5035sub_otp_data.regs.reg[i].page);
			write_cmos_sensor(gc5035sub_otp_data.regs.reg[i].addr,
					gc5035sub_otp_data.regs.reg[i].value);
			cam_pr_debug("reg[%d] P%d:0x%x -> 0x%x\n", i,
				gc5035sub_otp_data.regs.reg[i].page,
				gc5035sub_otp_data.regs.reg[i].addr,
				gc5035sub_otp_data.regs.reg[i].value);
		}
}

static void gc5035sub_otp_update(void)
{
	gc5035sub_otp_update_dd();
#if GC5035SUB_OTP_FOR_CUSTOMER
	gc5035sub_otp_update_wb();
#endif
	gc5035sub_otp_update_reg();
}

static kal_uint8 gc5035sub_otp_identify(void)
{
	kal_uint8 moduleID = 0;

	memset(&gc5035sub_otp_data, 0, sizeof(gc5035sub_otp_data));
	...
	gc5035sub_otp_read_group(GC5035SUB_OTP_ID_DATA_OFFSET,
			&gc5035sub_otp_data.otp_id[0], GC5035SUB_OTP_ID_SIZE);
	moduleID = gc5035sub_otp_read_sensor_info();
...
static void gc5035sub_otp_function(void)
{
	kal_uint8 i = 0, flag = 0;
	kal_uint8 otp_id[GC5035SUB_OTP_ID_SIZE];

	memset(&otp_id, 0, GC5035SUB_OTP_ID_SIZE);
	...
	gc5035sub_otp_read_group(GC5035SUB_OTP_ID_DATA_OFFSET,
			&otp_id[0], GC5035SUB_OTP_ID_SIZE);
	for (i = 0; i < GC5035SUB_OTP_ID_SIZE; i++)
		if (otp_id[i] != gc5035sub_otp_data.otp_id[i]) {
			flag = 1;
			break;
		}
	...
	if (flag == 1) {
		...
		memset(&gc5035sub_otp_data, 0, sizeof(gc5035sub_otp_data));
		for (i = 0; i < GC5035SUB_OTP_ID_SIZE; i++)
			gc5035sub_otp_data.otp_id[i] = otp_id[i];
		gc5035sub_otp_read_sensor_info();
	}

	gc5035sub_otp_update();
...
static kal_uint16 gain2reg(const kal_uint16 gain)
{
	...
	if (reg_gain < GC5035SUB_SENSOR_GAIN_BASE)
		reg_gain = GC5035SUB_SENSOR_GAIN_BASE;
	else if (reg_gain > GC5035SUB_SENSOR_GAIN_MAX)
		reg_gain = GC5035SUB_SENSOR_GAIN_MAX;
...
static kal_uint16 set_gain(kal_uint16 gain)
{
	...
	kal_uint16 GC5035SUB_AGC_Param[GC5035SUB_SENSOR_GAIN_MAP_SIZE][2] = {
		{  256,  0 },
		{  302,  1 },
		{  358,  2 },
		{  425,  3 },
	...
	for (gain_index = GC5035SUB_SENSOR_GAIN_MAX_VALID_INDEX - 1;
			gain_index > 0; gain_index--)
		if (reg_gain >= GC5035SUB_AGC_Param[gain_index][0])
			break;

	write_cmos_sensor(0xfe, 0x00);
	write_cmos_sensor(0xb6, GC5035SUB_AGC_Param[gain_index][1]);
	temp_gain = reg_gain * Dgain_ratio / GC5035SUB_AGC_Param[gain_index][0];
	write_cmos_sensor(0xb1, (temp_gain >> 8) & 0x0f);
	write_cmos_sensor(0xb2, temp_gain & 0xfc);
	cam_pr_debug("Exit! GC5035SUB_AGC_Param[gain_index][1] = 0x%x, temp_gain = 0x%x, reg_gain = %d\n",
		GC5035SUB_AGC_Param[gain_index][1], temp_gain, reg_gain);
...
static void sensor_init(void)
{
	...
	write_cmos_sensor(0x17, GC5035SUB_MIRROR);
	...
	write_cmos_sensor(0x54, GC5035SUB_RSTDUMMY1);
	...
	write_cmos_sensor(0x22, GC5035SUB_RSTDUMMY2);
		...
		do {
			*sensor_id = return_sensor_id();
			if (*sensor_id == imgsensor_info.sensor_id) {
				...
				printk("5035sub cam :  sub i2c write id: 0x%x, *sensor_id: 0x%x imgsensor_info.sensor_id : 0x%x  \n",
					imgsensor.i2c_write_id, *sensor_id,imgsensor_info.sensor_id);
				return ERROR_NONE;
			}
			printk("5035sub cam : sub Read sensor id fail, write id: 0x%x, id: 0x%x\n",
...
static kal_uint32 open(void)
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			sensor_id = return_sensor_id();
			if (sensor_id == imgsensor_info.sensor_id) {
				printk("5035sub cam : i2c write id: 0x%x, sensor id: 0x%x\n",
					imgsensor.i2c_write_id, sensor_id);
				break;
			}
			printk("5035sub cam :Read sensor id fail, write id: 0x%x, id: 0x%x\n",
				imgsensor.i2c_write_id, sensor_id);
			retry--;
		} while (retry > 0);
		i++;
...
UINT32 GC5035SUB_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
	cam_pr_debug("E\n");
