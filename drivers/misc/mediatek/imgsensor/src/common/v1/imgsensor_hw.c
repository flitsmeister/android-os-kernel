...
#ifdef CONFIG_MID_VLDO28_OP_LP
extern int vldo28_op_lp(void);
#endif
	...
	enum IMGSENSOR_HW_POWER_STATUS pwr_status)
{
	...
	printk(
		"cam sensor_idx %d, power %d curr_sensor_name %s, enable list %s\n",
		sensor_idx,
		pwr_status,
		curr_sensor_name,
	...
	if (ret == 0) {
		printk("Error! snprintf allocate 0");
		ret = IMGSENSOR_RETURN_ERROR;
		return ret;
	}
#endif
	if (sensor_idx==0)//IMGSENSOR_SENSOR_IDX_MAIN
	{
		if(0 == strcmp("gc5035sub_mipi_raw",curr_sensor_name)){
					return IMGSENSOR_RETURN_ERROR;}
		if(0 == strcmp("gc030a_mipi_raw",curr_sensor_name)){
					return IMGSENSOR_RETURN_ERROR;}	
		if(0 == strcmp("gc05a2sub_mipi_raw",curr_sensor_name)){
					return IMGSENSOR_RETURN_ERROR;}
	}
	else if (sensor_idx==1)//IMGSENSOR_SENSOR_IDX_SUB
	{
		if(0 == strcmp("gc5035_mipi_raw",curr_sensor_name)){
					return IMGSENSOR_RETURN_ERROR;}
		if(0 == strcmp("gc05a2_mipi_raw",curr_sensor_name)){
					return IMGSENSOR_RETURN_ERROR;}
	}

	printk(
	    "cam sensor_idx %d, power %d curr_sensor_name %s\n",
	    sensor_idx,
	    pwr_status,
	    curr_sensor_name);
