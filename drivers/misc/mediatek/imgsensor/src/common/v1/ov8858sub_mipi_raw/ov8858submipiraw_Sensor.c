 ...
 *	PengtaoFan
 *  0528:????¡±1get_info,??mipi ??? *  0604:?¡é?init preview ????¡ª???0ms
 *  0604:??-init setting???stream on,?otp??¨¨??
 *  0608:??¡ãov???setting
 *  0703:for  ????¡±1?non continue mode 4800
...
static struct imgsensor_info_struct imgsensor_info = {
	...
	.checksum_value = 0x4139e88b,//0xc2ded17b, /* checksum value for Camera Auto Test */
	...
	.pre = {
		...
		.mipi_data_lp2hs_settle_dc = 85,//14,//23,
	...
	.hs_video = {
		.pclk = 297600000,				//record different mode's pclk
...
#endif
#if 0
static kal_uint16 ov8858sub_table_write_cmos_sensor(kal_uint16 *para, kal_uint32 len)
					
{
	...
	while (len > IDX) {
		...
		if ((I2C_BUFFER_LEN - tosend) < 3 ||len == IDX ||addr != addr_last) {
...
static void r1asensor_init(void)
	{
		//LOG_INF("E\n");
	//	 ;-----------------------------------------
	//	 @@ OV8858SUBR1A Key 1632x1224_30FPS_MIPI_2LANE(Binning)
...
{
	//LOG_INF("E\n");
//  @@ OV8858SUBR2A Initial 1632x1224_30FPS_MIPI_2LANE(Binning)
//  100 99 1632 1224 ; Resolution
//  102 80 1
//  102 3601 bb8;30fps
//  102 40 0 ; HDR Mode off
...
//  ;;
	...
	write_cmos_sensor(0x5000, 0x7e);//0xfe);//modify by sam 7e);
	...
	if (ov8858subversion == OV8858SUBR1A){
		//LOG_INF("inter r1asensor_init() ov8858subversion = %d(0=r2a,1=r1a)\n",ov8858subversion); 
		r1asensor_init();
	}
...
static void hs_video_setting(void)
	{
		//LOG_INF("E\n");
	//	@@ OV8858SUBR2A VGA 120fps_PCLK=148.8M
	//	100 99 640 480
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
	  ...
	  pr_err("ov8858sub i2c write id: 0x%x, id: 0x%x, version = %d(0=r2a,1=r1a)\n",
			...
			if (0) {
				...
				imgsensor_dpcm_info_ov8858sub[1] = COMP8_DI_2A; }

			return ERROR_NONE;
			} else if ((read_cmos_sensor(0x302A)) == 0XB1) {
			ov8858subversion = OV8858SUBR1A;
				pr_err("=R1A sensor,contact OV please!=\n");

					return ERROR_NONE;
		}
				else{
				//printk("read ov8858sub R1A R2A bate fail\n");	  
				return ERROR_SENSOR_CONNECT_FAIL;
				}

		}

		pr_err("ov8858sub Read sensor id fail, id: 0x%x,sensor_id=0x%x\n",
...
static kal_uint32 open(void)
{
	...
	sensor_init();
		set_mirror_flip(imgsensor.mirror);//wangqiang 20141211
#if 0//def 0 //OV8858SUBR2AOTP
