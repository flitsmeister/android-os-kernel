...
static kal_uint16 read_cmos_sensor(kal_uint32 addr)
{
	  kal_uint16 get_byte=0;
	   ...
	   printk("cam log : s5k4h8mipi_Sensor.c 1111 %s %d \n",__FUNCTION__,__LINE__);   
...
static void ihdr_write_shutter_gain(kal_uint16 le, kal_uint16 se, kal_uint16 gain)
{
  ...
  //  kal_uint8 iReg;
    ...
    printk("cam log : s5k4h8mipi_Sensor.c 1111 %s %d \n",__FUNCTION__,__LINE__);
	se = le / 4;   // 3¡è??1a¨º¡À??¨º??¨¬??1a¨º¡À??8¡À?
#if 1    
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id) 
{
    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
    	    printk("cam log : s5k4h8mipi_Sensor.c 2222 %s %d \n",__FUNCTION__,__LINE__);
        spin_lock(&imgsensor_drv_lock);
        imgsensor.i2c_write_id = imgsensor_info.i2c_addr_table[i];
        spin_unlock(&imgsensor_drv_lock);
        printk("cam log : s5k4h8mipi_Sensor.c 3333 %s %d \n",__FUNCTION__,__LINE__);
        do {
			write_cmos_sensor(0x602C,0x4000);
			write_cmos_sensor(0x602E,0x0000);
			*sensor_id = read_cmos_sensor(0x6F12);
			  printk("cam log : s5k4h8mipi_Sensor.c 4444 %s %d \n",__FUNCTION__,__LINE__);
			 printk("cam: [s5k4h8mipi_Sensor]111 write id: 0x%x, sensor id: 0x%x imgsensor_info.sensor_id 0x%x \n", imgsensor.i2c_write_id,*sensor_id,imgsensor_info.sensor_id);      
			//*sensor_id = imgsensor_info.sensor_id;
            if (*sensor_id == imgsensor_info.sensor_id) {               
                printk("cam: [s5k4h8mipi_Sensor] 2222 write id: 0x%x, sensor id: 0x%x imgsensor_info.sensor_id 0x%x \n", imgsensor.i2c_write_id,*sensor_id,imgsensor_info.sensor_id);      
                return ERROR_NONE;
            }   
              printk("cam log : s5k4h8mipi_Sensor.c 5555 %s %d \n",__FUNCTION__,__LINE__);
            printk("cam: [s5k4h8mipi_Sensor]: Read sensor id fail, write id: 0x%x, sensor id = 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
            retry--;
        } while(retry > 0);
        i++;
        retry = 2;
    ...
    if (*sensor_id != imgsensor_info.sensor_id) {
        ...
        *sensor_id = 0xFFFFFFFF;
        return ERROR_SENSOR_CONNECT_FAIL;
            ...
            printk("cam log : s5k4h8mipi_Sensor.c 555 %s %d \n",__FUNCTION__,__LINE__);
    return ERROR_NONE;
}
...
static kal_uint32 open(void)
{
	...
	printk("cam log : s5k4h8mipi_Sensor.c 1111 %s %d \n",__FUNCTION__,__LINE__);
        ...
        do {
            ...
            sensor_id =  read_cmos_sensor(0x6F12);
			//sensor_id = imgsensor_info.sensor_id;
			 printk("cam: [s5k4h8mipi_Sensor] 11 i2c write id: 0x%x, sensor id: 0x%x imgsensor_info.sensor_id 0x%x  \n", imgsensor.i2c_write_id,sensor_id,imgsensor_info.sensor_id);   
            if (sensor_id == imgsensor_info.sensor_id) {                
                printk("cam: [s5k4h8mipi_Sensor] 22 i2c write id: 0x%x, sensor id: 0x%x imgsensor_info.sensor_id 0x%x  \n", imgsensor.i2c_write_id,sensor_id,imgsensor_info.sensor_id);   
                break;
            }   
            printk("cam: [s5k4h8mipi_Sensor] 33 Read sensor id fail, id: 0x%x\n", imgsensor.i2c_write_id,sensor_id);
            retry--;
        } while(retry > 0);
        i++;
        if (sensor_id == imgsensor_info.sensor_id)
