...
//xuel added isp5.0 <
	...
	.mipi_settle_delay_mode = MIPI_SETTLEDELAY_AUTO,	/* 0,MIPI_SETTLEDELAY_AUTO;
								 ...
								 */
	.sensor_output_dataformat = SENSOR_OUTPUT_FORMAT_RAW_Gb,//Gb R B
	.mclk = 24,
...
//    otp_lsc_update();
    ...
    msleep(5);//vivo zcw++ 20141027 Add for prevent WAIT_IRQ timeout    
    LOG_INF("Exit\n");
  //s 2014/12/01, bruce
		...
		do {
			...
			*sensor_id = read_cmos_sensor(0x6F12);
			...
			if (*sensor_id == imgsensor_info.sensor_id) {
				printk("xuchen s5k3m2 i2c write id: 0x%x, sensor id: 0x%x\n",
					imgsensor.i2c_write_id, *sensor_id);
				return ERROR_NONE;
			}
			printk("xuchen s5k3m2 Read sensor id fail, write id: 0x%x, sensor id = 0x%x\n",
				imgsensor.i2c_write_id, *sensor_id);
			retry--;
		} while (retry > 0);
		i++;
...
static kal_uint32 open(void)
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			...
			printk("xuchen s5k3m2 Read sensor id fail write_id: 0x%x, id: 0x%x\n",
				imgsensor.i2c_write_id, sensor_id);
			retry--;
		} while (retry > 0);
		i++;
