    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
        ...
        do {
            *sensor_id = return_sensor_id();
            printk("1111cam [Kernel/ov13850mipiraw_Sensor]: %s %d imgsensor_info.sensor_id: 0x%x \n", __func__,__LINE__,imgsensor_info.sensor_id);
            printk("2222cam [Kernel/ov13850mipiraw_Sensor]: %s %d i2c write id: 0x%x, sensor id: 0x%x \n", __func__,__LINE__,imgsensor.i2c_write_id,*sensor_id);
            if (*sensor_id == imgsensor_info.sensor_id) {
                printk("i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
                printk( "333cam [Kernel/ov13850mipiraw_Sensor]: i2c_write_id=0x%x, sensor_id=0x%x\n", imgsensor.i2c_write_id, *sensor_id );
                printk( "444cam [Kernel/ov13850mipiraw_Sensor]: << get_imgsensor_id(): OK\n" );
                return ERROR_NONE;
            }
            printk("Read sensor id fail, write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
            printk( "555cam [Kernel/ov13850mipiraw_Sensor]: Read sensor id fail, write_id=0x%x, sensor_id=0x%x\n", imgsensor.i2c_write_id, *sensor_id );
            retry--;
        } while(retry > 0);
        i++;
