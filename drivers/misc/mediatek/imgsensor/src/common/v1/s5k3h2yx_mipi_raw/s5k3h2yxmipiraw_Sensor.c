...
#define PFX "s5k3h2yx_camera_sensor"
#define LOG_1 LOG_INF("s5k3h2yx,MIPI 4LANE\n")
...
static struct imgsensor_info_struct imgsensor_info = {
	...
	.i2c_addr_table = {0x20, 0x5A, 0x6C, 0x6E, 0xff},
...
static kal_uint16 set_gain(kal_uint16 gain)
{
	...
	spin_unlock(&imgsensor_drv_lock);
	pr_info("gain = %d , reg_gain = 0x%x,shutter=%d,the result of gain*shutter is %d ", gain, reg_gain, imgsensor.shutter, gain * (imgsensor.shutter));
...
static void ihdr_write_shutter_gain(kal_uint16 le, kal_uint16 se, kal_uint16 gain)
{
	...
	if (imgsensor.ihdr_en)
	{
		...
		write_cmos_sensor_8(0x0104, 0x00);
		pr_info("iHDR:imgsensor.frame_length=%d\n", imgsensor.frame_length);
		set_gain(gain);
	}
...
{	
	   ...
	   write_cmos_sensor_8(0x0700, 0x05);	//fifo_water_mark_pixels = 1328 		   
	   write_cmos_sensor_8(0x0701, 0x30);				   
	   write_cmos_sensor_8(0x034C, 0x0C);	   //x_output_size = 3264			   
	   write_cmos_sensor_8(0x034D, 0xD0);				   
	   write_cmos_sensor_8(0x034E, 0x09);	//y_output_size = 2448			   
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff)
	{
		...
		do
		{
			*sensor_id = return_sensor_id();
			...
			{
				printk("s5k3h2yxmipiraw read id: 0x%x, sensor id: 0x%x\n", imgsensor_info.sensor_id, *sensor_id);

				printk("s5k3h2yxmipiraw read id success : 0x%x\n", *sensor_id);
				return ERROR_NONE;
			}
			printk("s5k3h2yxmipiraw Read sensor id fail 0x%x, id: 0x%x\n", imgsensor.i2c_write_id, *sensor_id);
			retry--;
		} while (retry > 0);
		i++;
		retry = 2;
	...
	if (*sensor_id != imgsensor_info.sensor_id)
	{
		...
		*sensor_id = 0xFFFFFFFF;
		LOG_INF("s5k3h2yxmipiraw read id fail : 0x%x\n", *sensor_id);
						  ...
						  MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	else //PIP capture: 24fps for less than 13M, 20fps for 16M,15fps for 20M
	{
		if (imgsensor.current_fps != imgsensor_info.cap1.max_framerate)
			LOG_INF("Warning: current_fps %d fps is not support, so use cap1's setting: %d fps!\n", imgsensor_info.cap1.max_framerate / 10, imgsensor.current_fps);
