...
static int vpu_unprepare_regulator_and_clock(int core)
{
	int ret = 0;
#define UNPREPARE_VPU_CLK(clk) \
	{ \
		if (clk != NULL) { \
			clk_unprepare(clk); \
			if (core == 0 && is_power_on[1] == false) { \
				clk = NULL; \
			} else if (core == 1 && is_power_on[0] == false) { \
				clk = NULL; \
			} \
		} \
	}
...
int vpu_shut_down(int core)
{
	...
	mutex_lock(&clc_mutex);
	ret = vpu_disable_regulator_and_clock(core);
	ret = vpu_unprepare_regulator_and_clock(core);
	mutex_unlock(&clc_mutex);
	if (ret) {
		LOG_ERR("[vpu_%d]fail to disable regulator and clock\n",
		core);
		goto out;
