...
static int icm206xx_accel_init_client(struct i2c_client *client, bool enable)
{
	...
	res = icm206xx_share_ReadChipInfo(strbuf, ICM206XX_BUFSIZE);
	if (res != ICM206XX_SUCCESS)
		return res;
...
static ssize_t orientation_store(struct device_driver *ddri, const char *buf, size_t tCount)
{
	int nDirection = 0;
	int res = 0;
	struct icm206xx_accel_i2c_data *obj = obj_i2c_data;
...
*/
#ifdef CONFIG_MID_ITEMS_SUPPORT
int icm206xxa_direction_calculate(int bma)
{
    int direction = 0;
    switch(bma){
        case 0:
...
static int icm206xx_accel_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
...
#ifdef CONFIG_MID_ITEMS_SUPPORT
	//if(!(item_integer("sensor.gyroscope.exist",0)))
	//	goto exit;
	orient = item_integer("sensor.accelerometer.bma.orientation",0);
	orient = orient < 0 ? 0 : orient;
	obj->hw.direction = icm206xxa_direction_calculate(orient);
	printk("icm206xx_gsensor_direction bma=%d,icm206xx=%d\n",orient,obj->hw.direction);
#endif
	...
	//printk("xuchen client->addr=0x%x\n",client->addr);
	client->addr = 0x68; //if ADO/SD0=0 addr=0X68 else addr=0X69
