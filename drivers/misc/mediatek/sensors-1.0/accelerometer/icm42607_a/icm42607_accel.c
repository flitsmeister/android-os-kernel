...
static int icm42607_accel_SetFullScale(struct i2c_client *client, u8 accel_fsr)
{
	...
	ACC_LOG("g_icm42607_accel_sensitivity %d ,write %x\n",g_icm42607_accel_sensitivity,databuf[0]);
#ifdef DEBUG
	res = icm42607_share_read_register(REG_ACCEL_CONFIG0, databuf, 1);
    if (res < 0) {
        ACC_PR_ERR("read fsr register err!\n");
    ...
    char *buf, int bufsize)
{
	...
	data[obj->cvt.map[ICM42607_AXIS_Y]] = obj->cvt.sign[ICM42607_AXIS_Y] * temp_buff[ICM42607_AXIS_Y];		data[obj->cvt.map[ICM42607_AXIS_Z]] = obj->cvt.sign[ICM42607_AXIS_Z] * temp_buff[ICM42607_AXIS_Z];
...
static int icm42607_accel_init_client(struct i2c_client *client, bool enable)
{
    ...
    if (res < 0) {
        ACC_PR_ERR("read REG_INT_CONFIG register err!\n");
        return ICM42607_ERR_BUS;
    }
    ...
    if (res != ICM42607_SUCCESS)
        return res;
    ACC_LOG("icm42607_accel_init_client OK!\n");
    return ICM42607_SUCCESS;
}
...
static ssize_t orientation_show(struct device_driver *ddri, char *buf)
{
   // ssize_t len = 0;
	...
	return sprintf(buf, "(%d, %d)\n[%+2d %+2d %+2d]\n[%+2d %+2d %+2d]\n",
                obj->hw->direction,atomic_read(&obj->layout), obj->cvt.sign[0], obj->cvt.sign[1],
                obj->cvt.sign[2],obj->cvt.map[0], obj->cvt.map[1], obj->cvt.map[2]);
    ...
    const char *buf, size_t tCount)
{
	...
	*/
	atomic_set(&obj->layout, nDirection);
	 
	if(!hwmsen_get_convert(nDirection, &obj->cvt))
	{
			pr_info("HWMSEN_GET_CONVERT function error!\r\n");
	}
	else if(!hwmsen_get_convert(obj->hw->direction, &obj->cvt))
	{
			pr_info("invalid layout: %d, restore to %d\n", nDirection, obj->hw->direction);
	}
	else
	{
			pr_info("invalid layout: (%d, %d)\n", nDirection, obj->hw->direction);
			hwmsen_get_convert(0, &obj->cvt);
	}
...
{
	struct i2c_client *client = icm42607_accel_i2c_client;
	struct icm42607_accel_i2c_data *data = i2c_get_clientdata(client);
...
static ssize_t layout_store(struct device_driver *ddri, const char *buf, size_t count)
{
	...
	{
			pr_info("invalid format = '%s'\n", buf);
	}

	return count;
}
#endif
static ssize_t regdump_show(struct device_driver *ddri, char *buf)
{
    ssize_t len = 0;
    struct icm42607_accel_i2c_data *obj = obj_i2c_data;
    int i;
...
static int icm42607_accel_factory_get_cali(int32_t data[3])
{
	...
	ACC_LOG();
        ...
        if (err) {
                ACC_LOG("icm42607_accel_ReadCalibration failed!\n");
                return -1;
        }
...
static int icm42607_accel_factory_set_cali(int32_t data[3])
{
	...
	struct SENSOR_DATA  sensor_data;
        ...
        if (err) {
               ACC_LOG("icm42607_accel_WriteCalibration failed!\n");
                return -1;
        }
        return 0;
}
...
static int icm42607_accel_enable_nodata(int en)
{
    ...
    ACC_LOG("icm42607_accel_enable_nodata OK!\n");
    return 0;
}
 ...
 *****************************************************************************/
/**
* calculate icm42607 direction according to bma direction
*  add by mid
*/
#ifdef CONFIG_MID_ITEMS_SUPPORT
int icm42607a_direction_calculate(int bma)
{
    int direction = 0;
    switch(bma){
        case 0:
    ...
    const struct i2c_device_id *id)
{
	...
	obj->hw->direction = icm42607a_direction_calculate(orient);
	printk("icm42607_gsensor_direction bma=%d,icm42607=%d\n",orient,obj->hw->direction);
#endif
    ...
    if (res) {
        ACC_PR_ERR("icm42607_accel_device misc register failed!\n");
	...
	res = accel_factory_device_register(&icm42607_accel_factory_device);
        ...
        if (res) {
                ACC_LOG("icm42607_accel_factory_device register failed!\n");
                goto exit_misc_device_register_failed;
        }
    ...
    if (res) {
        ACC_PR_ERR("icm42607_accel create attribute err = %d\n", res);
...
static int icm42607_accel_i2c_remove(struct i2c_client *client)
{
    ...
    res = icm42607_accel_delete_attr(
        &(icm42607_accel_init_info.platform_diver_addr->driver));
    if (res)
        ACC_PR_ERR("icm42607_accel_delete_attr fail: %d\n", res);
...
static int icm42607_accel_i2c_suspend(struct device *dev)
{
    ...
    struct icm42607_accel_i2c_data *obj = i2c_get_clientdata(client);
        ...
        ACC_LOG("icm42607_accel suspend ok\n");
    return res;
}
...
static int icm42607_accel_i2c_resume(struct device *dev)
{
    ...
    atomic_set(&obj->suspend, 0);
    ACC_LOG("icm42607_accel resume ok\n");
    return 0;
}
