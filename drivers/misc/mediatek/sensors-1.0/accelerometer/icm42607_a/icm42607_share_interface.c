...
int icm42607_switch_on_mclk(void)
{
	...
	if (need_mclk_cnt == 0) {
    	...
    	do {
	    	...
	    	if (res < 0) {
	        	ACC_PR_ERR("Read REG_MCLK_RDY failed: %d\n", res);
	        	return ICM42607_ERR_BUS;
	    	}
    	} while (!(regdata & 0x08));
	} else {
    	...
    	if (!(regdata & 0x10)) {
        	...
        	//ACC_PR_ERR("mismatch status in icm42607_switch_on_mclk!! %d count %d",regdata,need_mclk_cnt);
    	}
	}
...
int icm42607_share_read_blkreg(u8 blk, u8 addr, u8 *data, u8 len)
{
	...
	if (res < 0) {
        ACC_PR_ERR("icm42607_switch_on_mclk err!\n");
        return res;
    }
    ...
    if (res < 0) {
        ACC_PR_ERR("write REG_BLK_SEL_R 0x%0x err!\n", blk);
        return ICM42607_ERR_BUS;
    }
    ...
    if (res < 0) {
    	ACC_PR_ERR("write REG_MADDR_R 0x%0x: %d\n", addr, res);
    	return ICM42607_ERR_BUS;
    }

    res = icm42607_share_read_register(REG_M_R, data, len);
    if (res < 0) {
        ACC_PR_ERR("Read REG_M_R 0x%0x failed: %d\n", addr, res);
        return ICM42607_ERR_BUS;
    }
    ...
    res = icm42607_share_write_register(REG_BLK_SEL_R, databuf, 1);
    if (res < 0) {
        ACC_PR_ERR("write REG_BLK_SEL_R 0x00 err!\n");
        return ICM42607_ERR_BUS;
    }
	...
	if (res < 0) {
        ACC_PR_ERR("icm42607_switch_off_mclk err!\n");
    }
...
int icm42607_share_write_blkreg(u8 blk, u8 addr, u8 *data, u8 len)
{
    ...
    if (res < 0) {
        ACC_PR_ERR("write REG_BLK_SEL_W 0x%0x err!\n", blk);
        return ICM42607_ERR_BUS;
    }
    ...
    if (res < 0) {
    	ACC_PR_ERR("write REG_MADDR_W 0x%0x: %d\n", addr, res);
    	return ICM42607_ERR_BUS;
    }

    res = icm42607_share_write_register(REG_M_W, data, len);
    if (res < 0) {
        ACC_PR_ERR("Read REG_M_W 0x%0x failed: %d\n", addr, res);
        return ICM42607_ERR_BUS;
    }
    ...
    res = icm42607_share_write_register(REG_BLK_SEL_W, databuf, 1);
    if (res < 0) {
        ACC_PR_ERR("write REG_BLK_SEL_W 0x00 err!\n");
        return ICM42607_ERR_BUS;
    }
