...
static int icm42607_i2c_driver_setting(void)
{
	...
	res = icm42607_share_read_register(REG_INTF_CONFIG1, databuf, 1);
   	if (res) {
    	ACC_PR_ERR("Read REG_INTF_CONFIG1: %d\n", res);
    }
    ...
    if (res) {
      	ACC_PR_ERR("write REG_INTF_CONFIG1: %d\n", res);
    }
	ACC_LOG("write REG_INTF_CONFIG1 %x!\n", databuf[0]);

	databuf[0] |= i2c_slew_rate <<3 | spi_slew_rate;
	res = icm42607_share_write_register(REG_DRIVE_CONFIG2, databuf, 1);
    if (res) {
      	ACC_PR_ERR("write REG_DRIVE_CONFIG2: %d\n", res);
    }
	ACC_LOG("write REG_DRIVE_CONFIG2 %x!\n", databuf[0]);

	databuf[0] |= spi_slew_rate;
	res = icm42607_share_write_register(REG_DRIVE_CONFIG3, databuf, 1);
    if (res) {
      	ACC_PR_ERR("write REG_DRIVE_CONFIG3: %d\n", res);
    }
	ACC_LOG("write REG_DRIVE_CONFIG3 %x!\n", databuf[0]);
...
static int icm42607_EnableInterrupt(u8 int_type, bool enable)
{
    ...
    u8 databuf[2] = {0};
	u8 intsource[2] = {0};
	ACC_LOG("icm42607_EnableInterrupt type %d en %d!\n", int_type,enable);
    ...
    if(ICM42607_INT_TYPE_STD == int_type) {
		...
		if (res < 0) {
			ACC_PR_ERR("read REG_INT_SOURCE1 err!\n");
			return ICM42607_ERR_BUS;
		}
    	...
    	if (res < 0) {
			ACC_PR_ERR("write REG_INT_SOURCE1 err!\n");
			return ICM42607_ERR_BUS;
    	}
