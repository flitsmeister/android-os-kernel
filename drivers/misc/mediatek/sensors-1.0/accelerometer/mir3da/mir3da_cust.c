...
static int mir3da_writeCalibration(struct i2c_client *client, int dat[MIR3DA_AXES_NUM])
{
    ...
    else
    {
        ...
        obj->cali_sw[MIR3DA_AXIS_X] = cali[MIR3DA_AXIS_X];//obj->cvt.sign[MIR3DA_AXIS_X]*cali[obj->cvt.map[MIR3DA_AXIS_X]];
        obj->cali_sw[MIR3DA_AXIS_Y] = cali[MIR3DA_AXIS_Y];//obj->cvt.sign[MIR3DA_AXIS_Y]*cali[obj->cvt.map[MIR3DA_AXIS_Y]];
        obj->cali_sw[MIR3DA_AXIS_Z] = cali[MIR3DA_AXIS_Z];//obj->cvt.sign[MIR3DA_AXIS_Z]*cali[obj->cvt.map[MIR3DA_AXIS_Z]];
...
/*----------------------------------------------------------------------------*/
static ssize_t axis_data_show(struct device_driver *ddri, char *buf)
{
    int result;
    short x,y,z;
    int count = 0;
...
/*----------------------------------------------------------------------------*/
static ssize_t reg_data_store(struct device_driver *ddri, const char *buf, size_t count)
{
    int                 addr, data;
    int                 result;
...
#if MIR3DA_OFFSET_TEMP_SOLUTION
/*----------------------------------------------------------------------------*/
static ssize_t calibrate_show(struct device_driver *ddri, char *buf)
{
    int ret;       
...
/*----------------------------------------------------------------------------*/
static ssize_t calibrate_store(struct device_driver *ddri, const char *buf, size_t count)
{
    signed char     z_dir = 0;
...
static ssize_t average_enhance_show(struct device_driver *ddri, char *buf)
{
    int       ret = 0;
...
/*----------------------------------------------------------------------------*/
static ssize_t average_enhance_store(struct device_driver *ddri, const char *buf, size_t count)
{ 
    int       ret = 0;
    struct mir3da_filter_param_s    param = {0};
...
static DRIVER_ATTR_RO(axis_data);
...
#if MIR3DA_OFFSET_TEMP_SOLUTION
static DRIVER_ATTR_RW(calibrate_miraGSensor);
#endif
#if FILTER_AVERAGE_ENHANCE
static DRIVER_ATTR_RW(average_enhance);
#endif 
/*----------------------------------------------------------------------------*/
...
static int mir3da_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	*/
    ...
    printk("mir3da: direction bma=%d, mir3da=%d\n", bma, obj->hw.direction);
