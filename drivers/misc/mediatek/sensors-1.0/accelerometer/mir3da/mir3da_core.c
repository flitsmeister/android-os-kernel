...
static unsigned char is_223 = -1; 
static unsigned char is_218 = -1; 
...
int mir3da_parse_chip_info(PLAT_HANDLE handle){
    unsigned char i=0,tmp=0,tmp_reg = 0;   
  ...
  gsensor_chip_info.reg_value    = reg_value;               
	...
	res = mir3da_register_read(handle, NSA_REG_CHIP_INFO, &tmp_reg);
      if(res != 0) {
          return res;
      }
	if((gsensor_chip_info.asic == ASIC_2513A) || (gsensor_chip_info.asic == ASIC_2516))
	{
		if((tmp_reg == 0x4E) ||(tmp_reg == 0x5D))
			is_223 = 1;
		else if((tmp_reg == 0x48) || (tmp_reg == 0x59))
			is_218 = 1;
	}	
#if YZ_CROSS_TALK_ENABLE 
	if(1 != is_218){
...
#if MIR3DA_SUPPORT_FAST_AUTO_CALI	
    if((mir3da_gsensor_drv.method->support_fast_auto_cali !=NULL) &&(mir3da_gsensor_drv.method->support_fast_auto_cali()))
...
int mir3da_temp_calibrate(int *x,int *y, int *z)
{
    ...
    *z = *z+z_offset;
    ...
    {		
        is_static = mir3da_temp_calibrate_detect_static(*x,*y,*z-z_offset); 
        tem_z = squareRoot(MIR3DA_OFFSET_SEN*MIR3DA_OFFSET_SEN - (*x)*(*x) - (*y)*(*y)) + lz_offset;
        if(z_offset==0)
        {			  
            if(is_static == 1)
            {
	...
	else if((abs((abs(*x)-MIR3DA_OFFSET_SEN))<MIR3DA_OFFSET_MAX)&&(abs(*y)<MIR3DA_OFFSET_MAX)&&(z_offset))
	{
		if(abs(*x)>MIR3DA_OFFSET_SEN)
		{
		    *x = (*x>0) ? (*x-(abs(*x)-MIR3DA_OFFSET_SEN)*cus/MIR3DA_OFFSET_MAX):(*x+(abs(*x)-MIR3DA_OFFSET_SEN)*cus/MIR3DA_OFFSET_MAX);
		}
		else
		{
		    *x = (*x>0) ? (*x+(MIR3DA_OFFSET_SEN-abs(*x))*cus/MIR3DA_OFFSET_MAX):(*x-(MIR3DA_OFFSET_SEN-abs(*x))*cus/MIR3DA_OFFSET_MAX);
		}
		*y = (*y)*MIR3DA_OFFSET_CUS/MIR3DA_OFFSET_MAX;
	}
	else if((abs((abs(*y)-MIR3DA_OFFSET_SEN))<MIR3DA_OFFSET_MAX)&&(abs(*x)<MIR3DA_OFFSET_MAX)&&(z_offset))
	{
		if(abs(*y)>MIR3DA_OFFSET_SEN)
		{
	   		*y = (*y>0) ? (*y-(abs(*y)-MIR3DA_OFFSET_SEN)*cus/MIR3DA_OFFSET_MAX):(*y+(abs(*y)-MIR3DA_OFFSET_SEN)*cus/MIR3DA_OFFSET_MAX);
		}
		else
		{
			*y = (*y>0) ? (*y+(MIR3DA_OFFSET_SEN-abs(*y))*cus/MIR3DA_OFFSET_MAX):(*y-(MIR3DA_OFFSET_SEN-abs(*y))*cus/MIR3DA_OFFSET_MAX);
		}
		*x = (*x)*MIR3DA_OFFSET_CUS/MIR3DA_OFFSET_MAX;		
	}
	else if(z_offset==0)
	{
	    if((abs(*x)<MIR3DA_OFFSET_MAX)&&(abs((*y > 0) ? (MIR3DA_OFFSET_SEN-*y):(MIR3DA_OFFSET_SEN+*y))<MIR3DA_OFFSET_MAX))
	    {
			*z = ((*z>=0)?(1):(-1))*abs(*x)*MIR3DA_OFFSET_CUS/MIR3DA_OFFSET_MAX;
	    }
		else if((abs(*y)<MIR3DA_OFFSET_MAX)&&(abs((*x > 0) ? (MIR3DA_OFFSET_SEN-*x):(MIR3DA_OFFSET_SEN+*x))<MIR3DA_OFFSET_MAX))
		{
