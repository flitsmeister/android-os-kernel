...
static ssize_t daemon_show(struct device_driver *ddri, char *buf)
{
	char strbuf[MMC5603NJ_BUFSIZE];
	sprintf(strbuf, "memsic");
...
{
	/* get raw data */
	return mmc5603nj_get_data(&data[0], &data[1], &data[2], status);
}
...
static int mmc5603nj_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	printk("-------dsy  %s %d  client->addr =0x%02x ------- \n", __func__,__LINE__,client->addr);
	client->addr = 0x30;
