...
#define STK3X3X_DRIVER_VERSION          "v3.x.3 for android9.x"
...
*******************************************************************************/
#ifdef STK_TUNE0
#define STK_MAX_MIN_DIFF		300//30
#define STK_LT_N_CT				180//18
#define STK_HT_N_CT				300//30	 
#endif /* #ifdef STK_TUNE0 */

#define STK_H_PS				10000
#define STK_H_HT				200
#define STK_H_LT				180
...
static const struct dev_pm_ops stk3x3x_pm_ops = {
	SET_SYSTEM_SLEEP_PM_OPS(stk3x3x_i2c_suspend, stk3x3x_i2c_resume)
};
#endif
...
/*chenxinfeng: fix stk3x3x can not show als date  start*/
//static int stk3x3x_get_als_value(struct stk3x3x_priv *obj, u16 als);
/*chenxinfeng: fix stk3x3x can not show als date  end*/
...
int stk3x3x_read_als(struct i2c_client *client, u16 *data)
{
	...
	ret = stk3x3x_master_recv(client, obj->addr.data1_als, buf, 0x2);
	if (ret < 0)
	{
		APS_DBG("error: %d\n", ret);
		return -EFAULT;
...
int stk3x3x_get_ps_high_thd(struct i2c_client *client, u16 *data)
{
    struct stk3x3x_priv *obj = i2c_get_clientdata(client);
    int ret = 0;
    u8 buf[2];
    ...
    ret = stk3x3x_master_recv(client, obj->addr.thdh1_ps, buf, 0x02);
...
int stk3x3x_get_ps_low_thd(struct i2c_client *client, u16 *data)
{
    struct stk3x3x_priv *obj = i2c_get_clientdata(client);
    int ret = 0;
    u8 buf[2];
    ...
    ret = stk3x3x_master_recv(client, obj->addr.thdl1_ps, buf, 0x02);
...
static int show_allreg(void)
{
	...
	for(cnt = 0; cnt < 14; cnt++){
		rbuf = 0;
		ret = stk3x3x_master_recv(stk3x3x_obj->client, reg[cnt], &rbuf, 1);
		if (ret < 0)
		{
			APS_DBG("error: %d\n", ret);
		}else{
...
static int stk3x3x_enable_ps_set_thd(struct stk3x3x_priv *obj)
{
	...
	else if (obj->boot_cali == 1 && obj->ps_low_thd_boot < 6000)
	{
		atomic_set(&obj->ps_high_thd_val, obj->ps_high_thd_boot);
		atomic_set(&obj->ps_low_thd_val, obj->ps_low_thd_boot);
	}
...
static int stk3x3x_ps_val(void)
{
	...
	for(i = 0; i < 4;i++){
		ret = stk3x3x_master_recv(stk3x3x_obj->client, (0x24 + i*2), buf, 2);
		if (ret < 0)
		{
	...
	if((stk3x3x_obj->data_count%10)==3){
...
#else
	obj->ps_high_thd_boot = obj->ps_stat_data[1] + STK_H_HT;
	obj->ps_low_thd_boot = obj->ps_stat_data[1] + STK_H_LT;
	atomic_set(&obj->ps_high_thd_val, obj->ps_high_thd_boot);
	atomic_set(&obj->ps_low_thd_val, obj->ps_low_thd_boot);
#endif
...
static int stk_ps_tune_zero_func_fae(struct stk3x3x_priv *obj)
{
	...
	if(obj->data_count > 50000){
        ...
        ret = stk3x3x_get_ps_high_thd(obj->client, &high_thd);
        if (ret < 0)
        {
        ...
        ret = stk3x3x_get_ps_low_thd(obj->client, &low_thd);
        if (ret < 0)
        {
            APS_DBG("fail, error: %d\n", ret);
            return ret;
	...
	if ((obj->psi_set != 0))
	{
		if (obj->ps_distance_last == 1)
		{
			...
			if (ret == 0)
			{
				...
				if (word_data >= 0)
				{
					...
					{
						obj->tracking_count++;
						APS_LOG("%s: CTTRACKING pre ps=%d, count=%d\n", __func__, word_data, obj->tracking_count);
						if(obj->tracking_count > 3){
		...
		}else if(obj->ps_thd_update == false){
			ret = stk3x3x_ps_val();
			if (ret == 0)
			{
				ret = stk3x3x_master_recv(obj->client, 0x11, buf, 2);
				...
				word_data = (buf[0] << 8) | buf[1];		
				APS_LOG("%s: judge ps=%d\n", __FUNCTION__, word_data);
				if(word_data > (obj->psi + STK_H_PS))
				{
					...
					APS_LOG("%s:smudge ps = 0x%x, psi = 0x%x\n", __FUNCTION__, word_data, obj->psi);
					APS_LOG("%s:smudge HT=%d, LT=%d\n", __func__, atomic_read(&obj->ps_high_thd_val), atomic_read(&obj->ps_low_thd_val));
				}
			}
		}
		return 0;
	...
	if (diff > obj->stk_max_min_diff)
	{
		...
		{
			if ((atomic_read(&obj->ps_high_thd_val) + STK_H_HT) < (obj->ps_high_thd_boot))
			{
...
#endif
		APS_LOG(" kzhkzh %s: ps interrupt, nf_state =%d\n", __FUNCTION__, near_far_state);
		stk_ps_int_handle(obj, near_far_state);
#if 1
		APS_LOG("kzhkzh %s: ps interrupt, show all reg,near_far_state:%d\n", __FUNCTION__,near_far_state);
		show_allreg(); //for debug
#endif
	}
...
int stk3x3x_setup_eint(struct i2c_client *client)
{
	...
	if (stk3x3x_obj->irq_node)
	{
		...
		if (request_irq(stk3x3x_obj->irq, stk3x3x_eint_handler, IRQF_TRIGGER_LOW, "ALS-eint", NULL))
		{
...
static int stk3x3x_init_client(struct i2c_client *client)
{
	...
	*/
	...
	//A GAIN default
	buffer = 0x15;
	stk3x3x_master_send(client, 0xDB, &buffer , 1);

	//PS INT mode
	buffer = 0x01;
	stk3x3x_master_send(client, 0xFA, &buffer, 1);

	//BGIR
	buffer = 0x10;
	stk3x3x_master_send(client, 0xA0, &buffer, 1);
	buffer = 0x64;
	stk3x3x_master_send(client, 0xAA, &buffer, 1);

	//Other
	buffer = 0x0F;
	stk3x3x_master_send(client, 0xA1, &buffer, 1);

	
	buffer = 0xC0;
	stk3x3x_master_send(client, 0xF4, &buffer, 1);
...
/*----------------------------------------------------------------------------*/
static ssize_t alslv_store(struct device_driver *ddri, const char *buf, size_t count)
{
	if (!stk3x3x_obj)
	{
		APS_ERR("stk3x3x_obj is null!!\n");
...
/*----------------------------------------------------------------------------*/
static ssize_t alsval_store(struct device_driver *ddri, const char *buf, size_t count)
{
	if (!stk3x3x_obj)
	{
		APS_ERR("stk3x3x_obj is null!!\n");
...
static ssize_t maxdiff_store(struct device_driver *ddri, const char *buf, size_t count)
{
...
static ssize_t maxdiff_show(struct device_driver *ddri, char *buf)
{
	if (!stk3x3x_obj)
	{
		APS_ERR("stk3x3x_obj is null!!\n");
...
static ssize_t ltnct_store(struct device_driver *ddri, const char *buf, size_t count)
{
	unsigned long value = 0;
	int ret;
...
static ssize_t ltnct_show(struct device_driver *ddri, char *buf)
{
	if (!stk3x3x_obj)
	{
		APS_ERR("stk3x3x_obj is null!!\n");
...
static ssize_t htnct_store(struct device_driver *ddri, const char *buf, size_t count)
{
	unsigned long value = 0;
	int ret;
...
static ssize_t htnct_show(struct device_driver *ddri, char *buf)
{
	if (!stk3x3x_obj)
	{
		APS_ERR("stk3x3x_obj is null!!\n");
...
/*---Offset At-------------------------------------------------------------------------*/
static ssize_t pthredcalibration_show(struct device_driver *ddri, char *buf)
{
	if (!stk3x3x_obj)
	{
		APS_ERR("stk3x3x_obj is null!!\n");
...
/*----------------------------------------------------------------------------*/
static ssize_t pscalibration_store(struct device_driver *ddri, const char *buf, size_t count)
{
	int ret;
...
static ssize_t ps_offset_store(struct device_driver *ddri, const char *buf, size_t count)
{
#ifdef STK_TUNE0
	struct stk3x3x_priv *obj = stk3x3x_obj;
	int ps_cali_val = 0;
...
static DRIVER_ATTR_RW(alslv);
static DRIVER_ATTR_RW(alsval);
...
static DRIVER_ATTR_RW(maxdiff);
static DRIVER_ATTR_RW(ltnct);
static DRIVER_ATTR_RW(htnct);
#endif
...
static int stk3x3x_i2c_suspend(struct device *dev) 
{
	...
	err = stk3x3x_enable_als(obj->client, 0);
	if (err)
...
static int stk3x3x_i2c_resume(struct device *dev)
{
	...
	if (test_bit(STK_BIT_ALS, &obj->enable)) {
		err = stk3x3x_enable_als(obj->client, 1);
		if (err)
...
static int stk3x3x_als_factory_enable_sensor(bool enable_disable, int64_t sample_periods_ms)
{
	int err = 0;
	APS_FUN();
...
static int stk3x3x_als_factory_get_data(int32_t *data)
{
	int status;
...
static int stk3x3x_als_factory_get_raw_data(int32_t *data)
{
	int err = 0;
	struct stk3x3x_priv *obj = stk3x3x_obj;
	...
	err = stk3x3x_read_als(obj->client, &obj->als);
	if (err) {
		APS_ERR("%s failed\n", __func__);
		return -1;
	}
	*data = stk3x3x_obj->als;

	return 0;
}
static int stk3x3x_als_factory_enable_calibration(void)
{
	APS_FUN();
	return 0;
}
static int stk3x3x_als_factory_clear_cali(void)
{
	APS_FUN();
	return 0;
}
static int stk3x3x_als_factory_set_cali(int32_t offset)
{
	APS_FUN();
	return 0;
}
static int stk3x3x_als_factory_get_cali(int32_t *offset)
{
	APS_FUN();
	return 0;
}
static int stk3x3x_ps_factory_enable_sensor(bool enable_disable, int64_t sample_periods_ms)
{
	int err = 0;
...
static int stk3x3x_ps_factory_get_data(int32_t *data)
{
	int err = 0, status = 0;
...
static int stk3x3x_ps_factory_get_raw_data(int32_t *data)
{
	int err = 0;
	struct stk3x3x_priv *obj = stk3x3x_obj;

	APS_FUN();
	err = stk3x3x_read_ps(obj->client, &obj->ps);
	if (err) {
		APS_ERR("%s failed\n", __func__);
		return -1;
	}
...
static int stk3x3x_ps_factory_enable_calibration(void)
{
	APS_FUN();
	return 0;
}
static int stk3x3x_ps_factory_clear_cali(void)
{
	struct stk3x3x_priv *obj = stk3x3x_obj;
...
static int stk3x3x_ps_factory_set_cali(int32_t offset)
{
	struct stk3x3x_priv *obj = stk3x3x_obj;
...
static int stk3x3x_ps_factory_get_cali(int32_t *offset)
{
	struct stk3x3x_priv *obj = stk3x3x_obj;
...
#if 0
static int stk3x3x_ps_factory_set_threashold(int32_t threshold[2])
{
	int err = 0;
	struct stk3x3x_priv *obj = stk3x3x_obj;
	...
	err = stk3x3x_write_ps_high_thd(obj->client, atomic_read(&obj->ps_high_thd_val));
	err = stk3x3x_write_ps_low_thd(obj->client, atomic_read(&obj->ps_low_thd_val));
...
static int stk3x3x_ps_factory_get_threashold(int32_t threshold[2])
{
	struct stk3x3x_priv *obj = stk3x3x_obj;
...
#endif
static struct alsps_factory_fops stk3x3x_factory_fops = {
	.als_enable_sensor = stk3x3x_als_factory_enable_sensor,
	.als_get_data = stk3x3x_als_factory_get_data,
	.als_get_raw_data = stk3x3x_als_factory_get_raw_data,
	.als_enable_calibration = stk3x3x_als_factory_enable_calibration,
	.als_clear_cali = stk3x3x_als_factory_clear_cali,
	.als_set_cali = stk3x3x_als_factory_set_cali,
	.als_get_cali = stk3x3x_als_factory_get_cali,

	.ps_enable_sensor = stk3x3x_ps_factory_enable_sensor,
	.ps_get_data = stk3x3x_ps_factory_get_data,
	.ps_get_raw_data = stk3x3x_ps_factory_get_raw_data,
	.ps_enable_calibration = stk3x3x_ps_factory_enable_calibration,
	.ps_clear_cali = stk3x3x_ps_factory_clear_cali,
	.ps_set_cali = stk3x3x_ps_factory_set_cali,
	.ps_get_cali = stk3x3x_ps_factory_get_cali,
	//.ps_set_threashold = stk3x3x_ps_factory_set_threashold,
	//.ps_get_threashold = stk3x3x_ps_factory_get_threashold,
};

static struct alsps_factory_public stk3x3x_factory_device = {
	.gain = 1,
	.sensitivity = 1,
	.fops = &stk3x3x_factory_fops,
};
...
static int stk3x3x_init_reg(struct i2c_client *client)
{
	...
	client->addr = 0x47;//stk3337
	//client->addr = 0x57;//origin IC
	printk("client->addr======%x\n", client->addr);
	obj->client = client;
	...
	i2c_config_reg = STK_PS_PRS4 | STK_PS_GAIN2 | STK_PS_IT100;
...
static int stk3x3x_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	int ret;
	APS_LOG("%s: driver version: %s\n", __FUNCTION__, STK3X3X_DRIVER_VERSION);
	...
	printk(" kzhkzh node is %s,ret:%d\n",client->dev.of_node->name,ret);
	if (err < 0)
	{
		printk("get customization info from dts failed\n");
	...
	stk3x3x_obj = obj;
	//obj->hw = hw;
	printk(" kzhkzh %s:%d obj->hw.polling_mode_ps:%d,obj->hw.polling_mode_als:%d\n",__func__,__LINE__,obj->hw.polling_mode_ps,obj->hw.polling_mode_als);
		//mid add for 光感轮询，距感中断
...
/*
	...
	APS_LOG("stk3x3x_init_client() OK!\n");
	err = alsps_factory_device_register(&stk3x3x_factory_device);
	if(err)
	{
...
static int stk3x3x_i2c_remove(struct i2c_client *client)
{
	...
	alsps_factory_device_deregister(&stk3x3x_factory_device);
