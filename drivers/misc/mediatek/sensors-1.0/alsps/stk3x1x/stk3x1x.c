...
int stk3x1x_read_id(struct i2c_client *client)
{
	...
	else if ((buf[0] == STK3311X_PID)||(buf[0] == 0x15))
...
static struct alsps_factory_fops stk3x1x_factory_fops = {
	...
	.ps_set_threshold = stk3x1x_ps_factory_set_threashold,
	.ps_get_threshold = stk3x1x_ps_factory_get_threashold,
};
...
//	obj->irq_node = client->dev.of_node;
	...
	if (obj->hw.polling_mode_als == 0) {
		...
		APS_ERR("%s: enable ALS interrupt\n", __func__);
	}
	...
	APS_LOG("stk3x1x_device misc_register OK!\n");
	...
	if (obj->hw.polling_mode_ps == 1) {
		...
		ps_lock = wakeup_source_register(NULL,"ps wakelock");
		printk("%s: enable PS polling mode\n", __func__);
	} else {
	...
	stk3x1x_init_flag = 0;
	APS_ERR("%s: state_val=0x%x, psctrl_val=0x%x, alsctrl_val=0x%x\n",
		__func__, atomic_read(&obj->state_val),
		atomic_read(&obj->psctrl_val), atomic_read(&obj->alsctrl_val));
	APS_ERR("ledctrl_val=0x%x, wait_val=0x%x, int_val=0x%x\n",
		obj->ledctrl_val, obj->wait_val, obj->int_val);
	APS_ERR("%s: OK\n", __func__);
	return 0;
